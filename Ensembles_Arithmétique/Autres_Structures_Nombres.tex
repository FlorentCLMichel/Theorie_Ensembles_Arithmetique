...
\noindent\textbf{Démonstration :}
    ...
    \end{equation*}
    Pour tout entier naturel $n$ supérieur ou égal à $n_1$, on a donc $\abs{q - v_n} \leq \frac{\abs{q}}{2}$, donc, d'après l'inégalité triangulaire, $\abs{q} - \abs{v_n} \leq \abs{q} \divslash 2$, et donc $\abs{v_n} \geq \abs{q} \divslash 2$. 
