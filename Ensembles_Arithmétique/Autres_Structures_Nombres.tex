...
\noindent\textbf{Démonstration :} Soit $u$ uns suite de nombres rationnels majorée et croissante. 
    Soit $M$ un majorant de $u$.
    On suppose par l'absurde qu'elle n'est pas de Cauchy. 
    On peut donc choisir un nombre rationnel $\epsilon$ strictement positif tel que, pour tout entier naturel $n$, il existe eux entiers naturels $m$ et $k$ supérieurs ou égaux à $n$ tels que $\abs{u_m - u_k} > \epsilon$. 
    Montrons par récurrence sur $l$ que, pour tout entier naturel $l$, on peut choisir un entier naturel $n$ tel que $u_n > l \times \epsilon$. 
    Soit $v_n$ la suite définie par : pour tout entier naturel $n$, $v_n = n \divslash 1$. 
    On aura alors $u_n > \epsilon \times v_n$ pour tout entier naturel $n$.
    Puisque $v$ tend vers $+\infty$ et puisque $\epsilon > 0$, $\epsilon \times v$ tend vers $+\infty$, et donc $u$ également, ce qui est impossible puisque $u$ est majorée.
...
\textbf{Remarque :} La réciproque est fausse.
    ...
    \begin{itemize}[nosep]
        \item $u_0 = 1$
        \item pour tout entier naturel $n$, $u_{n+1} = u_n + \left(2 - (u_n \times u_n) \right) \divslash 2$ .
    \end{itemize}
   
    Supposons que la suite $u$ admette une limite $l$. 
    Définissons les suites $v$ et $w$ par : pour tout entier naturel $n$, $v_n = u_{n+1}$ et $w_n = u_n + \left(2 - (u_n \times u_n) \right) \divslash 2$.
    Alors, $v$ tend vers $l$ et $w$ tend vers $l + (2 - l \times l) \divslash 2$. 
    Puisque $v_n = u_n$ pour tout entier naturel $n$, on a donc $l = l + (2 - l \times l) \divslash 2$, donc $(2 - l \times l) \divslash 2 = 0$, donc $2 - l \times l = 0$, et donc $l \times l = 2$.
    ...
    ***
    ...
    Pour ce faire, on montre d'abord par récurrence que $u_n \times u_n \leq 2$ pour tout entier naturel $n$. 
    Cette suite est donc majorée—en effet, pour tout entier naturel $n$, on aura $u_n \leq 2$ (sans quoi on aurait $u_n \times u_n > 4 > 2$). 
    Pour tout entier naturel $n$, on aura alors $\left(2 - (u_n \times u_n) \right) \divslash 2 \geq 0$, montrant que la suite $u$ est croissante, et donc (car majorée et croissante) de Cauchy.
