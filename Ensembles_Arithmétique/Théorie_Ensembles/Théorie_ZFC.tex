...
\end{equation*}
Cet ensemble est unique d'après l'axiome d'extensionnalité. 
L'ensemble des parties (ou enseble des sous-ensembles) d'un ensemble $x$ est aussi appelé \emph{ensemble puissance} de $x$ et noté $\mathcal{P}(x)$.
\aindex{Ensemble puissance}
...
\noindent\textbf{Notation :} Soit $E$ un ensemble et $P$ un prédicat dépendant des variables $x$, $a$, ..., $b$.
    On peut noter par 
    \begin{itemize}[nosep]
...
\noindent\textbf{Définition :} \index{Ensemble quotient} Soit $E$ un ensemble et $\mathcal{R}$ une relation d'équivalence sur $E$.
    L'ensemble des classes d'équivalence de $\mathcal{R}$ est noté $E \divslash \mathcal{R}$ et appelé \emph{ensemble quotient} de $E$ par $\mathcal{R}$.
...
Pour tout sous-ensemble $Y'$ de $Y$, on note $f^{-1}(Y')$ l'ensemble $\lbrace x \in X \vert f(x) \in Y' \rbrace$, appelé \textit{image inverse}\index{Image inverse} de $G$ par $f$. 
...
\noindent\textit{\textbf{Énoncé :} \index{Choix (axiome)} Pour tout ensemble $X$ d'ensembles non vides, il existe une fonction sur $X$ qui à chaque ensemble $x$ appartenant à $X$ associe un élément de $x$ :}
\begin{equation*}
...
\subsubsection{Théorie de \aindex{Tarski-Grothendieck}}

\noindent\textbf{Définition :} \aindex{Univers} \aindex{Grothendieck} Un ensemble $U$ est dit \emph{univers de Grothendieck} si les quatre proptiétés suivantes sont vraies : 
\begin{itemize}[nosep]
...
\noindent\textit{\textbf{Axiome de Tarski :}} Pour tout ensemble $E$, il existe un univers de Grothendieck $U$ tel que $E \in U$.
