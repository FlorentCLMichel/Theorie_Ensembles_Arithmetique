...
\noindent \textbf{Premier théorème d'incomplétude de Gödel :} Sous ces conditions, il existe une formule $f$ de la théorie sans variable libre dont on ne peut montrer (par un algorithme fini) ni qu'ellle est vraie ni qu'elle est fausse.
...
La numérotation originelle de Gödel, que nous nommerons dans la suite simplement \textit{encodage de Gödel} par la suite, noté $\mathrm{enc}$, est obtenue de la manière suivante : 
\begin{itemize}[nosep]
    ...
    \item Si $n$ est un entier naturel et $x_1$, $x_2$, ..., $x_n$ sont des symboles, le nombre associé à la formule $x_1 x_2 \dots x_n$ est $p_1^{x_1} \times p_2^{x_2} \times \cdots p_n^{x_n}$.
        Plus formellement, $\mathrm{enc}(x_1 x_2 \dots x_n) = \prod_{i=1}^n p_i^{x_i}$. 
\end{itemize}
...
\noindent \textbf{Exemple :} Si trois variables primaires $x$, $y$ et $z$ sont représentées respectivement par les nombres $1$, $2$ et $3$, si $+$ et $=$ sont respectivement représentée par les nombres $4$ et $5$, et si la suite $p$ commence par $(2, 3, 5, 7, 11)$, alors $\mathrm{enc}(x + y = z) = 2^1 \times 3^4 \times 5^2 \times 7^5 \times 11^3 = 90598973850$.
