...
La logique du premier ordre\index{Logique du premier ordre}, aussi appelée \textit{logique des prédicats} ou \textit{calcul des prédicats du premier ordre}, est un cadre semi-formel%
...
Ces dernières sont aussi appelées \textit{propositions}, \textit{énoncés} ou \textit{prédicats}.\index{Formule}\index{Enonce@Énoncé}\index{Prédicat}\index{Proposition} 

Elle est une extension de la \textit{logique propositionelle}, qui exprime des énoncés, ou \textit{propositions}, aussi appelés \textit{prédicats}, auxquels on attribue une valeur dite de \textit{vérité} : vrai ou faux.\index{Valeur de vérité}
...
\begin{itemize}[nosep]
    \item Un \textit{alphabet}, c'est-à-dire un ensemble (au sens intuitif du terme) de symboles, dont certaines chaînes forment des \textit{termes}.\index{Alphabet}\index{Symbole} 
    ...
    \item Des \textit{formules} (aussi appelées \textit{propositions}) obtenues à partir de ces règles, exprimant des prédicats. 
        ...
        Deux formules seront dites \textit{équivalentes} si elles prennent toujours la même valeur de vérité.\index{Formules équivalentes} 
        \begin{itemize}[nosep]
        ...
        \end{itemize}
    \item Un ensemble d'\textit{axiomes}, ou propositions tenues pour vraies.\index{Axiome} 
...
On définit également les constantes de vérité $\mathsf{V}$ pour « vraie » et $\mathsf{F}$ pour « fausse ».\index{Vrai}\index{Faux} 
Elles sont deux formules, et $\mathsf{F}$ est équivalente à $\neg \mathsf{V}$.\sindex[isy]{$\mathsf{V}$}\sindex[isy]{$\mathsf{F}$}
...
\index{Egalite@Égalité}\sindex[isy]{$=$}
...
\index{Unicité}\sindex[isy]{$\exists ^^21$}
...
\index{Indéfinie}\sindex[isy]{$\mathsf{I}$}
...
On suppose un système d'axiomes permettant de définir un ensemble de nombres $\mathbb{N}$ satisfaisnat les propriétés établies en section~\ref{sub:constN}. 
...
On considère qu'un algorithme à un nombre fini d'instructions démontrant une formule $F$ peut être décrit par une formule de la théorie, de la forme $P \Rightarrow F$, où $P$ est soit un axiome de la théorie soit une formule démontrable.
...
La numérotation originelle de Gödel, que nous nommerons dans la suite \textit{encodage de Gödel}, noté $\mathbf{G}$, est obtenue de la manière suivante : 
\begin{itemize}[nosep]
