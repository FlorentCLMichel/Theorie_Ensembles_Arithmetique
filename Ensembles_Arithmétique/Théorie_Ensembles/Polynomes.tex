...
\begin{itemize}[nosep]
    \item \textit{Addition :} 
        ...
        \begin{itemize}[nosep]
            ...
            \item Sinon, on a $n \leq \mathrm{deg}(\mathbf{a}) \vee n \leq \mathrm{deg}(\mathbf{b})$. 
                On a alos trois possibilités.\footnote{
                ...
                \begin{itemize}[nosep]
                    \item Si $n \leq \mathrm{deg}(\mathbf{a}) \vee n > \mathrm{deg}(\mathbf{b})$, alors $\mathbf{b}_n = 0$, donc $e_n = \mathbf{a}_n$. 
                        Avec les notations de la définition, cela donne $e_n = a_n$.
                    \item Si $n > \mathrm{deg}(\mathbf{a}) \vee n \leq \mathrm{deg}(\mathbf{b})$, alors $\mathbf{a}_n = 0$, donc $e_n = \mathbf{b}_n$. 
                        Avec les notations de la définition, cela donne $e_n = b_n$.
                    \item Si $n \leq \mathrm{deg}(\mathbf{a}) \vee n \leq \mathrm{deg}(\mathbf{b})$, alors $e_n = \mathbf{a}_n + \mathbf{b}_n$. 
    ...
    \item \textit{Multiplication :} 
        ...
        Motrons que, pour tout entier naturel $n$, $f_n = 0$ si $n > \mathrm{deg}(\mathbf{a}) + \mathrm{deg}(\mathbf{b})$ (et donc $f_n = \mathbf{d}_n$) et $f_n = \mathbf{d}_n$ sinon.
        \begin{itemize}[nosep]
            ...
            \item Soit $n$ un entier naturel inférieur ou égal à $\mathrm{deg}(\mathbf{a}) + \mathrm{deg}(\mathbf{b})$.
                ...
                Pour tout entier naturel $i$ inférieur ou égal à $n$, $\mathbf{a}_i \times \mathbf{b}_{n-i}$ n epeut donc être non nul que si $n - \mathrm{deg}(\mathbf{b}) \leq i \leq \mathrm{deg}(\mathbf{a})$.
                Puisque, avec les mêmes notations $0 \leq i$, l'inégalité de gauche implique $\mathrm{max}(0, n - \mathrm{deg}(\mathbf{b})) \leq i$.
                Puisque, avec les mêmes notations $i \leq n$, l'inégalité de droite implique $i \leq \mathrm{min}(n, \mathrm{deg}(\mathbf{a}))$.
                On a donc $\sum_{i = 0}^n \mathbf{a}_i \times \mathbf{b}_{n-i} = \sum_{i = \mathrm{max}(0, n - \mathrm{deg}(\mathbf{b}))}^{\mathrm{min}(n, \mathrm{deg}(\mathbf{a}))} \mathbf{a}_i \times \mathbf{b}_{n-i}$, ce qui montre que $\mathbf{d}_n$ est bien le coefficnet d'indice $n$ du polynôme $\mathbf{a} \times \mathbf{b}$.
        \end{itemize}
\end{itemize}
