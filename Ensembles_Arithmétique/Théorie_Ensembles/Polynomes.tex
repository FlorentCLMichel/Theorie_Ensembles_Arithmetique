...
\noindent\textbf{Définition (\aindex{Polynôme}) :} Soit $(A, +, \times)$ un anneau commutatif. 
    ...
    \begin{itemize}[nosep]
        \item Définition de $\mathbf{A}$ : Pour tout $x$, $x \in \mathbf{A}$ si et seulement si on peut choisir un entier naturel $n$, appelé \textit{degré\index{Degré}} de $x$ et noté $\mathrm{deg}(x)$, et $n+1$ éléments $a_0, a_1, \dots, a_n$ de $A$ tels que $x = (a_0, a_1, \cdots, a_n)$ et $a_n \neq 0_A \vee n = 0$.
...
\noindent\textbf{Définition (\aindex{Degré}) (rappel) :} Soit $(A, +, \times)$ un anneau commutatif et $\mathbf{a}$ un élément de $A[X]$.
