...
\noindent\textbf{Démonstration :} 
    ...
    Supposons maintennat $\abs{E} > 0$.
    
    ******

    \textit{(supposer par l'absurde que ce n'est pas le cas, choisir un élément $y$ de $F$ ayant plus d'un antécédent pat $f$, appeler $E_y$ l'ensemble de ses antécédents montrer que $\abs{E \setminus E_y} < \abs{E} - 1$, définir $g$ par $g = f \setminus \lbrace z \in E \times F \vert \exists x \in E_y \, z = (x,y) \rbrace$, montrer que $g$ est une fonction de $E \setminus E_y$ vers $F \setminus \lbrace y \rbrace$ est qu'elle est surjective ; cela est contradictoire puisque $\abs{F \setminus \lbrace y \rbrace} = \abs{F} - 1 = \abs{E} - 1$, donc $\abs{F \setminus \lbrace y \rbrace} > \abs{E - E_y}$)} 
