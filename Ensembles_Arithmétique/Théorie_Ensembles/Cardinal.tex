...
\noindent\textbf{Démonstration :} 
    ...
    Puisque le cardinal de $E$ n'est pas $0$, $E$ admet au moins un élément (puisque $\emptyset$ est inclus dans tout entier). 
    ...
    En outre, pour tout élément $x$ de $E'$, on a $x \in F$ (puisque $E \subset F$) et $x \neq e$, donc $x \in F'$.
    Donc, $E' \subset F'$. 
    On en déduit que $E' = F'$, et donc que $E = F$. 

    Par récurrence, ce résultat est vrai pour tout entier $n$.
...
\noindent\textbf{Lemme :} Soit $E$ et $F$ deux ensembles. 
    ...
    En outre, d'après le lemme précédent, $\abs{F} \leq \abs{E}$ si et seulement si $F = E$.
...
\noindent\textbf{Démonstration :} 
    ...
    Donc, $E \setminus \lbrace x \rbrace$ est de cardinal $n-1$.
...
\noindent\textbf{Lemme :} L'ensemble $\mathbb{N}$ a un cardinal infini. 
...
\noindent\textbf{Démonstration :} Montrons par récurrence qu'il n'existe aucune bijection entre un entier naturel $n$ et $\mathbb{N}$. 
    ...
    On procède par l'absurde : on suppose qu'une telle bijection existe, notée $f$ dans la suite, et on montre que cela aboutie à une contradiction. 
    Définissons la fonction $g$ de $n$ vers $\mathbb{N}$ par : pour tout élément $x$ de $n$, $g(x) = f(x)$ si $f(x) < f(n)$ et $g(x) = f(x)-1$ sinon. 
    (Cela est possible car, puisque $f$ est injective, on a $f(x) > f(n)$ dans le second cas, et donc $f(x) > 0$.)
    ...
    Donc, $f(x)-1 = f(y)-1$, et donf $f(x) = f(y)$.
    Puisque $f$ est injective, on a donc $x=y$. 
    Si $f(x) < f(n)$ et $f(y) > f(n)$, alors $g(y) = f(y)-1$, d'où $g(y) \leq f(n)$, ce qui est impossible.
    De même, $f(x) > f(n)$ et $f(y) < f(n)$ est impossible. 
    ...
    Si $m < f(n)$, on peut choisir un élément $x$ de $n+1$ tel que $f(x) = m$ et $x \neq m$, et donc $n \in n$ et $g(x) = m$.
    Si $m \geq f(n)$, $m+1 > f(n)$, donc on peut choisir un élément $x$ de $n+1$ tel que $n \neq n$ $f(x) = m+1$, et donc $g(x) = m$.
...
\noindent\textbf{Lemme :} Soit $n$ un entier naturel tel que $n \neq 0$. 
...
\noindent\textbf{Démonstration :} Puisque $n \neq 0$, $E \neq \emptyset$. 
    ...
    D'après un lemme précédent, le cardinal de $F$ est $n-1$. 
    En outre, on a $E = F \cup \lbrace x \rbrace$, ce qui montre lemme. 
    ...
    \begin{itemize}[nosep]
        ...
        \item Soit $y$ un élément de $F \cup \lbrace x \rbrace$. 
            ...
            Si $x \in \lbrace x \rbrace$, qlors $y = x$, et donc $y \in E$. 
...
\noindent\textbf{Démonstration :}
    ...
    Montrons maintennat que l'ensemble $G$ défini par $G = F_1 \setminus \lbrace x_2 \rbrace$ satisfait : $G = F_2 \setminus \lbrace x_1 \rbrace$. 
...
\noindent\textbf{Démonstration :} On procède par récurrence sur le cardinal de $E$, noté $n$.
    ...
    Soit $E$ et $F$ deux esembles de cardinal $n+1$ et $f$ une injection de $E$ vers $F$. 
    ...
    Puisque ces deux derniers ensembles sont de cardinal $n$, on en déduit que $g$ est une bijection. 
