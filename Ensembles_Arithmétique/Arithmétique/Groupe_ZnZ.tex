    ...
    \begin{tikzpicture}
        ...
        \foreach[evaluate=\i as \im using {Mod(\i-1, \nCirclesPerLine)}] \i in {1, ..., \maxN} {
            ...
            \foreach[evaluate=\j as \k using int(\j-1)] \j in {1, ..., \i} {
                \fill[black] ({\im * \figHSpace + \figRadius * sin(360*\k/\i)}, {\figRadius * cos(360*\k/\i) - \line * \figVSpace}) circle(\figPointSize);
                \node at ({\im * \figHSpace + \figRadiusL * sin(360*\k/\i)}, {\figRadiusL * cos(360*\k/\i) - \line * \figVSpace}) {$\k$};
            }
        }
    \end{tikzpicture}
