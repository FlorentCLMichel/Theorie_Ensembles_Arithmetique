...
Un entier naturel $p$ est dit \textit{premier}\index{Nombre premier} s'il admet exactement deux entiers naturels distincts pour diviseurs: $1$ et lui-même.
...
\noindent\textbf{Définition :} Soit $a$ et $b$ deux entiers naturels non nuls. L'ensemble de leurs multiples communs non nuls est un sous-ensemble de $\mathbb{N}$ non vide (il contient au moins $a \times b$). Il admet donc un unique élément minimal, appelé \textit{plus petit commun multiple}, ou \textit{ppcm}\index{PPCM}, de $a$ et $b$. 
...
\subsubsection{Nombres premiers particuliers}

\noindent\textbf{Définition :}\index{Nombre de Fermat} Un \emph{nombre de Fermat} est un nombre premier $p$ satisfaisant $p = 2^n + 1$ pour un entier naturel $n$.
