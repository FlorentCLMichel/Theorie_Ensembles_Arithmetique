...
\begin{equation*}
    ...
    \rbrace .
\end{equation*}
L'ensemble $E$ est un sous-ensemble non vide (il contient au moins $a$, obtenu pour $k=0$) de $\mathbb{N}$, donc il admet un unique élément minimal (et donc un minimum) $r$. 
...
En outre, $0 \leq r < b$. 
En effet, $r$ est un entier naturel par définition de $E$, donc $r \geq 0$ et, si $r$ était supérieur ou égal à $b$, on aurait $r - b \in \mathbb{N}$ et $r - b = (a - q b) - b = a - (q b + b) = a - (q+1) b$, et donc $r - b \in E$. 
Puisque $b > 0$, $r - b < r$ ; cela contredirait donc la définition de $r$ comme minimum de $E$. 

Notons aussi que $r$ est le seul élément de $E$ compris entre $0$ et $b-1$ (inclus). 
...
Notons que, si $a$ est non nul, un diviseur $b$ de $a$ satisfait $b \leq a$. 
...
Notons que tout entier naturel divise $0$.
(Soit $b$ un entier naturel, on a $0 \times b = 0$, donc $0$ divise b.)
Notons aussi que tout entier naturel non nul se divise lui-même. 
En effet, soit $a$ un entier naturel non nul, on a $a = 1 \times a$.
