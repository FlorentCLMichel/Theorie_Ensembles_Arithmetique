\startsection[title=Nombres premiers]

\startsubsection[title=Définition, reference=sub:defNombresPremiers]

Un entier naturel $p$ est dit \emph{premier}\index{Nombre premier} s'il admet exactement deux entiers naturels distincts pour diviseurs: $1$ et lui-même.
...
Deux nombres premiers $p$ et $q$ sont dits \emph{jumeaux} si $p-q=2$ ou $q-p=2$. 

\blank[big]

\noindent\bold{Lemme :} Tout entier naturel strictement supérieur à $1$ est divisible par au moins un nombre premier.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence forte. 
...
\noindent\bold{Lemme :} Il existe une infinité de nombres premiers.

\blank[medium]

\noindent\bold{Démonstration :} Supposons par l'absurde que l'ensemble des nombres premiers et fini. 
...
\noindent\bold{Lemme :} Soit $p$, $q$ et $r$ trois nombres premiers tels que $r > q$, $q > p$ et $r-q = q-p$. Alors $p=3$ ou $q-p$ est divisible par $3$.

\blank[medium]

\noindent\bold{Démonstration :} Notons $a$ l'entier $q-p$. 
...
Montrons que les autres cas, \emph{i.e.} ceux où $b$ et $c$ sont chacun égaux à $1$ ou $2$, sont impossibles. 
...
Des fonctions Haskell déterminant si un entier naturel est premier et calculant les premier nombres premiers sont données en \in{appendice}{}[app:Haskell_primalité]. 
(Ces fonctions sont données à titre d'illustration uniquement, et ne sont pas particulièrement efficaces. 
En particulier, il est en général plus rapide pour calculer les premiers nombre premiers d'utiliser le crible d'Érastosthène, dont une implémentation en C++ et Rust est donnée en \in{appendice}{}[app:code_erastosthene].)

\stopsubsection

\subsection[title=Théorème de Bachet-Bézout]

\blank[big]

\noindent\bold{Théorème (Bachet-Bézout) :} 
...
\noindent\bold{Corolaire :} Soit $a$, $b$, et $c$ trois entiers tels que $a$ et $c$ sont premiers entre eux et $b$ et $c$ sont premiers entre eux. Alors $ab$ et $c$ sont aussi premiers entre eux.

\blank[medium] 

\noindent\bold{Démonstration :} Soit $d$ un diviseur de $ab$ et $c$. 
...
\noindent\bold{Corolaire :} Soit $a$ et $b$ deux entier naturels non nuls, et $c$ un entier naturel non nul premier avec $a$ et avec $b$. 
...
\noindent\bold{Corolaire :} Soit $n$ un entier naturel non nul et $a_1$, $a_2$, ..., $a_n$ des entiers naturels non nuls (où $a_n$ est absent si $n \leq 2$ et $a_2$ est absent si $n = 1$). 
...
\noindent\bold{Corolaire :} Soit $a$ et $b$ deux entier premier entre eux, et $c$ un entier. Si $a$ et $b$ divisent $c$, alors $ab$ divise $c$.
...
\noindent\bold{Corolaire :} Soit $p$ un entier naturel strictement supérieur à $1$, $a_1$, $a_2$, ..., $a_p$ des entiers naturels deux-à-deux premier entre eux, et $b$ un entier. Si $a_i$ divise $b$ pour tout élément $i$ de $[\![1,p]\!]$, alors $a_1 \times a_2 \times \cdots \times a_p$ divise $b$.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur $p$. 
...
\noindent\bold{Lemme :} Soit $a$, $b$ et $c$ trois entiers naturels tels que $a \vert b c$ et $a$ et $b$ sont premiers entre eux. 
    Alors, $a \vert c$.

\blank[medium]

\noindent\bold{Démonstration :} Puisque $a$ et $b$ sont premiers entre eux, on peux choisir deux entiers $u$ et $v$ tels que $u a + v b = 1$. 
    Donc, $u a c + v b c = c$.
...
\subsection[title=Plus petit commun multiple]

\noindent\bold{Définition :} Soit $a$ et $b$ deux entiers naturels non nuls. L'ensemble de leurs multiples communs non nuls est un sous-ensemble de $\mathbb{N}$ non vide (il contient au moins $a \times b$). Il admet donc un unique élément minimal, appelé \emph{plus petit commun multiple}, ou \emph{ppcm}\index{PPCM}, de $a$ et $b$. 

\blank[medium]

\noindent\bold{Lemme :} Soit $a$ et $b$ deux entiers naturels non nuls, $c$ leur pgcd, et $d$ leur ppcm.
...
\startsubsection[title=Théorème du reste chinois]

\blank[medium]

\noindent\bold{Théorème :} Soit $p$ un entier naturel strictement supérieur à $1$. Soit $n_1$, $n_2$, ..., $n_p$ des entiers naturels strictement supérieurs à $1$ deux-à-deux premiers entre eux. 
...
\noindent\bold{Démonstration :} 
\startitemize
    \item \emph{Unicité :} Supposons avoir deux tels entiers $n$ et $m$. 
...
\noindent\bold{Corolaire :} Soit $a$ et $b$ deux entiers naturels premiers entre eux et strictement supérieurs à $2$. Alors il existe au moins quatre entiers naturels $x_1$, $x_2$, $x_3$ et $x_4$ deux à deux distincts, chacun strictement inférieur à $ab$ et tels que, pour tout élément $i$ de $\lbrace 1, 2, 3, 4 \rbrace$, $x_i^2 \equiv 1 \, [a b]$.
...
\startsubsection[title=Décomposition en produit de facteurs premiers, reference=sub:dec_fact_prem]

\blank[big]

\noindent\bold{Théorème :} 
Tout entier naturel non nul peut s'écrire comme un produit de facteurs premiers, \emph{i.e.}, pour tout entier naturel non nul $a$, il existe un entier naturel $N$, un $N$-uplet $\left( p_1, p_2, \dots, p_N \right)$ de nombres premiers deux-à-deux distincts et un $N$-uplet $\left( n_1, n_2, \dots, n_N \right)$ d'entiers naturels non nuls tels que
\startformula
    a = \prod_{i=1}^N p_i^{n_i}.
\stopformula
Le couple $\left( \left( p_1, p_2, \dots, p_N \right), \left( n_1, n_2, \dots, n_N \right)\right)$ est appelé \emph{décomposition de $a$ en produit de facteurs premiers}.
...
\startitemize
    \item \emph{Existence :} On procède par récurrence forte sur $a$. 
...
\noindent\bold{Remarque :} Soit $N$ un entier naturel non nul. 
...
\startsubsection[title=Représentation schématique]
...
\startitemize[nowhite]
    \item Soit $a$ et $b$ deux entiers naturels non nuls.
        ...
        \emph{Exemple 1 :} $80$ est un multiple de $20$ :
        \startitemize[nowhite]
            \item $80$: \inbox[color=blue]{2}\inbox[color=blue]{2}\inbox[color=blue]{5}\inbox{2}\inbox{2}
            \item $20$: \inbox[color=blue]{2}\inbox[color=blue]{2}\inbox[color=blue]{5}
        \stopitemize
        \emph{Exemple 2 :} $80$ n'est pas un multiple de $60$ :
        \startitemize[nowhite]
            \item $80$: \inbox[color=blue]{2}\inbox[color=blue]{2}\inbox[color=blue]{5}\inbox{2}\inbox{2}
            \item $60$: \inbox[color=blue]{2}\inbox[color=blue]{2}\inbox[color=blue]{5}\inbox[color=red]{3}
        \stopitemize
    \item Soit $a$ et $b$ deux entiers naturels non nuls et $c$ leur pgcd.
        ...
        \emph{Exemple :} Le pgcd de $100$ et $30$ est $10$
        \startitemize[nowhite]
            \item $10$: \inbox[color=blue]{2}\inbox[color=blue]{5}\inbox[color=green!50!black]{2}\inbox[color=green!50!black]{5}
            \item $30$: \inbox[color=blue]{2}\inbox[color=blue]{5}\inbox[color=red]{3}
            \item \color[blue]{$2 \times 5 = 10$}
        \stopitemize
\stopitemize

\stopsubsection

\startsubsection[title=Nombres premiers particuliers]

\noindent\bold{Définition :}Un \emph{\words{nombre de Fermat}} est un nombre premier $p$ satisfaisant $p = 2^n + 1$ pour un entier naturel $n$.

\blank[medium]

\noindent\bold{Lemme :} Soit $n$ un entier naturel tel que $2^n + 1$ est premier (donc un nombre de Fermat). 
    Alors $n$ est une puissance de $2$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $l$ la puissance de $2$ dans la décomposition de $n$ en produit de facteurs premiers. 
...
\startsubsection[title=Petit théorème de Fermat]

\noindent\bold{Théorème :} Soit $p$ un nombre premier et $a$ un entier naturel non multiple de $p$. 
Alors, $a^{p-1} \equiv 1 \, [p]$.
\index{Fermat (petit théorème)}

\blank[medium]

\noindent\bold{Démonstration :} Pour tout élément $i$ de $[\![1, p-1]\!]$, on définit l'entier naturels $a_i$ par $a_i = i a$ et on note $r_i$ le reste de la division euclidienne de $a_i$ par $p$. 
...
\noindent\bold{Remarque :} Avec les mêmes notations, il n'est pas toujours vrai que $p-1$ est le plus petit entier naturel $n$ tel que $a^n \equiv 1 \, [p]$.
...
\noindent\bold{Lemme :} Soit $p$ un nombre premier. 
...
\noindent\bold{Remarque :} Le dernier résultat n'est pas vrai en général si $s$ et $p-1$ ne sont pas premiers entre eux.
    ...
    Puisque $g > 1$, $m < p-1$. 
    Puisque le groupe $(\mathbb{Z} \divslash (p \mathbb{Z}))^*$ est cyclique (voir \in{section}{}[subsec:cyclicity_Z_pZ]), il admet au moins un générateur. 
...
\noindent\bold{Démonstration :} Soit $i$ un élément de $[\![1, n]\!]$.
