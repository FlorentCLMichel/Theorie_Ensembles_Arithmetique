\startsection[title=Concepts fondamentaux]

\startsubsection[title=Division euclidienne, reference=sub:divEuclidienne]
...
\noindent\bold{Définition (\words{division euclidienne}) :}
...
Cet élément est appelé \emph{\words{reste} de la division euclidienne de $a$ par $b$}. 
...
est appelé \emph{\words{quotient} de la division euclidienne de $a$ par $b$}. 
Notons que l'on a : $a = q b + r$. 

\blank[medium]

\noindent\bold{Définition :} On définit les deux opérations $\symbols{\divslash\!\divslash}$ et $\symbols{\char"0025}$ de $\mathbb{N} \times \mathbb{N}^*$ vers $\mathbb{N}$ de la manière suivante : pour tout entier naturel $a$ et tout entier naturel non nul $b$, $a \mathrel{\divslash\!\divslash} b$ est le quotient de la division Euclidienne de $a$ par $b$ et $a \mathrel{\%} b$ est son reste. 

\blank[medium]

\noindent\bold{Lemme :} Soit $a$ un entier naturel et $b$ un entier naturel non nul.
...
\noindent\bold{Définition (diviseur, multiple) :} 
    Soit $a$ et $b$ deux entiers naturels avec $b \neq 0$.
    Si le reste de la division euclidienne de $a$ par $b$ est $0$, on dit que \emph{$b$ divise $a$}, ou encore que \emph{$b$ est un diviseur de $a$}, que \emph{$a$ admet $b$ pour diviseur} ou que \emph{$a$ est un multiple de $b$}. 
...
\noindent\bold{Lemme :} Soit $a$ et $b$ deux entiers naturels. 
...
\noindent\bold{Définition :} Un entier naturel est dit \emph{pair} s'il est un multiple de $2$ et \emph{impair} sinon. 

\blank[medium]

\noindent\bold{Définition :} Soit $a$ et $b$ deux entiers naturels non nuls. L'ensemble de leurs diviseurs communs est un sous-ensemble de $\mathbb{N}$ non vide (il contient au moins $1$) et borné supérieurement par le minimum de $a$ et $b$. Il admet donc un unique élément maximal, appelé \emph{plus grand diviseur commun}, ou \emph{\words{pgcd}}, de $a$ et $b$.
...
\noindent\bold{Définition :} Deux entiers naturels $a$ et $b$ sont dits \emph{premiers entre eux} si leur pgcd est $1$.

\blank[medium]

\noindent\bold{Lemme :} Soit $a$ et $b$ deux entiers naturels non nuls et $c$ leur pgcd.
...
Des fonctions Haskell, C et Rust calculant le pgcd de deux entiers naturels non nuls sont données en \in{appendice}{}[app:Haskell_pgcd]. 

\stopsubsection

\startsubsection[title=Modulo]
\index{Modulo}

Soit $p$, $q$ et $r$ trois entiers relatifs. 
On écrit $p \symbols{\equiv} r \, [q]$, ou $p \equiv r \, \symbols{\mathss{mod}} \, q$, ou encore $p \equiv r \, ( \mathss{mod} \, q)$ le prédicat : il existe un entier relatif $k$ tel que $p = r + k q$, \emph{i.e.}, $\exists k \in \mathbb{Z} \, p = r + k q$. 
Si ce prédicat est vrai, on dit que \emph{$p$ est égal à $r$ modulo $q$}.
...
\noindent\bold{Lemme (symétrie) :} Soit $p$, $q$ et $r$ trois entiers tels que $p \equiv r \, [q]$.
    Alors, $r \equiv p \, [q]$.
...
\noindent\bold{Lemme (transitivité) :} Soit $p$, $q$, $r$ et $s$ quatre entiers tels que $p \equiv r \, [q]$ et $r \equiv s \, [q]$.
    Alors, $p \equiv s \, [q]$.
...
\noindent\bold{Lemme :} Soit $p$, $q$ et $r$ trois entier naturels tels que $p \equiv r \, [q]$ et $r < q$.
...
\noindent\bold{Démonstration :}
    ...
    Donc, avec les notations de la \in{section}{}[sub:divEuclidienne], $r \in E_{p,q}$. 
...
\noindent\bold{Remarque :} Réciproquement, et par définition du reste de la division euclidienne, si $p$ et $q$ sont deux entiers naturels, alors $p \equiv (p \mathrel{\%} q) \, [q]$.

\blank[medium] 

\noindent\bold{Corolaire 1 :} Soit $p$, $q$ et $r$ trois entier naturels tels que $p = r \, [q]$ et $r < q$.
    Si $r \neq 0$, alors $p$ n'est pas un multiple de $q$.

\blank[medium] 

\noindent\bold{Corolaire 2 :} Soit $p$, et $q$ deux entier naturels tels que $q \neq 0$ et $q \vert p$.
    Alors, $p \equiv 0 \, [q]$.

\blank[medium] 

\noindent\bold{Lemme :} Soit $q$, $p_1$, $p_2$, $r_1$ et $r_2$ cinq entiers relatifs tels que $p_1 \equiv r_1 \, [q]$ et $p_2 \equiv r_2 \, [q]$.
Alors, 
\startitemize[nowhite]
...
\noindent\bold{Démonstration :} Choisissons deux entiers $k_1$ et $k_2$ tels que $p_1 = r_1 + k_1 q$ et $p_2 = r_2 + k_2 q$. 
On a :
\startitemize[nowhite]
...
\noindent\bold{Définition :} Un entier naturel $n$ est pair si et seulement si $n \equiv 0 \, [2]$ et impair si et seulement si $n \equiv 1 \, [2]$. 
...
\noindent\bold{Notation :} Soit $a$ un entier non nul, $n$ un entier naturel non nul et $c_0$, $c_1$, ..., $c_n$ des entiers.
