...
\noindent\bold{Démonstration bis :} 
    ...
    $P$ est équivalent à : $\forall x \, \forall e \, ((e \in x) \vee \lnot (e \in \emptyset))$, c'est-à-dire, à : $\forall x \, \forall e \, ((e \in x) \vee (e \notin \emptyset))$.
...
\noindent\bold{Démonstration bis :} 
    ...
    Il est équivalent à : $\forall x \, (x \subset \emptyset) \Rightarrow ((x \subset \emptyset) \wedge (\emptyset \subset x))$, donc à $\forall x \, \lnot (x \subset \emptyset) \vee ((x \subset \emptyset) \wedge (\emptyset \subset x))$, donc à $\forall x \, (\lnot (x \subset \emptyset) \vee (x \subset \emptyset)) \wedge (\lnot (x \subset \emptyset) \vee (\emptyset \subset x))$.
    Puisque $\lnot (x \subset \emptyset) \vee (x \subset \emptyset)$ est toujours vrai (soit $f$ la formule $x \subset \emptyset$, il s'agit de $\lnot f \vee f$, qui est vrai que $f$ soit vraie ou fausse), $P$ est équivalent à $\forall x \, (\lnot (x \subset \emptyset) \vee (\emptyset \subset x))$.
    On a vu que $\forall x \, \emptyset \subset x$ est vrai.
    Donc, $\emptyset \subset x$ est équivalente à $\mathss{V}$.
    Donc, $P$ est équivalente à $\forall x \, (\lnot (x \subset \emptyset) \vee \mathss{V})$, donc à $\forall x \, \mathss{V}$, et donc à $\mathss{V}$.
    Donc, $P$ est vrai.
...
\noindent\bold{Démonstration bis :} 
    ...
    Pour toute formule $f$, $(\lnot f) \vee f$ est vrai que $f$ soit vraie ou fausse, donc équivalent à $\mathss{V}$.
...
\noindent\bold{Démonstration bis :} 
    ...
    Puisque, pour tout prédicat $P$, $P \wedge \lnot P$ est équivalent à $\mathss{F}$, cela est équivalent à $(x \in A) \vee \mathss{F}$, et donc à $x \in A$.
...
Soit $a$ et $b$ deux ensembles. 
On appelle \emph{\words{intersection}} de $a$ et $b$, notée $a \symbols{\cap} b$, l'ensemble 
\startformula
...
\noindent\bold{Lemme :} Soit $E$ un ensemble. 
...
\noindent\bold{Démonstration bis :} 
    \startitemize[nowhite]
        \item Notons $P_1$ et $P_2$ les prédicats à un paramètre libre suivants : $P_1(x): x \in E \cup \emptyset$, $P_2(x): x \in E$. 
            ...
            Puisque $x \in \emptyset$ est équivalent à $\mathss{F}$, $P_1$ est équivalent à $x \in E$.
            Donc, $P_1$ est équivalent à $P_2$.
            Donc, $P_1 \Leftrightarrow P_2$ est équivalent à $\mathss{V}$.
        ...
        \item Notons $P_1$ le prédicat à un paramètre libre : $P_1(x): x \in E \cap \emptyset$. 
            ...
            Puisque $x \in \emptyset$ est équivalent à $\mathss{F}$, $P_1$ est équivalent à $\mathss{F}$, et donc à $x \in \emptyset$.
            Donc, $P_1 \Leftrightarrow (x \in \emptyset)$ est équivalent à $\mathss{V}$.
...
\noindent
Soit $E$ un ensemble non vide. On note $\symbols{\cup} E$ l'ensemble $F$ défini par:
\startformula
    \forall x \, (
        (x \in F) \Leftrightarrow (\forall y \, ((y \in E) \Rightarrow (x \in y)))
    ).
\stopformula
Cet ensemble existe d'après le schéma d'axiomes de compréhension, en considérant pour $a$ un élément de $E$ (qui existe puisque $E$ est non vide) et la formule à un paramètre $P x: \forall y  \, (y \in E) \Rightarrow (x \in y)$. 
Notions que cette définition ne dépends pas du choix de $a$. 
Pour montrer cela, fixons deux éléments $a$ et $b$ de $E$, et notons $F$ et $G$ les ensembles obtenus en partant de $a$ et de $b$. 
On veut montrer que $F = G$. 
Soit $x$ un élément de $F$. 
On a $x \in a$ et $x \in c$ pour tout élément $c$ de $E$. 
Puisque $b \in E$, on en déduit que $x \in b$, et donc $x \in G$.
Ainsi, $F \subset G$. 
Le même raisonnement en échangeant les rôles de $F$ et $G$ et $a$ et $b$ montre que $G \subset F$; on a donc $F = G$.

\startsubsection[title=Schéma d'axiomes de remplacement]

La théorie de Zermelo plus cet axiome donne la théorie ZF. 

\blank[medium]

\noindent\emph{\bold{Énoncé :} Soit $F$ une formule à deux variables libres (notées en première et second position) et d'éventuels paramètres notés $a_1 \dots a_p$. Alors, }
\startformula
    \forall a_1 \dots a_p \, 
        \left( 
...
\noindent\bold{Lemme :} Pour un choix donné des paramètres tel que le membre de gauche de l'implication est satisfait et pour tout $b$, l'ensemble $c$ définit par $\forall z \, \left[ (z \in c) \Leftrightarrow (\exists x \, [(x \in b) \wedge (F x z a_1 \dots a_p)]) \right]$ est unique d'après l'axiome d'extensionnalité.
...
\noindent\bold{Lemme :} Le schéma d'axiomes de compréhension est une conséquence du schéma d'axiomes de remplacement, obtenue en prenant $F x y : (x = y) \wedge P(x)$. 
...
\noindent\bold{Lemme:} En présence du schéma d'axiomes de remplacement, l'axiome de la paire est une conséquence des autres. 

\blank[medium]

\noindent\bold{Démonstration:} Tout d'abord, d'après le schéma d'axiomes de compréhension, l'ensemble vide $\emptyset$ existe. 
...
\startsubsection[title=Axiome de fondation]
...
\noindent\emph{\bold{Énoncé :} Tout ensemble $x$ non vide possède un élément $y$ n'ayant aucun élément commun avec $x$: \\
$\forall \, x, \; \left[ x \neq \emptyset \Rightarrow (\exists y \, y \in x \, \wedge \, y \cap x = \emptyset) \right]$.} 

\blank[medium]

\noindent\bold{Corolaire 1 :} Aucun ensemble ne peut être un élément de lui-même. 

\blank[medium]

\noindent\bold{Démonstration :} Soit $y$ un ensemble quelconque, et considérons l'ensemble $x = \lbrace y \rbrace$. 
...
\noindent\bold{Corolaire 2 :} Soit deux ensembles $x$ et $y$. Si $x \in y$, alors $y \notin x$. 

\blank[medium]

\noindent\bold{Démonstration :} Soit $x$ et $y$ deux ensembles tels que $x \in y$. 
...
\startsubsection[title=Couples]

\noindent\bold{Définition :} Soit deux ensembles $x$ et $y$. D'après l'axiome de la paire, $\lbrace x \rbrace$ et $\lbrace x, y \rbrace$ existent. En utilisant à nouveau l'axiome de la paire, l'ensemble $\lbrace \lbrace x \rbrace, \lbrace x, y \rbrace \rbrace$ existe. On l'appelle le \emph{\words{couple}} de $x$ et $y$, noté $(x,y)$.

\blank[medium]

\noindent\bold{Lemme :} Soit $a$, $b$, $c$ et $d$ quatre ensembles tels que $(a,b) = (c,d)$.
    Alors $a = c$ et $b = d$.

\blank[medium]

\noindent\bold{Démonstration :} On distingue deux cas selon que $a$ et $b$ sont égaux ou non. 
...
On dit parfois que $x$ est la \emph{première composante} de $z$ et $y$ sa \emph{deuxième composante}, ou \emph{seconde composante}.

\stopsubsection

\startsubsection[title=Produit Cartésien]
...
\stopformula
On note $a \times b$ et on appelle \emph{\words{produit Cartésien} de $a$ et $b$} l'ensemble des éléments de $e$ satisfaisant la propriété $P$.
...
On appelle \emph{domaine} de $c$ l'ensemble $\lbrace x \in a \vert \exists y \, y \in b \wedge (x,y) \in c \rbrace$. 

\blank[medium]

\noindent\bold{Lemme :} Soit $a$, $b$, $a'$ et $b'$ quatre ensembles tels que $a' \subset a$ et $b' \subset b$. 
...
\startsubsection[title=Graphe de relation binaire]

Soit $a$ et $b$ deux ensembles. 
Un \emph{\words{graphe} de relation binaire} sur $a$ et $b$ est un sous-ensemble de $a \times b$. 
...
On dira alors que la relation $R$ est \emph{définie sur $a$ et $b$}.
...
Soit $E$ un ensemble. 
Une relation binaire $\leq$ définie sur $E \times E$ est dite \emph{\words{relation d'ordre}} sur $E$ si elle satisfait les trois propriétés suivantes : 
\startitemize[nowhite]
    \item \emph{Réflexivité :} $\forall x \, x \in E \Rightarrow x \leq x$.
    \item \emph{Antisymétrie :} $\forall x \, \forall y \, x \in E \wedge y \in E \wedge (x \leq y) \wedge (y \leq x) \Rightarrow x = y$.
    \item \emph{Transitivité :} $\forall x \, \forall y \, \forall z \, x \in E \wedge y \in E \wedge z \in E \wedge (x \leq y) \mathrel{\wedge} (y \leq z) \Rightarrow x \leq z$.
\stopitemize 
Une relation d'ordre $\leq$ sur $E$ est dite \emph{relation d'ordre total} si la formule suivante est vraie : $\forall x \in E \, \forall y \in E \, (x \leq y) \vee (y \leq x)$.
Un élément $e$ de $E$ tel que : $\forall f \, f \in E \wedge f \leq e \Rightarrow f = e$ est dit \emph{\words{minimal}} (pour l'ensemble $E$ et pour la relation $\leq$) ; on dit aussi que $E$ admet $e$ pour élément minimal pour la relation $\leq$. 
Un élément $e$ de $E$ tel que $\forall x \in E \, e \leq x$ est dit \emph{plus petit élément}, ou \emph{\words{minimum}}, de $E$ (pour la relation $\leq$).
Un élément $e$ de $E$ tel que : $\forall f \, f \in E \wedge e \leq f \Rightarrow f = e$ est dit \emph{\words{maximal}} (pour l'ensemble $E$ et pour la relation $\leq$) ; on dit aussi que $E$ admet $e$ pour élément maximal pour la relation $\leq$. 
Un élément $e$ de $E$ tel que $\forall x \in E \, x \leq e$ est dit \emph{plus grand élément}, ou \emph{\words{maximum}}, de $E$ (pour la relation $\leq$).
Un ensemble muni d'une relation d'ordre est dit \emph{\words{ordonné}}.
Un ensemble muni d'une relation d'ordre total est dit \emph{totalement ordonné}.

\blank[medium]

\noindent\bold{Remarque :} Soit $E$ un ensemble et $F$ un sous-ensemble de $E$.
...
\noindent\bold{Remarque :} Un ensemble a au plus un minimum et au plus un maximum. 

\blank[medium]

\noindent\bold{Démonstration :} Soit $E$ un ensemble et $\leq$ une relation d'ordre sur $E$. 
\startitemize[nowhite]
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $\leq$ une relation d'ordre sur $E$. 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $\leq$ une relation d'ordre total sur $E$. 
...
\startitemize[nowhite]
    ...
    \item \emph{(Nous adoptions ici une approche un brin pédestre.)} 
        ...
        \emph{(Ici, on pourrait directement conclure que, puisque $f \leq e$ implique $f = e$ et donc $e \leq f$, la première formule est équivalente à $e \leq f$. Dans la suite, nous montrons cela plus formellement \emph{via} le calcul des prédicats.)}
        Cette dernière formule peut se récrire en : $f = e \vee \lnot (f \leq e)$.
        La conjonction de ces deux prédicats donne : $(e \leq f \vee f \leq e) \wedge (f = e \vee \lnot (f \leq e))$.
        En développant cette formule, il vient : $(e \leq f \wedge f = e) \vee (e \leq f \wedge \lnot (f \leq e)) \vee (f \leq e \wedge f = e) \vee (f \leq e \wedge \lnot (f \leq e))$.
        Cette formule peut être simplifiée en : $(f = e) \vee (e \leq f \wedge \lnot (f \leq e)) \vee (f = e) \vee \mathss{F}$, 
        ou en $(f = e) \vee (e \leq f \wedge \lnot (f \neq e))$.
...
\noindent\bold{Démonstration :}
\startitemize[nowhite]
    \item \emph{Réflexivité :} Soit $x$ un élément de $E$. On a $x \leq x$ par réflexivité de la relation $\leq$, donc $x \geq x$. 
    \item \emph{Antisymétrie :} Soit $x$ et $y$ deux éléments de $E$ tels que $x \geq y$ et $y \geq x$.
        Alors, $y \leq x$ et $x \leq y$.
        Par antisymétrie de la relation $\leq$, on en déduit que $x = y$.
    \item \emph{Transitivité :} Soit $x$, $y$ et $z$ trois éléments de $E$ tels que $x \geq y$ et $y \geq z$.
...
On dit que $F$ est \emph{borné supérieurement} (dans $E$ et pour la relation $\leq$) s'il existe un élément $m$ de $E$ tel que : $\forall e \, (e \in F) \Rightarrow (e \leq m)$. 
On dit alors que cet élément est une \emph{borne supérieure} de $F$ (dans $E$ et pour la relation $\leq$).
On dit que $F$ est \emph{borné inférieurement} (dans $E$ et pour la relation $\leq$) s'il existe un élément $m$ de $E$ tel que : $\forall e \, (e \in F) \Rightarrow (m \leq e)$.
On dit alors que cet élément est une \emph{borne inférieure} de $F$ (dans $E$ et pour la relation $\leq$).
 
Une relation binaire $<$ antisymétrique, transitive et telle que $\forall x \, x \in E \Rightarrow \lnot (x < x)$ (antireflexivité) est dite \emph{relation d'ordre strict}. 
...
\noindent\bold{Démonstration :} 
\startitemize[nowhite]
    \item \emph{Antiréflexivité :} Soit $x$ un élément de $E$. 
        Puisque $x = x$, la formule $x \neq x$ est fausse, donc $x < x$ est fausse.
    \item \emph{Antisymétrie :} Soit $x$ et $y$ deux éléments de $E$ tels que $x < y$ et $y < x$. 
        Alors, $x \leq y$ et $y \leq x$. 
        Puisque $\leq$ est une relation d'ordre, cela implique $x = y$.
    \item \emph{Transitivité :} Soit $x$, $y$ et $z$ trois éléments de $E$ tels que $x < y$ et $y < z$. 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $<$ une relation d'ordre strict sur $E$. 
...
\noindent\bold{Démonstration :} 
\startitemize[nowhite]
    \item \emph{Réflexivité :} Soit $x$ un élément de $E$. 
        Puisque $x = x$ est vrai par réflexivité de l'égalité, $x \leq x$ est vrai.
    \item \emph{Antisymétrie :} Soit $x$ et $y$ deux éléments de $E$ tels que $x \leq y$ et $y \leq x$. 
        Alors, $x < y$ ou $x = y$. 
        De même, $y < x$ ou $x = y$. 
        Puisque $x < y$ et $y < x$ ne peuvent être simultanément vrais, on en déduit que $x = y$.
    \item \emph{Transitivité :} Soit $x$, $y$ et $z$ trois éléments de $E$ tels que $x \leq y$ et $y \leq z$. 
        On a $x < y$ ou $x = y$.
...
\noindent\bold{Lemme :} Soit $E$ un ensemble, $\leq$ une relation d'ordre sur $E$, et $<$ la relation d'ordre strict sur $E$ définie par : $\forall x \forall y \, (x \in E \wedge y \in E) \Rightarrow (x < y \Leftrightarrow (x \leq y \wedge x \neq y))$.
...
\noindent\bold{Démonstration :} Notons d'abord que, dans les deux cas, on a $x \leq y$ et $y \leq z$, donc $x \leq z$ par transitivité de la relation $\leq$. 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $\leq$ une relation d'ordre total définie sur $E$. 
    Alors la relation $>$ définie sur $E$ par : pour tous éléments $x$ et $y$ de $E$, $a > b \Leftrightarrow \lnot (a \leq b)$ est une relation d'ordre strict. 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $<$ une relation d'ordre strict définie sur $E$, telle que : $\forall x \in E \, \forall y \in E (x < y) \vee (y < x) \vee (x = y)$. 
    Alors la relation $\geq$ définie sur $E$ par : pour tous éléments $x$ et $y$ de $E$, $a \geq b \Leftrightarrow \lnot (a < b)$ est une relation d'ordre total. 
...
\startitemize[nowhite]
    ...
    \item \emph{Antisymétrie :} Soit $x$ et $y$ deux éléments de $E$ tels que $x \geq y$ et $y \geq x$. 
        Alors, $\lnot (x < y)$ et $\lnot (y < x)$. 
        Donc, $x = y$.
    \item \emph{Transitivité :} Soit $x$, $y$ et $z$ trois éléments de $E$ tels que $x \geq y$ et $y \geq z$. 
        On a $\lnot (x < y)$ et $\lnot (y < z)$. 
...
\noindent\bold{Vocabulaire :} Soit $E$ un ensemble et $\leq$ une relation d'ordre sur $E$. 
    ...
    \startitemize[nowhite]
        \item si $a \leq b$, on dira que \emph{$a$ est inférieur ou égal à $b$}, 
        \item si $a \geq b$, on dira que \emph{$a$ est supérieur ou égal à $b$}, 
        \item si $a < b$, on dira que \emph{$a$ est strictement inférieur à $b$}, 
        \item si $a > b$, on dira que \emph{$a$ est strictement supérieur à $b$}.
    \stopitemize

\blank[medium]

\noindent\bold{Notation :} Soit $E$ un ensemble, $\geq$ une relation d'ordre sur $E$, et $<$ la relation d'ordre strict sur $E$ définie par : pour tous éléments $a$ et $b$ de $E$, $a < b \Leftrightarrow ( a \leq b \wedge a \neq b )$. 
...
\noindent\bold{Définition :} Soit $E$ un ensemble et $\leq$ une relation d'ordre sur $E$. 
    La relation $\leq$ est dit un \emph{bon ordre} sur $E$ si tout sous-ensemble non vide de $E$ admet un plus petit élément.
    L'ensemble $E$ est alors dit \emph{bien ordonné}.

\blank[medium]

\noindent\bold{Lemme :} Soit $E$ un ensemble et $\leq$ un bon ordre sur $E$. 
...
\startsubsection[title=Induction transfinie]

\noindent\bold{Lemme (\words{induction transfinie}) :}
...
\noindent\bold{Démonstration :} 
    ...
    \startformula
        S = \left\lbrace x \in E \middle \vert \lnot P(x) \right\rbrace.
    \stopformula
    ...
    Donc, $P(n)$ est vrai.
    On obtient donc une contradiction ($\lnot P(n) \wedge P(n)$), montrant que l'hypothèse de départ est fausse.

    \done

\stopsubsection

\startsubsection[title=Partition] 

Soit $E$ et $P$ deux ensembles. 
On dit que \emph{$P$ est une \words{partition} de $E$} si les quatre propriétés suivantes sont satisfaites : 
\startitemize[nowhite]
...
\startsubsection[title=Relation d'équivalence]

Soit $E$ un ensemble. 
Une relation binaire $\sim$ définie sur $E \times E$ est dite \emph{\words{relation d'équivalence}} sur $E$ si elle satisfait les trois propriétés suivantes : 
\startitemize[nowhite]
    \item \emph{Réflexivité :} $\forall x \, x \in E \Rightarrow x \sim x$
    \item \emph{Symétrie :} $\forall x \, \forall y \, (x \in E) \wedge (y \in E) \wedge (x \sim y) \Rightarrow (y \sim x)$.
    \item \emph{Transitivité :} $\forall x \, \forall y \, \forall z \, (x \in E) \wedge (y \in E) \wedge (z \in E) \wedge (x \sim y) \wedge (y \sim z) \Rightarrow (x \sim z)$. 
\stopitemize

Soit $E$ un ensemble et $\sim$ une relation d'équivalence sur $E$. 
Pour tout $x \in E$, on définit la \emph{\words{classe d'équivalence}} de $x$ pour $\sim$, notée ici $[x]$, par : $[x] = \lbrace y \in E \vert y \sim x \rbrace$. 
Les éléments d'une classe d'équivalence sont parfois appelés ses \emph{représentants}, ou \emph{représentations}. \index{Représentant}
...
\noindent \bold{Lemme :} Soit $x$ et $y$ deux éléments de $E$. Si $x \sim y$, alors $[x] = [y]$. 
...
\noindent \bold{Lemme :} L'ensemble des classes d'équivalence de $E$ pour la relation $\sim$ forme une partition de $E$.
...
\noindent\bold{Définition :} Soit $E$ un ensemble et $\mathcal{R}$ une relation d'équivalence sur $E$.
    L'ensemble des classes d'équivalence de $\mathcal{R}$ est noté $E \divslash \mathcal{R}$ et appelé \emph{\words{ensemble quotient}} de $E$ par $\mathcal{R}$.

\stopsubsection

\startsubsection[title=Fonctions, reference=sub:fonctions]
...
\noindent\bold{Définition :} Soit deux ensembles $X$ et $Y$. Une \emph{\words{fonction}}, ou \emph{\words{application}}, $f$ de $X$ vers $Y$ (ou de $X$ dans $Y$, ou de $X$ sur $Y$) est un ensemble (parfois appelé \emph{\words{graphe}}) tel que : 
\startformula
...
et 
\startformula
    \forall y \, \forall y\p \, ([\exists x \, ((x,y) \in f \wedge (x,y\p) \in f)] \Rightarrow (y = y\p)).
\stopformula
La première condition est équivalente à dire que $f$ est un sous-ensemble de $X \times Y$, \emph{i.e.}, à : $f \subset X \times Y$. 
...
On dit alors que $y$ est l'\emph{\words{image}} de $x$ ou que $x$ est un \emph{\words{antécédent}} de $y$ par $f$. 
On dit aussi que $f$ \emph{associe} $y$ à $x$. 

On dit que $f$ est \emph{définie sur} $X$, ou que $X$ est le \emph{\words{domaine de définition}} de $f$. 
L'ensemble des éléments de $y$ tels qu'il existe un élément $x$ de $X$ satisfaisant $f(x) = y$ est appelé \emph{image} de $f$, notée $\mathrm{Im}(f)$ (c'est donc l'ensemble $\lbrace y \in Y \vert \exists x \, (x \in X) \wedge f(x) = y \rbrace$). 
La notation $f: X \to Y$, signifie que $f$ est une fonction de $X$ vers $Y$. 
Avec les mêmes notations, si $X'$ est un sous-ensemble de $X$ et s'il n'y a pas d'ambiguité\footnote{Une telle ambiguité pourrait survenir dans des cas particuliers si $X' \in X$.}, on appellera \emph{image de $X'$ par $f$} l'ensemble des éléments $y$ de $Y$ tels qu'il existe un élément $x$ de $X'$ tel que $f(x) = y$.

Pour tout sous-ensemble $Y'$ de $Y$, on note $f^{-1}(Y')$ l'ensemble $\lbrace x \in X \vert f(x) \in Y' \rbrace$, appelé \emph{\words{image inverse}} de $G$ par $f$. 
S'il n'y a pas d'ambigüité, et si $y \in Y$, on notera parfois $f^{-1}(y)$ l'ensemble $f^{-1}(\lbrace y \rbrace)$.
(Les ensembles ainsi obtenus pour différentes valeurs de $y$ sont deux à deux disjoints. 
En effet, soit $y$ et $z$ deux éléments de $Y$ et $x$ un élément de $X$. 
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles non vides et $P$ un prédicat à deux paramètres libres tel que, pour tout élément $e$ de $E$, il existe un unique élément $f$ de $F$ tel que $P e f$ est vrai, \emph{i.e.}, 
\startformula
    \forall e \in E, \, 
        (\exists f \in F, \, P e f)
        \wedge 
...
\noindent\bold{Démonstration :} Montrons que l'ensemble $G$ satisfait les trois conditions pour être une fonction de $E$ vers $F$.
\startitemize[nowhite]
    ...
    \item Soit $e$ un élément de $E$ et $y$ et $y\p$ deux éléments de $F$ tels que $(e,f) \in G$ et $(e,f\p) \in G$.
        Alors, $P e f$ et $P e f\p$ sont vrais.
        Donc, $f = f\p$. 
\stopitemize
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles et $f$ et $g$ deux fonctions de $E$ vers $F$. 
...
\noindent\bold{Démonstration :} 
    ...
    Puisque $x \in E$, on peut choisir un élément $y\p$ de $F$ tel que $(x,y\p) \in g$. 
    On a alors $y = f(x)$ et $y\p = g(x)$. 
    Puisque $f(x) = g(x)$, on en déduit $y\p = y$. 
    Donc, $(x,y) \in g$, et donc $z \in g$. 
    Cela montre que $f \subset g$.
    
    Soit $z$ un élément de $g$. 
    On peut choisir un élément $x$ de $E$ et un élément $y$ de $F$ tel que $z = (x,y)$. 
    Puisque $x \in E$, on peut choisir un élément $y\p$ de $F$ tel que $(x,y\p) \in f$. 
    On a alors $y = g(x)$ et $y\p = f(x)$. 
    Puisque $g(x) = f(x)$, on en déduit $y\p = y$. 
...
On dit que
\startitemize[nowhite]
    \item $f$ est \emph{injective} (ou \emph{une injection}) si $\forall x \; \forall y \, [f(x) = f(y) \Rightarrow x = y]$.
        Puisque chaque élément de $f$ est dans $E \times F$, cela est équivalent à : $\forall x \in E \; \forall y \in F \, [f(x) = f(y) \Rightarrow x = y]$.
    \item $f$ est \emph{surjective} (ou \emph{une surjection}) si $\forall y \in F \; \exists x \, [f(x) = y]$.
        Puisque chaque élément de $f$ est dans $E \times F$, cela est équivalent à : $\forall y \in F \; \exists x \in E \, [f(x) = y]$.
    \item $f$ est \emph{bijective} (ou \emph{une bijection}) si elle est à la fois injective et surjective, ce qui est équivalent à : $\forall y \in F \; \exists ! x \, [f(x) = y]$ et à : $\forall y \in F \; \exists ! x \in E \, [f(x) = y]$.
\stopitemize
...
\noindent\bold{Lemme :} Soit $E$ un ensemble. 
    ...
    Alors, $I$ est une bijection de $E$ vers $E$, appelée \emph{fonction identité} sur $E$. 
...
\startitemize[nowhite]
    \item Montrons d'abord que $I$ est une fonction de $E$ vers $E$. 
        \startitemize[nowhite]
            ...
            \item Soit $y$ et $y\p$ deux éléments de $E$ et $x$ un élément de $E$ tel que $(x,y) \in I$ et $(x,y\p) \in I$.
                Alors, il existe deux éléments $x\p$ et $x\pp$ de $E$ tels que $(x,y) = (x\p,x\p)$ et $(x,y\p) = (x\pp,x\pp)$. 
                La première égalité donne $x = x\p$ et $y = x\p$, donc $x = y$.
                La seconde égalité donne $x = x\pp$ et $y\p = x\pp$, donc $x = y\p$. 
                Donc, $y = y\p$.
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles, $f$ une fonction de $E$ vers $F$, $I_E$ la fonction identité sur $E$ et $I_F$ la fonction identité sur $F$. 
...
\startitemize[nowhite]
    ...
    \item Soit $x$ et $x\p$ deux éléments de $E$ et $y$ un élément de $F$ tels que $(y,x) \in g$ et $(y,x\p) \in g$.
        Alors, $P x y$ et $P x\p y$ sont vraies.
        Si $y \in \mathrm{Im}(f)$, cela implique $f(x) = y$ et $f(x\p) = y$, donc $f(x) = f(x\p)$, et donc (puisque $f$ est injective) $x = x\p$.
        Sinon, cela implique $x = a$ et $x\p = a$, donc $x = x\p$.
        Dans tous les cas, on a $x = x\p$. 
...
\noindent\bold{Démonstration :} 
    ...
    \startitemize[nowhite]
        ...
        \item Soit $y$ un élément de $F$. 
            ...
            On a $(y,x) \in h$.
        \item Soit $y$ un élément de $F$ et $x$ et $x\p$ deux éléments de $E$ tels que $(y,x) \in h$ et $(y,x\p) \in h$.
            Alors, $x = g (f^{-1}(\lbrace y \rbrace))$ et $x\p = g (f^{-1}(\lbrace y \rbrace))$.
            Donc, $x = x\p$.
    \stopitemize
    L'ensemble $h$ est donc bien une fonction de $F$ vers $E$.

    Montrons que $h$ est injective. 
    Soit $y$ et $y\p$ deux éléments de $F$ tels que $h(y) = h(y\p)$. 
    ...
    De même, puisque $h(y\p) = g (f^{-1}(\lbrace y\p \rbrace))$ et $g (f^{-1}(\lbrace y\p \rbrace)) \in f^{-1}(\lbrace y\p \rbrace)$, on a $h(y\p) \in f^{-1}(\lbrace y\p \rbrace)$, et donc $f(h(y\p)) = y\p$.
    Puisque $h(y) = h(y\p)$, on en déduit que $y = y\p$. 
...
\noindent\bold{Composition de deux fonctions :} Soit $E$, $F$ et $G$ trois ensembles. 
    Soit $f$ une fonction de $E$ vers $F$ et $g$ une fonction de $F$ vers $G$. 
    La \emph{composée} de $g$ et $f$, notée $g \circ f$, est la fonction de $E$ vers $G$ définie par : $\forall x \in E \, (g \circ f) (x) = g(f(x))$. 
    Plus formellement, $g \circ f = \lbrace z \in E \times G \vert \exists x \in E \, z = (x,g(f(x))) \rbrace$.

\blank[medium]

\noindent\bold{Lemme :} L'ensemble ainsi défini est bien une fonction de $E$ vers $G$.
...
\startitemize[nowhite]
    ...
    \item Soit $y$ et $y\p$ deux ensembles. 
        Soit $x$ un ensemble tel que $(x,y) \in g \circ f$ et $(x,y\p) \in g \circ f$. 
        Alors, on peut choisir un élément $x\p$ de $E$ tel que $(x,y) \in (x\p,g(f(x\p)))$ et un élément $x\pp$ de $E$ tel que $(x,y\p) = (x\pp,g(f(x\pp)))$. 
        On a donc $x = x\p$, $y = g(f(x\p))$, $x = x\pp$ et $y\p = g(f(x\pp))$. 
        Donc, $y = g(f(x))$ et $y\p = g(f(x))$. 
        Donc, $y = y\p$.
\stopitemize
...
\noindent\bold{Remarque :} Avec les mêmes notations, si $f$ et $g$ sont deux injections, alors $g \circ f$ en est aussi une. 
    En effet, soit $x$ et $y$ deux éléments de $G$ tels que $(g \circ f)(x) = (g \circ f)(y)$, on a $g(f(x)) = g(f(y))$, donc $f(x) = f(y)$, et donc $x = y$.

\blank[medium]

\noindent\bold{Remarque :} Avec les mêmes notations, si $f$ et $g$ sont deux surjections, alors $g \circ f$ en est aussi une. 
...
\noindent\bold{Remarque :} Avec les mêmes notations, si $f$ et $g$ sont deux bijections, alors $g \circ f$ en est aussi une. 
    En effet, il s'agit d'une injection et d'une surjection d'après les deux points précédents.

\blank[medium]

\noindent\bold{Lemme (associativité de la composition de fonctions) :} Soit $E$, $F$, $G$ et $H$ quatre ensembles. 
    Soit $f$, $g$ et $h$ des fonctions respectivement de $E$ vers $F$, de $F$ vers $G$ et de $G$ vers $H$. 
    Alors $h \circ (g \circ f) = (h \circ g) \circ f$.

\blank[medium]

\noindent\bold{Démonstration :} Montrons d'abord que $h \circ (g \circ f)$ et $(h \circ g) \circ f$ sont deux fonctions de $E$ vers $H$. 
...
\noindent\bold{Démonstration :}
    ...
    \startitemize[nowhite]
        ...
        \item Soit $y$ un élément de $F$ et $x$ et $x\p$ deux éléments de $E$ tels que $(y,x) \in g$ et $(y,x\p) \in g$.
            Alors, $(x,y) \in f$ et $(x\p,y) \in f$, donc $f(x) = y$ et $f(x\p) = y$, donc $f(x) = f(x\p)$.
            Puisque $f$ est injective, on en déduit que $x = x\p$.
    \stopitemize
    ...
    \startitemize[nowhite]
        \item Soit $y$ et $y\p$ deux éléments de $F$ tels que $g(y) = g(y\p)$. 
            Puisque $(y,g(y)) \in g$, on a $(g(y),y) \in f$, donc $f(g(y)) = y$.
            De même, puisque $(y\p,g(y\p)) \in g$, on a $(g(y\p),y\p) \in f$, donc $f(g(y\p)) = y\p$.
            Puisque $g(y\p) = g(y)$, cela implique $f(g(y)) = y\p$, et donc $y = y\p$.
...
\noindent\bold{Définition :} Soit $E$ et $F$ deux ensembles, $E'$ un sous-ensemble de $E$, et $f$ une fonction de $E$ vers $F$. 
    On appelle \emph{restriction de $f$ à $E'$} la fonction $g$ de $E'$ vers $F$ définie par : pour tout élément $e$ de $E'$, $g(e) = f(e)$.

\blank[medium]

\noindent\bold{Notation:} Soit $E$, $F$ et $G$ trois ensembles. 
...
\startsubsection[title=Axiome du choix]

\noindent\emph{\bold{Énoncé :} \index{Choix (axiome)} Pour tout ensemble $X$ d'ensembles non vides, il existe une fonction sur $X$ qui à chaque ensemble $x$ appartenant à $X$ associe un élément de $x$ :}
\startformula
...
\startsubsection[title={Théorie de Tarski-Grothendieck}]
\index{Tarski-Grothendieck}

\noindent\bold{Définition :} Un ensemble $U$ est dit \emph{\words{univers de Grothendieck}} si les quatre proptiétés suivantes sont vraies : 
\startitemize[nowhite]
    \item L'ensemble $U$ est \emph{transitif} : pour tout élément $x$ de $U$ et tout élément $y$ de $x$, $y \in U$.
    \item Pour tous éléments $x$ et $y$ de $U$, $\lbrace x, y \rbrace \in U$.
    \item Pour tout élément $x$ de $U$, l'ensemble des sous-ensembles de $x$, $\mathcal{P}(x)$, est un élément de $U$.
    \item Pour tout sous-ensemble $S$ de $U$, l'union des éléments de $S$, $\cup S$, est un élément de $U$.
\stopitemize
Formellement, cela peut s'écrire :
\startitemize[nowhite]
    \item $\forall x \, \forall y \, (x \in U \wedge y \in x) \Rightarrow y \in U$.
    \item $\forall x \, \forall y \, (x \in U \wedge y \in U) \Rightarrow \lbrace x, y \rbrace \in U$.
    \item $\forall x \, x \in U \Rightarrow \mathcal{P}(x) \in U$.
    \item $\forall S\, S \subset U \Rightarrow \cup S \in U$.
\stopitemize

\blank[medium]

\noindent\emph{\bold{Axiome de Tarski :}} Pour tout ensemble $E$, il existe un univers de Grothendieck $U$ tel que $E \in U$.

\blank[medium]

La \emph{théorie de Tarski-Grothendieck} ceomprend les axiomes de la théorie ZF plus l'axiome de Tarski. 
Sauf mention contraire explicite, on ne supposera pas l'axiome de Tarski ici.

\stopsubsection

\startsubsection[title=Lemme de Zorn (en théorie ZFC)]

Dans cette section seulement, on définit la notion de \emph{chaine} de la manière suivante.
Soit $X$ un ensemble et $\leq$ une relation d'ordre sur $X$.
Un sous-ensemble $C$ de $X$ est une \emph{chaine} de $X$ pour $\leq$ si $\leq$ est une relation d'ordre total sur $C$, autrement dit, si
\startformula
...
\noindent\bold{Énoncé :} Soit $X$ un ensemble et $\leq$ une relation d'ordre sur $X$.
...
On peut choisir une borne supérieure $u$ de $C$ dans $X$, et un élément $x$ de $X$, dit \emph{borne supérieure stricte de $C$} tel que $x > u$. 
...
Un sous-ensemble $D$ de $C$ est dit \emph{segment initial} de $C$ s'il existe un élément $x$ de $C$ tel que $D = P(C, x)$.

On dit d'un sous-ensemble $A$ de $X$ qu'il est \emph{conforme} s'il satisfait les deux conditions suivantes : 
\startitemize[nowhite]
...
\noindent\bold{Lemme :} Soit $A$ et $B$ deux sous-ensembles conformes de $X$ tels que $A \neq B$.
...
\noindent\bold{Lemme :} $U$ est un sous-ensemble conforme de $X$.
...
\noindent\bold{Lemme :} $U \cup \lbrace x \rbrace$ est un sous-ensemble conforme de $X$.
