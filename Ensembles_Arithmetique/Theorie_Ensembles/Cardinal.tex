\startsection[title=Cardinal]

\startsubsection[title=Cardinal fini]

Un ensemble $E$ est dit \emph{de \words{cardinal} fini}, ou simplement \emph{fini}\index{Ensemble fini}, s'il existe une bijection de $E$ vers un entier naturel.
S'il n'existe aucun netier naturel $n$ tel qu'il existe une bijection de $E$ vers $n$, on dit que $E$ est \emph{de cardinal infini}, ou simplement \emph{infini}\index{Ensemble infini}. 

Soit $E$ un ensemble et $n$ un entier naturel. 
On dit que $E$ est de \emph{\words{cardinal}} $n$ s'il existe une bijection de $E$ vers $n$.%
\footnote{
...
On pourra noter l'ensemble $E$ par la liste de ses éléments, \emph{i.e.}, des $f(x)$ pour $x$ décrivant $[\![0, n-1]\!]$, séparés par des virgules, entre les crochets $\lbrace$ et $\rbrace$ : $E = \lbrace f(0), f(1), \dots, f(n-1) \rbrace$. 
...
\noindent\bold{Lemme :} Avec les mêmes notations, si $f$ est injective, alors $G$ est de cardinal $n$.
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles finis de même cardinal. 
    On suppose que $E \subset F$. 
    Alors $E = F$. 

\blank[medium]

\noindent\bold{Remarque :} La réciproque est évidente : Soit $E$ et $F$ deux ensembles finis, si $E = F$, alors $E \subset F$. 
    On déduit donc le résultat suivant : si $E$ et $F$ sont finis et ont le même cardinal, alors $E \subset F \Leftrightarrow E = F$.

\blank[medium]

\noindent\bold{Démonstration du lemme :} 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble de cardinal fini $n$. 
    Soit $x$ tel que $x \notin E$.
...
\noindent\bold{Lemme :} Soit $n$ un entier naturel non nul. 
...
\noindent\bold{Démonstration :} Puisque $n \neq 0$, $E \neq \emptyset$. 
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles. 
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles finis tels que $E \cap F = \emptyset$.
    Alors $E \cup F$ est fini et $\abs{E \cup F} = \abs{E} + \abs{F}$.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur le cardinal de $F$.
...
\noindent\bold{Démonstration :} Notons d'abord que $F$ et $E \setminus F$ sont deux sous-ensembles de $E$, donc finis, et $\abs{F} \leq \abs{E}$.
...
\noindent\bold{Démonstration :} On procède par récurrence forte sur le cardinal de $E$. 
    Si $\abs{E} = 0$, alors $E = \emptyset$. 
...
\noindent\bold{Démonstration :} Si $E = \emptyset$, alors $E$ est fini et $\abs{E} = 0$, donc $\abs{E} \leq \abs{F}$.
...
\noindent\bold{Corolaire (contraposée) :} Soit $E$ et $F$ deux ensembles. 
    On suppose qu'il existe une injection de $E$ vers $F$ et que $E$ est infini.
    Alors $F$ est infini.

\blank[medium]

\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles de même cardinal fini. 
    Soit $f$ une injection de $E$ vers $F$.
    Alors $f$ est une bijection. 

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur le cardinal de $E$, noté $n$.
...
\noindent\bold{Démonstration :} 
    ...
    Notons $E_y$ l'ensemble de ses antécédents, \emph{i.e.}, $E_y = \lbrace x \in E \vert f(x) = y \rbrace$. 
...
\noindent\bold{Lemme :} Soit $E$ et $F$ deux ensembles finis. 
    Alors les ensembles $E \cap F$ et $E \cup F$ sont finis, et $\abs{E \cup F} = \abs{E} + \abs{F} - \abs{E \cap F}$.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur le cardinal de $E$. 
...
\startitemize[nowhite]
    \item \emph{$f$ est surjective :}
        ...
        \stopitemize
    \item \emph{$f$ est injective :} Soit $u$ et $v$ deux éléments de $E \times F$ tels que $h(u) = h(v)$. 
...
\noindent\bold{Corolaire :} Soit $n$ un entier naturel supérieur ou égal à $2$ et $E_1$, $E_2$, ..., $E_n$ des ensembles finis.
...
\noindent\bold{Lemme :} Soit $E$ un ensemble non vide et $F$ un ensemble infini.
    Alors $E \times F$ et $F \times E$ sont infinis.

\blank[medium]

\noindent\bold{Démonstration :} Montrons qu'il existe une injection de $F$ vers $E \times F$ et une injection de $F$ vers $F \times E$.
...
\noindent\bold{Lemme :} Soit $E$ un ensemble fini non vide et $\leq$ une relation d'ordre total sur $E$.
    Alors, $E$ admet un minimum et un maximum pour la relation d'ordre $\leq$.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur le cardinal de $E$ moins $1$.
...
\noindent\bold{Lemme :} Tout ensemble fini, non vide et totalement ordonné admet un maximum et un minimum.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur le cardinal de $E$ moins $1$, noté $c$.
...
\startsubsection[title=Cas de l'ensemble des entiers naturels]

\noindent\bold{Lemme :} L'ensemble $\mathbb{N}$ est infini. 

\blank[medium]

\noindent\bold{Démonstration :} Montrons par récurrence qu'il n'existe aucune bijection entre un entier naturel $n$ et $\mathbb{N}$. 
...
\noindent\bold{Lemme :} Soit $E$ un ensemble de cardinal infini. Alors il existe une injection de $\mathbb{N}$ vers $E$. 
...
\noindent\bold{Démonstration :} Il s'agit de montrer qu'il existe une suite $u$ d'éléments de $E$ deux-à-deux distincts. 
...
\noindent\bold{Définition :} Soit $\mathcal{E}$ l'ensemble des fonctions d'une partie de $\mathbb{Y}$ vers $\mathbb{Y}$. 
...
\noindent\bold{Lemme :} Soit $n$ un entier naturel et $\Sigma_n$ et $\Pi_n$ définis comme ci-dessus. 
...
\noindent\bold{Lemme :} Soit $n$ et $E$ un sous-ensemble de $\mathbb{Y}$ de cardinal $n+1$. 
...
\startsubsection[title=Ensemble défini par une liste d'éléments] 

Soit $p$ un entier naturel non nul et $a_1$, $a_2$, ..., $a_p$ des ensembles. 
On note $\lbrace a_1, a_2, \dots, a_p \rbrace$ l'ensemble contenant exactement $a_1$, $a_2$, ..., $a_p$, \emph{i.e.}, 
\startformula
...
\startsubsection[title=Intervalle de $\mathbb{N}$ ou $\mathbb{Z}$]

\noindent\bold{Lemme :} Soit $a$ et $b$ deux éléments de $\mathbb{N}$. 
...
\noindent\bold{Lemme :} Soit $a$ et $b$ deux éléments de $\mathbb{Z}$. 
    Alors, $[\![a, b]\!]$ est fini et
    \startitemize[nowhite]
        \item si $a > b$, $\abs{[\![a, b]\!]} = 0$, 
...
\startsubsection[title=Ensemble dénombrable, reference=sub:dénombrable]

\noindent\bold{Définition :} Un ensemble $E$ est dit \emph{dénombrable} s'il existe une bijection de $E$ vers $\mathbb{N}$. 
    De manière équivaleente, un ensemle $E$ est dénombrable si et seulement si il existe une bijection de $\mathbb{N}$ vers $E$.

\blank[medium]

\noindent\bold{Remarque :} Puisque $\mathbb{N}$ est infini, un ensemble dénombrable est nécéssairement infini.

\blank[medium]

Nous montrerons ci-dessous que, $\mathbb{N}^2$ est \goto{dénombrable}[N_to_N2], et que, en fait, $\mathbb{N}^n$ est dénombrable pour tout élément $n$ de $\mathbb{N}^*$. 
Par contre, l'ensemble $\mathcal{F}\left( \mathbb{N}, \lbrace 0, 1 \rbrace \right)$ \goto{n'est pas dénombrable}[F_N_0_1_non_denombrable].

\stopsubsection

\startsubsection[title=Théorème de Cantor-Bernstein]

\noindent\bold{Théorème :} Soit $E$ et $F$ deux ensembles. On suppose qu'il existe une injection de $E$ vers $F$ et une injection de $F$ vers $E$. 
...
\noindent\bold{Démonstration :} 
    ...
    \startformula
        \startcases
            \NC C_0 = A \setminus B \NR
            \NC \forall n \in \mathbb{N}^* \; C_n = u \left( C_{n-1} \right) \NR
        \stopcases .
    \stopformula
    ...
    Montrons maintenant que $v$ est surjective. 
...
\noindent\bold{Corolaire :} Soit $E$ un ensemble. On suppose qu'il existe une injection de $\mathbb{N}$ vers $E$. 
    Alors, $E$ a un cardinal infini.

\blank[medium]

\noindent\bold{Démonstration :} Supposons par l'absurde que ce n'est pas le cas, et montrons qu'on aboutit à une contradiction. 
...
\noindent\bold{Exemple d'application :} Bijection entre $\mathbb{N}$ et $\mathbb{N}^2$. \reference[N_to_N2]{} 
\startitemize[nowhite]
    ...
    \item Soit $g: \mathbb{N}^2 \to \mathbb{N}$ la fonction définie par : $\forall x \in \mathbb{N} \, \forall y \in \mathbb{N} \, g((x,y)) = 2^x 3^y$. 
        D'après l'unicité de la décomposition d'un entier naturel en produit de facteurs premiers (voir \in{section}{}[sub:dec_fact_prem]), la fonction $g$ est injective. 
...
\noindent\bold{Exemple d'application 2 :} Bijection entre $\mathbb{N}$ et $\mathbb{N}^n$ pour $n \in \mathbb{N}^*$. 
\startitemize[nowhite]
    ...
    \item Puisqu'il existe une infinité de nombres premiers distincts (voir \in{section}{}[sub:defNombresPremiers]), on peut  en choisir $n$, par exemple les $n$ plus petits, notés $p_1$, $p_2$, ..., $p_n$.
        ...
        D'après l'unicité de la décomposition d'un entier naturel en produits de facteurs premiers (voir \in{section}{}[sub:dec_fact_prem]), la fonction $g$ est injective.%
...
\noindent\bold{Contre-exemple :}\reference[{F_N_0_1_non_denombrable}]{} En guise d'exemple d'ensembles qui ne sont pas en bijection, considérons les ensembles $\mathbb{N}$ et $\mathcal{F}(\mathbb{N}, \lbrace 0, 1 \rbrace)$. 
...
\noindent\bold{Généralisation :}  Soit $n$ un entier naturel supérieur ou égal à $2$. 
    Soit $E$ un ensemble.
    ...
    \startformula
        g = \lbrace
            ...
            \exists x \in \mathbb{N} \, 
                z = \startcases
                    \NC h(1) \NC \text{si } f(x)(x) = h(0) \NR
                    \NC h(0) \NC \text{sinon} \NR
                \stopcases
        \rbrace .
    \stopformula
...
\noindent\bold{Corolaire :}  On montre facilement que $\lbrace (0, 0), (1, 1) \rbrace$ est une injection de $2$ vers $\mathbb{N}$.
    Il n'existe donc aucune surjection de $\mathbb{N}$ vers $\mathcal{F} (\mathbb{N}, \mathbb{N})$.

\blank[medium]

\noindent\bold{Définition :} Soit $n$ un entier naturel et $u$ une fonction de $n$ vers $\mathbb{Y}$. 
    Alors, 
    \startitemize[nowhite]
...
\startsubsection[title=Relations entre cardinaux]

\noindent\bold{Notation :} Soit $E$ et $F$ deux ensembles. On note
\startitemize[nowhite]
...
\noindent\bold{Lemme :} Soit $E$ un ensemble et $F$ l'ensemble des sous-ensembles de $E$ (\emph{i.e.} $F = \mathcal{P}(E)$).
    Alors, $\abs{E} < \abs{F}$.
...
\noindent\bold{Lemme :} Soit $E$ un ensemble non vide.
...
\noindent\bold{Corrolaire :} Soit $E$ un ensemble (vide ou non).
    Alors (puisqu'il existe une injection de $\emptyset$ vers lui-même—qui n'est autre que la fonction vide $\emptyset$), $\abs{\emptyset} \leq \abs{E}$.

\blank[medium]

\noindent\bold{Lemme :} Soit $E$ un ensemble tel que $\abs{E} \leq \abs{\emptyset}$.
    Alors, $E = \emptyset$.

\blank[medium]

\noindent\bold{Démonstration :} Puisque $E \leq \abs{\emptyset}$, on peut choisir une injection $f$ de $E$ vers $\emptyset$. 
    Soit $x$ un ensemble, $x$ ne peut appartenir à $E$ (car alors l'ensemble $f(x)$ erait un élément de $\emptyset$, ce qui est impossible).
    Donc, $E = \emptyset$.
...
\noindent\bold{Corrolaire :} Il n'existe aucun ensemble $E$ tel que $\abs{E} < \emptyset$.

\blank[medium]

\noindent\bold{Lemme :} On admet l'axiome du choix.
    Soit $E$ et $F$ deux ensembles.
...
\noindent\bold{Démonstration :} 
    Soit $E$ un ensemble tel qu'il existe une inection de $E$ vers $\emptyset$.
    Alors, $E = \emptyset$.
    Donc, il existe une surjection de $E$ vers $\emptyset$ (la fonction vide $\emptyset$).
