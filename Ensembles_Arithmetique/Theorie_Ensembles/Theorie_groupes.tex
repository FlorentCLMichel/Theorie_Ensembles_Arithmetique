...
\noindent\bold{Définition (\words{Action} d'un groupe) :} Soit $(G, \cdot)$ un groupe, $e$ son élément neutre, et $E$ un ensemble.
    Une opération $\star$ de $G \times E$ vers $E$ est dite \emph{action de $(G, \cdot)$ sur $F$} si les deux propriété suivantes sont satisfaites :
    \startitemize[nowhite]
        \item $\forall x \in E \, e \star x = x$,
        \item $\forall (g, g') \in G^2 \, \forall x \in E \, g' \star (g \star x) = (g' \cdot g) \star x$.
    \stopitemize

\blank[medium]

\noindent
Deux examples importants d'actions d'un groupe $(G, \cdot)$ sur lui-même sont la \emph{\words{translation} à gauche} $\mathss{trans}: (g, x) \mapsto g \cdot x$ et la \emph{\words{conjugaison}} $\mathss{cong}: (g, x) \mapsto g \cdot (x \cdot g^{-1})$, où $g^{-1}$ désigne l'inverse de $g$. 
Montrons qu'il s'agit bien d'actions : 
\startitemize
    \item Soit $e$ l'élément neutre de $(G, \cdot)$. 
        Pout tout élément $x$ de $G$, on a : $\mathss{trans}(e, x) = e \cdot x = x$ et $\mathss{conj}(e, x) = e \cdot (x \cdot e^{-1}) = e \cdot (x \cdot e) = e \cdot x = x$.
    \item Soit $g$, $g'$ et $x$ trois éléments de $G$. 
        Alors, $\mathss{trans}(g', \mathss{trans}(g,x)) = g' \cdot (g \cdot x) = (g' \cdot g) \cdot x = \mathss{trans}(g' \cdot g, x)$ et $\mathss{conj}(g', \mathss{conj}(g,x)) = g' \cdot ((g \cdot (x \cdot g^{-1})) \cdot g^{\prime -1}) = (g' \cdot g) \cdot (x \cdot (g^{-1} \cdot g^{\prime -1})) = (g' \cdot g) \cdot (x \cdot (g' \cdot g)^{-1}) = \mathss{conj}(g' \cdot g, x)$.
\stopitemize
Plus généralement, soit $(G, \cdot)$ et $(F, \ast)$ deux groupes et $m$ un morphisme de groupe de $(G, \cdot)$ vers $(F, \ast)$, alors les fonctions $t$ et $c$ de $G$ vers $F$ définies par $t: (g, f) \mapsto m(g) \ast f$ et $c: (g, f) \mapsto f(g) \ast (x \ast f(g^{-1}))$ sont des actions. 
En effet, 
\startitemize
    \item Soit $e$ l'élément neutre de $(G, \cdot)$ et $e'$ celui de $(G, \ast)$. 
        Pout tout élément $f$ de $F$, on a : $t(e, f) = m(e) \ast f = e' \ast f = f$ et $c(e, x) = m(e) \ast (f \ast m(e^{-1})) = e' \ast (f \ast e') = e' \ast f = f$.
    \item Soit $g$ et $g'$ deux éléments de $G$ et $f$ un élément de $f$. 
        Alors, $t(g', t(g, f)) = m(g') \ast (m(g) \ast f) = (m(g') \ast m(g)) \ast f = m(g' \cdot g) \ast f = t(g' \cdot g, x)$ et $c(g', c(g, f)) = m(g') \ast ((m(g) \ast (f \ast m(g^{-1}))) \ast m(g^{\prime -1})) = m(g' \cdot g) \ast (x \ast m(g^{-1} \cdot g^{\prime -1})) = m(g' \cdot g) \ast (x \ast m((g' \cdot g)^{-1})) = c(g' \cdot g, x)$.
\stopitemize
...
\noindent\bold{Définition :} Soit $(G, \cdot)$ un groupe abélien et $(H, \cdot)$ un sous-groupe de $(G, \cdot)$.
    On définit l'ensemble $G \symbols{\divslash} H$ comme l'ensemble des classes d'équivalences de $G$ pour la relation $R$ définie par : $\forall g \in G \, \forall g' \in G \, g R g' \Leftrightarrow g^{-1} \cdot g' \in H$, oú un exposant $-1$ indique l'inverse. 
...
\startitemize[nowhite]
    ...
    \item \emph{Symétrie :} Soit $g$ et $g'$ deux éléments de $G$ tels que $g R g'$.
        ...
        Puisque $(H, \cdot)$ est un groupe, $(g^{-1} \cdot g')^{-1} \in H$, donc $g^{\prime -1} \cdot g \in H$, donc $g' R g$.
    \item \emph{Transitivité :} Soit $g$, $g'$ et $g''$ trois éléments de $G$ tels que $g R g'$ et $g' R g''$.
        Alors, on peut choisir deux éléments $h$ et $h'$ de $H$ tels que $g^{-1} \cdot g' = h$ et $g^{\prime -1} \cdot g'' = h'$.
...
\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe abélien et $(H, \cdot)$ un sous-groupe de $(G, \cdot)$.
    On définit les deux relations $R_G$ et $R_D$ sur $G$ de la manière suivante : pour tous éléments $g$ et $g'$ de $G$, $g R_G g' \Leftrightarrow g^{-1} \cdot g' \in H$ et $g R_D g' \Leftrightarrow g \cdot g^{\prime -1} \in H$. 
...
\startitemize[nowhite]
    ...
    \item \emph{Symétrie ($R_G$) :} Soit $g$ et $g'$ deux éléments de $G$ tels que $g R_G g'$.
        Alors, $g^{-1} \cdot g' \in H$.
        Puisque $(H, \cdot)$ est un groupe, $(g^{-1} \cdot g')^{-1} \in H$, donc $g^{\prime -1} \cdot g \in H$, donc $g' R_G g$.
    \item \emph{Symétrie ($R_D$) :} Soit $g$ et $g'$ deux éléments de $G$ tels que $g R_D g'$.
        Alors, $g \cdot g^{\prime -1} \in H$.
        Puisque $(H, \cdot)$ est un groupe, $(g \cdot g^{\prime -1})^{-1} \in H$, donc $g' \cdot g^{-1} \in H$, donc $g' R_D g$.
    ...
    \item \emph{Transitivité ($R_D$) :} Soit $g$, $g'$ et $g''$ trois éléments de $G$ tels que $g R_D g'$ et $g' R_D g''$.
        Alors, on peut choisir deux éléments $h$ et $h'$ de $H$ tels que $g \cdot g^{\prime -1} = h$ et $g' \cdot g'^{\prime -1} = h'$.
        Puisque $(H, \cdot)$ est un groupe, $h \cdot h' \in H$, donc $g \cdot g'^{\prime -1} \in H$, donc $g R_D g''$.
\stopitemize
...
\noindent\bold{Lemme :} Avec les notations du lemme précédent, si $(H, \cdot)$ est normal, alors les relations $R_L$ et $R_D$ définies ci-dessus sont identiques, au sens suivant : $\forall g \in G \, \forall g' \in G \, g R_L g' \Leftrightarrow g R_D g'$.
...
\noindent\bold{Preuve que cette loi de composition interne est bien définie :} 
    ...
    Alors, on peu choisir deux éléments $h$ et $h'$ de $H$ tels que $e^{-1} \cdot e' = h$ (puisque $e R_G e'$) et $f \cdot f^{\prime -1} = h'$ (puisque $f R_D f'$).
    ...
    De même, $e' \cdot h' \cdot e^{\prime -1}$, donc $e' \cdot f \cdot f^{\prime -1} \cdot e^{\prime -1} \in H$, donc $(e' \cdot f) \cdot (e' \cdot f')^{-1} \in H$, donc $(e' \cdot f) \mathrel{R} (e' \cdot f')$.
