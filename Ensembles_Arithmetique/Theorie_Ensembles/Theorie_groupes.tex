\startsection[title=Éléments de théorie des groupes]

Dans cette section, nous donnons quelques concepts de base de théorie des groupes. 

Soit $E$, $F$ et $G$ trois ensembles et $\star$ un symbole, on dira qur $\star$ est une \emph{\words{opération}} ou \emph{\words{loi de compsition}} de $E \times F$ vers $G$ s'il existe une fonction $\lambda$ de $E \times F$ vers $G$ telle que, pour tous éléments $e$ de $E$ et $f$ de $F$, $e \star f$ désigne $\lambda(e, f)$. 
(On peut voir une opération comme une notation abrégée pour désigner une fonction.)

Avec les mêmes notations, on parlera simplemement d'opération de $E$ vers $F$ pour désigner une opération de $E \times E$ vers $F$.

\startsubsection[title=Définitions]

\noindent\bold{Définition (\words{Magma}) :} Un \emph{\words{magma}} $\mathcal{M}$ est un couple formé par un ensemble $M$ et une \emph{loi de composition interne} $\cdot$ sur $M$, c'est-à-dire une fonction de $M \times M$ vers $M$. 
    Si $a$ et $b$ sont deux éléments de $M$, on note $a \cdot b$ l'image de $(a,b)$ par $\cdot$.

\blank[medium]

\noindent\bold{Définition (\words{Élément neutre}) :} Soit $(M,\cdot)$ un magma.
    Un élément $e$ de $M$ est dit \emph{élément neutre} si 
    \startformula
        \forall m \in M, \, e \cdot m = m \wedge m \cdot e = m.
    \stopformula
    Un magma admettant un élément neutre est dit \emph{\words{unifère}} ou \emph{\words{unitaire}}.

\blank[medium]

\noindent\bold{Lemme :} Un magma admet au plus un élément neutre.

\blank[medium]

\noindent\bold{Démonstration :} Soit $(M,\cdot)$ un magma et $e$ et $f$ deux éléments identités pour ce magma.
    Puisque $e$ est un élément identité, $e \cdot f = f$.
    Puisque $f$ est un élément identité, $e \cdot f = e$.
    Par commutativité et transitivité de l'égalité, on en déduit $e = f$.

    \hfill \square

\blank[medium]

\noindent\bold{Lemme :} Soit $(M, +)$ un magma unifère et $e$ son élément neutre.
    Soit $n$ un entier naturel non nul.
    Alors, l'élément $(\cdots ((e + e) + e) + \cdots +e)$, où $e$ apparaît $n$ fois, est égal à $e$.

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur $n$. 
    Pour $n = 1$, le résultat est évident puisque $e = e$.
    Soit $n$ un entier naturel non nul tel que le résultat est vrai. 
    Soit $x$ l'élément $(\cdots ((e+e) + e) + \cdots + e)$, où $e$ apparaît $n+1$ fois. 
    Alors, on peut noter $x = (y + e)$, où $y$ est l'élément défini dans le lemme avec $e$ apparaissant $n$ fois. 
    Donc, $y = e$.
    Donc, $x = e$. 
    On en déduit que le résultat attendu est vrai en remplaçant $n$ par $n+1$. 
    Par récurrence, il est donc vrai pour tout entier naturel non nul $n$.

    \done

\blank[medium]

\noindent\bold{Définition (\words{Morphisme} de magmas) :} Soit $\left(M, \cdot \right)$ et $\left(N, \ast \right)$ deux magmas.
    Une fonction $f$ de $M$ vers $N$ est dite \emph{morphisme de magmas} de $\left(M, \cdot \right)$ vers $\left(N, \ast \right)$ si elle satisfait : 
    \startformula
        \forall (a,b) \in M^2, \, f (a \cdot b) = f(a) \ast f(b) .
    \stopformula

\blank[medium]

\noindent\bold{Définition (\words{Noyau}) :} Soit $(M_1, \dot)$ un magma et $\mathcal{M}_2$ un magma unifère.  
    Soit $0$ l'élément neutres de $\mathcal{M}_2$ et $f$ un morphisme de magmas de $(M_1, \dot)$ vers $\mathcal{M}_2$.
    On appelle \emph{noyau} de $f$, noté $\mathrm{Ker}(f)$, l'ensemble des antécéédants de $0$ par $f$: $\mathrm{Ker}(f) = \lbrace x \in M_1 \vert f(x) = 0 \rbrace$.

\blank[medium]

\noindent\bold{Définition (\words{Isomorphisme} de magmas) :} Soit $\mathcal{M}$ et $\mathcal{N}$ deux magmas.
    Un morphise de magmas $f$ de $\mathcal{M}$ vers $\mathcal{N}$ est dit \emph{isomorphisme de magmas} s'il est également une bijection.

\blank[medium]

\noindent\bold{Lemme :} L'image d'un élément neutre par un morphisme de magmas surjectif (et donc, en particulier, par un isomorphisme) est un élément neutre.

\blank[medium]

\noindent\bold{Démonstration :} Soit $\left(M, \cdot \right)$ et $\left(N, \ast \right)$ deux magmas et soit $f$ un morphisme surjectif du premier vers le second. 
    Soit $e$ un élément neutre de $\left(M, \cdot \right)$. 
    On a $f(e) \in N$. 
    Soit $y$ un élément de $N$. 
    Il s'agit de montrer que $f(e) \ast y = y$ et $y \ast f(e) = y$.

    Puisque $f$ est surjectif, on peut choisir un élément $x$ de $M$ tel que $f(x) = y$. 
    Donc, $f(e) \ast y = f(e) \ast f(x)$. 
    Puisque $f$ est un morphisme de magmas, $f(e) \ast f(x) = f(e \cdot x)$. 
    Puisque $e$ est un élément neutre pour $\cdot$, $e \cdot x = x$. 
    Donc, $f(e) \ast f(x) = f(x)$.
    Donc, $f(e) \ast y = y$. 

    De même, puisque $f$ est un morphisme de magmas, $f(x) \ast f(e) = f(x \cdot e)$. 
    Puisque $e$ est un élément neutre pour $\cdot$, $x \cdot e = x$. 
    Donc, $f(x) \ast f(e) = f(x)$.
    Donc, $y \ast f(x) = y$. 

    \hfill \square

\blank[medium]

\noindent\bold{Corolaire :} Soit $\mathcal{M}$ et $\mathcal{N}$ deux magmas. 
    On suppose qu'il existe un morphisme de magmas surjectif de $\mathcal{M}$ vers $\mathcal{N}$. 
    Si $\mathcal{M}$ est unifère, alors $\mathcal{N}$ l'est aussi. 
    (Car l'image par le morphisme de l'élément neutre de $\mathcal{M}$ est un élément neutre pour $\mathcal{N}$.)

\blank[medium]

\noindent\bold{Lemme :} L'inverse d'un isomorphisme de magmas est un isomorphisme de magmas.

\blank[medium]

\noindent\bold{Démonstration :} Soit $\left(M, \cdot \right)$ et $\left(N, \ast \right)$ deux magmas et soit $f$ un isomorphisme du premier vers le second. 
    Soit $g$ l'inverse de $f$ (qui existe et est une bijection puisque $f$ est une bijection). 
    Montrons que $g$ est un morphisme de magmas. 
    Puisque $g$ est également une bijection, il s'agira alors d'un isomorphisme. 

    Soit $a$ et $b$ deux éléments de $N$. 
    Puisque $f$ est une bijection, elle est surjective, donc on peut choisir deux éléments $c$ et $d$ de $M$ tels que $a = f(c)$ et $b = f(d)$. 
    On a alors : $g(a) \cdot g(b) = g(f(c)) \cdot g(f(d))$.
    Puisque $g$ est l'inverse de $f$, cela donne : $g(a) \cdot g(b) = c \cdot d$. 

    Par ailleurs, $g(a \ast b) = g(f(c) \ast f(d))$. 
    Puisque $f$ est un morphisme de magmas on a $f(c) \ast f(d) = f (c \cdot d)$, donc cela donne $g(a \ast b) = g(f(c \cdot d))$.
    Puisque $g$ est l'inverse de $f$, cela donne : $g(a \ast b) = c \cdot d$. 
    Donc, $g(a \ast b) = g(a) \cdot g(b)$. 

    Cela étant vrai pour tous éléments $a$ et $b$ de $M$, on en déduit que $g$ est un morphisme de magmas, et donc un isomorphisme, de $\left(M, \cdot \right)$ vers $\left(N, \ast \right)$.

    \hfill \square

\blank[medium]

\noindent\bold{Définition (\words{Associativité}) :} Soit $(M,\cdot)$ un magma.
    La loi de composition interne $\cdot$ est dite \emph{associative} si
    \startformula
        \forall a \in M, \, \forall b \in M, \, \forall c \in M , \,  
        (a \cdot b) \cdot c = a \cdot (b \cdot c).
    \stopformula
    Le magma $(M, \cdot)$ est alors dit \emph{associatif}. 
    Un magma associatif est aussi appelé \emph{demi-groupe}.

\blank[medium]

\noindent\bold{Lemme :} Soit $\mathcal{M}_1$ et $\mathcal{M}_2$ deux magmas.
    On suppose que $\mathcal{M}_1$ est associatif et qu'il existe un morphisme de magmas surjectif de $\mathcal{M}_1$ vers $\mathcal{M}_2$.
    Alors $\mathcal{M}_2$ est associatif.

\blank[medium]

\noindent\bold{Démonstration :} Soit $(M,\cdot)$ et $(N,\ast)$ deux magmas. 
    On suppose que le premier est associatif et qu'il existe un morphisme de magmas surjectif, $f$, du premier vers le second. 
    Soit $a$, $b$ et $c$ trois éléments de $N$. 
    Puisque $f$ est surjectif, on peut choisir trois éléments $d$, $e$ et $g$ de $M$ tels que $f(d) = a$, $f(e) = b$ et $f(g) = c$.
    On a alors : $(a \ast b) \ast c = (f(d) \ast f(e)) \ast f(g)$.
    Puisque $f$ est un morphisme de magmas, $(f(d) \ast f(e)) \ast f(g) = f(d \cdot e) \ast f(g) = f((d \cdot e) \cdot g)$.
    Puisque $\cdot$ est associative, $f((d \cdot e) \cdot g) = f(d \cdot (e \cdot g))$. 
    En utilisant à nouveau le fait que $f$ est un morphisme de magmas, on obtient : $f(d \cdot (e \cdot g)) = f(d) \ast f(e \cdot g)$ et $f(d) \ast f(e \cdot g)) = f(d) \ast (f(e) \ast f(g))$. 
    Enfin, puisque $d$, $e$ et $g$ sont des antécédents respectifs de $a$, $b$ et $c$ par $f$, on a $f(d) \ast (f(e) \ast f(g)) = a \ast (b \ast c)$. 
    En combinant ces formules et en utilisant la transitivité de l'égalité, il vient : $(a \ast b) \ast c = a \ast (b \ast c)$.

    \hfill \square

\blank[medium]

\noindent\bold{Définition (\words{Commutativité}) :} Soit $(M,\cdot)$ un magma.
    La loi de composition interne $\cdot$ est dite \emph{commutative} si
    \startformula
        \forall a \in M, \, \forall b \in M,  
        a \cdot b = b \cdot a.
    \stopformula
    Le magma $(M, \cdot)$ est alors dit \emph{commutatif} ou \emph{abélien}.

\blank[medium]

\noindent\bold{Lemme :} Soit $\mathcal{M}_1$ et $\mathcal{M}_2$ deux magmas.
    On suppose que $\mathcal{M}_1$ est commutatif et qu'il existe un morphisme de magmas surjectif de $\mathcal{M}_1$ vers $\mathcal{M}_2$.
    Alors $\mathcal{M}_2$ est commutatif.

\blank[medium]

\noindent\bold{Démonstration :} Soit $(M,\cdot)$ et $(N,\ast)$ deux magmas. 
    On suppose que le premier est commutatif et qu'il existe un morphise de magmas surjectif, $f$, du premier vers le second. 
    Soit $a$ et $b$ deux éléments de $N$. 
    Puisque $f$ est surjectif, on peut choisir deux éléments $d$ et $e$ de $M$ tels que $f(d) = a$ et $f(e) = b$.
    On a alors : $a \ast b = f(d) \ast f(e)$.
    Puisque $f$ est un morphisme de magmas, $f(d) \ast f(e) = f(d \cdot e)$.
    Puisque $\cdot$ est commutative, $d \cdot e = e \cdot d$, donc $f(d \cdot e) = f(e \cdot d)$. 
    En utilisant à nouveau le fait que $f$ est un morphisme de magmas, on obtient : $f(e \cdot d) = f(e) \ast f(d)$. 
    Enfin, puisque $d$ et $e$ sont des antécédents respectifs de $a$ et $b$ par $f$, on a $f(e) \ast f(d) = b \ast a$. 
    En combinant ces formules et en utilisant la transitivité de l'égalité, il vient : $a \ast b = b \ast a$.

    \hfill \square

\blank[medium]

\noindent\bold{Définition (\words{Monoïde}) :} Un magma unifère et associatif est appelé \emph{monoïde}.

\blank[medium]

\noindent\bold{Définition (Monoïde \words{abélien}) :} Un magma unifère, associatif et abélien est dit \emph{monoïde abélien}.

\blank[medium]

\noindent\bold{Définition (\words{Puissance}) :} Soit $(M,\cdot)$ un monoïde abélien et $1$ son élément neutre.
    On définit la suite $\mathrm{P}$ de fonctions de $M$ vers $M$ par : 
    \startitemize[nowhite]
        \item pour tout élément $m$ de $M$, $P_0(m) = 1$,
        \item pour tout entier naturel $n$, pour tout élément $m$ de $M$, $P_{n+1}(m) = P_n(m) \cdot n$.
    \stopitemize
    Pour tout élément $m$ de $M$ et tout entier naturel $n$, on notera $m^n$ l'élément $P_n(m)$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(M,\cdot)$ un monoïde abélien, $m$ un élément de $M$, et $a$ et $b$ deux entiers naturels. 
    Alors, $m^{a + b} = m^a \cdot m^b$.

\blank[medium]

\noindent\bold{Démonstration :} On procéde par récurrence sur $b$.
    Fixons $m$ et $a$.
    On veut montrer que le prédicat à un paramère libre $P$ définit par : $P(b): m^{a+b} = m^a \cdot m^b$ est vrai pour tout entier naturel $b$.
    \startitemize[nowhite]
        \item $P(0)$ est équivalent à $m^a = m^a$, qui est vrai.
        \item Soit $b$ un entier naturel tel que $P(b)$ est vrai. 
            Alors, $m^{a+(b+1)} = m^{(a+b)+1} = m^{a+b} \cdot m = (m^a \cdot m^b) \cdot m = m^a \cdot (m^b \cdot m) = m^a \cdot m^{b+1}$.
            Donc, $P(b+1)$ est vrai.
    \stopitemize
    Par récurrence, $P(b)$ est vrai pour tout entier naturel $b$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(M,\cdot)$ un monoïde abélien, $m$ un élément de $M$, et $a$ et $b$ deux entiers naturels. 
    Alors, $m^{a \times b} = (m^a)^b$.

\blank[medium]

\noindent\bold{Démonstration :} On procéde par récurrence sur $b$.
    Fixons $m$ et $a$.
    On veut montrer que le prédicat à un paramère libre $P$ définit par : $P(b): m^{a \times b} = (m^a)^b$ est vrai pour tout entier naturel $b$.
    \startitemize[nowhite]
        \item $P(0)$ est équivalent à $m^{a \times 0} = (m^a)^0$, donc à $m^0 = e$, qui est vrai.
        \item Soit $b$ un entier naturel tel que $P(b)$ est vrai. 
            Alors, $m^{a \times (b+1)} = m^{(a \times b) + a} = m^{a \times b} \times m^a = (m^a)^b \times m^a = (m^a)^{b+1}$.
            Donc, $P(b+1)$ est vrai.
    \stopitemize
    Par récurrence, $P(b)$ est vrai pour tout entier naturel $b$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(M, \cdot)$ un monoïde abélien, $a$ et $b$ deux éléments de $M$, et $n$ un entier naturel. 
    Alors, $(a \cdot b)^n = a^n \cdot b^n$.

\blank[medium]

\noindent\bold{Démonstration :} On procéde par récurrence sur $n$.
    Soit $P$ le prédicat à un paramètre libre définit par : $P(n): (a \cdot b)^n = a^n \cdot b^n$.
    Notons $e$ l'élément neutre de $(M, \cdot)$.

    On a : $(a \cdot b)^0 = e$ et $a^0 \cdot b^0 = e \cdot e = e$.
    Donc, $(a \cdot b)^0 = a^0 \cdot b^0$.
    Donc, $P(0)$ est vrai.

    Soit $n$ un entier naturel tel que $P(n)$ est vrai. 
    Alors, $(a \cdot b)^{n+1} = (a \cdot b)^n \cdot (a \cdot b) = (a^n \cdot b^n) \cdot (a \cdot b) = (a^n \cdot a) \cdot (b^n \cdot b) = a^{n+1} \cdot b^{n+1}$.
    Donc, $P(n+1)$ est vrai.

    Par récurrence, on en déduit que $P(n)$ est vrai pour tout entier naturel $n$.

    \done

\blank[medium]

\noindent\bold{Définition (\words{Morphisme} de monoïdes) :} Un morphisme de magmas d'un monoïde vers un autre est dit \emph{morphisme de monoïdes}.

\blank[medium]

\noindent\bold{Définition (\words{Isomorphisme} de monoïdes) :} Un isomorphisme de magmas d'un monoïde vers un autre est dit \emph{isomorphisme de monoïdes}.

\blank[medium]

\noindent\bold{Lemme :} L'inverse d'un isomorphisme de monoïdes est un isomorphisme de monoïdes.

\blank[medium]

\noindent\bold{Démonstration :} Conséquence directe du même résultat pour un isomorphisle de magmas.

\blank[medium]

\noindent\bold{Définition (\words{Inverse}) :} Soit $(M,\cdot)$ un monoïde et $e$ son élément neutre. 
    Soit $m$ un élément de $M$. 
    Un élément $n$ de $M$ est dit \emph{inverse} de $m$ (pour $\cdot$) si $m \cdot n = e \wedge  n \cdot m = e$.

\blank[medium]

\noindent\bold{Remarque :} Soit $(M,\cdot)$ un monoïde et $e$ son élément neutre.
    Alors, $e$ est son propre inverse.
    
\blank[medium]

\noindent\bold{Lemme :} Soit $(M,\cdot)$ un monoïde et $m$ un élément de $M$.
    Alors, $m$ admet au plus un seul inverse pour $\cdot$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $(M,\cdot)$ un monoïde et $e$ son élément neutre. 
    Soit $m$ un élément de $M$. 
    Soit $n$ et $o$ deux inverse de $m$ pour $\cdot$. 
    Alors, $(n \cdot m) \cdot o = e \cdot o = o$.
    Par ailleurs, $n \cdot (m \cdot o) = n \cdot e = n$.
    Puisque $\cdot$ est associative, $(n \cdot m) \cdot o = n \cdot (m \cdot o)$. 
    Donc, $o = n$.

    \hfill \square

\blank[medium]

\noindent\bold{Lemme :} Soit $(M,\cdot)$ un monoïde et $m$ et $n$ deux éléments de $M$.
    Si $n$ est l'inverse de $m$, alors $m$ est l'inverse de $n$.

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $e$ l'élément neutre de $(M, \cdot)$.
    Si $n$ est l'inverse de $m$, alors $m \cdot n = n \cdot m = e$.
    Donc, $n \cdot m = m \cdot n = e$.
    Donc, $m$ est l'inverse de $n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(M,\cdot)$ un monoïde et $a$ et $b$ deux éléments de $M$.
    On suppose que $a$ a un inverse $c$ et $b$ a un inverse $d$. 
    Alors, $a \cdot b$ a un inverse, et son inverse est $d \cdot c$.

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $e$ l'élément neutre de $(M, \cdot)$.
    On a : $(a \cdot b) \cdot (d \cdot c) = a \cdot (b \cdot (d \cdot c)) = a \cdot ((b \cdot d) \cdot c) = a \cdot (e \cdot c) = a \cdot c = e$.

    \done

\blank[medium]

\noindent\bold{Définition (\words{Groupe}) :} Soit $(M,\cdot)$ un monoïde. 
    Si chaque élément de $M$ admet un inverse pour $\cdot$, alors $(M,\cdot)$ est appelé \emph{groupe}.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe, $1$ son élément neutre et $a$ un élément de $G$
    Si $a \cdot b = 1$, alors $b$ est l'inverse de $a$.

\blank[medium]

\noindent\bold{Démonstration :}
    Soit $c$ l'inverse de $a$.
    On a : $c \cdot (a \cdot b) = c \cdot 1$, donc $(c \cdot a) \cdot b = c$, donc $1 \cdot b = c$, donc $b = c$.

    \done

\blank[medium]

\noindent\bold{Définition (Groupe \words{abélien}) :} Soit $(M,\cdot)$ un monoïde abélien. 
    Si chaque élément de $M$ admet un inverse pour $\cdot$, alors $(M,\cdot)$ est appelé \emph{groupe abélien}.

\blank[medium]

\noindent\bold{Définition (\words{Morphisme} de groupes) :} Un morphisme de magmas d'un groupe vers un autre est dit \emph{morphisme de groupes}.

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +)$ et $(B, \oplus)$ deux groupes. 
    Notosns $0_A$ l'élément neutre du premier et $0_B$ celui du second. 
    Soit $f$ un morphisme de groupes de $(A, +)$ vers $(B, \oplus)$.
    Alors, 
    \startitemize[nowhite]
        \item $f(0_A) = 0_B$,
        \item soit $a$ un élément de $A$ et $b$ son inverse, alors $f(b)$ est l'inverse de $f(a)$, 
        \item $f$ est injective si et seulement si son noyau est $\lbrace 0_A \rbrace$.
    \stopitemize

\blank[medium]

\noindent\bold{Démonstration :}
    \startitemize[nowhite]
        \item On a : $f(0_A) = f(0_A + 0_A) = f(0_A) \oplus f(0_A)$. 
            Soit $c$ l'inverse de $f(0_A)$.
            On a : $c \oplus f(0_A) = c \oplus (f(0_A) \oplus f(0_A)) = (c \oplus f(0_A)) \oplus f(0_A)$.
            Donc, $0_B = 0_B \oplus f(0_A) = f(0_A)$.
        \item On a : $f(a) \oplus f(b) = f(a + b) = f(0_A) = 0_B$.
            Donc, $f(b)$ est l'inverse de $f(a)$.
        \item 
            \startitemize[nowhite]
                \item Si $f$ est injective, $0_B$ ne peut admettre au plus qu'un antécédant pat $f$. 
                    Puisque $0_A$ est un tel antécédant, il ne peut y en avoir d'autre, donc son noyaux est $\lbrace 0_A \rbrace$.
                \item Supposons que le noyau de $f$ est $\lbrace 0_A \rbrace$
                    Alors, $0_A$ est le seul antécédant de $0_B$ par $f$.
                    Soit $a$ et $b$ deux éléments de $A$ tels que $f(a) = f(b)$. 
                    Soit $c$ l'inverse de $a$.
                    On a : $f(c) \oplus f(a) = f(c + a) = 0_B$.
                    Donc, $f(c) \oplus f(b) = 0_B$.
                    Donc, $f(c + b) = 0_B$.
                    Donc, $c + b = 0_A$, donc $a + (c + b) = a + 0_A$, donc $(a + c) + b = a$, donc $0_A + b = a$, donc $b = a$.
                    Cela montre que $f$ est injective.
            \stopitemize
    \stopitemize

    \done

\blank[medium]

\noindent\bold{Définition (\words{Isomorphisme} de groupes) :} Un isomorphisme de magmas d'un groupe vers un autre est dit \emph{isomorphisme de groupes}.

\blank[medium]

\noindent\bold{Lemme :} L'inverse d'un isomorphisme de groupes est un isomorphisme de groupes.

\blank[medium]

\noindent\bold{Démonstration :} Conséquence direct du même résultat pour un isomorphisme de magmas.

\blank[medium]

\noindent\bold{Définition (\words{Puissance négative}) :} Soit $(G,\cdot)$ un groupe abélien. 
    Pour tout entier $n$ strictement négatif et tout élément $g$ de $G$, on note $g^n$ l'élément $h^n$, où $h$ est l'inverse de $g$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G,\cdot)$ un groupe abélien, $e$ l'élément neutre de $G$ et $g$ un élément de $G$.
    Pour tout entier naturel $n$, $g^n \cdot g^{-n} = e$.

\blank[medium]

\noindent\bold{Démonstration :} 
    On procède par récurrence sur $n$. 
    Soit $P$ le prédicat à un paramètre libre définit par : $P(n): g^n \cdot g^{-n} = e$.
    Soit $h$ l'inverse de $g$. 
    \startitemize[nowhite]
        \item $P(0)$ est équivalent à $g^0 \cdot g^0 = e$, donc à $e \cdot e = e$, qui est vrai.
        \item Soit $n$ un entier naturel tel que $P(n)$ est vrai.
            On a : $g^{n+1} \cdot g^{-(n+1)} = g^{n+1} \cdot h^{n+1} = g^n \cdot g \cdot h^n \cdot h = g^n \cdot g \cdot h \cdot h^n = g^n \cdot e \cdot h^n = g^n \cdot h^n = g^n \cdot g^{-n} = e$.
            Donc, $P(n+1)$ est vrai.
    \stopitemize
    Par récurrence, $P(n)$ est vrai pour tout entier naturel $n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(G,\cdot)$ un groupe abélien, $g$ un élément de $G$, et $a$ et $b$ deux entiers.
    Alors, $g^{a+b} = g^a \times g^b$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $h$ l'inverse de $g$. 
    \startitemize[nowhite]
        \item Si $a \geq 0$ et $b \geq 0$, on se ramène au cas déjà démontré.
        \item Si $a \geq 0$, $b < 0$ et $a+b \geq 0$ on a $g^{a+b} \cdot g^{-b} = g^{(a+b) + (-b)}= g^a$, donc $g^{a+b} = g^a \cdot g^b$.
        \item Si $a \geq 0$, $b < 0$ et $a+b < 0$ on a $g^{a+b} \cdot g^{-a} = h^{-(a+b)} h^a = h^{-b} = g^b$, donc $g^{a+b} = g^b \cdot g^a = g^a \cdot g^b$.
        \item Si $a < 0$, $b \geq 0$ et $a+b \geq 0$ on a $g^{a+b} \cdot g^{-a} = g^{(a+b) + (-a)} = g^b$, donc $g^{a+b} = g^b \cdot g^a = g^a \cdot g^b$.
        \item Si $a < 0$, $b \geq 0$ et $a+b < 0$ on a $g^{a+b} \cdot g^{-b} = h^{-(a+b)} h^b = h^{-a} = g^a$, donc $g^{a+b} = g^a \cdot g^b$.
        \item Si $a < 0$ et $b < 0$, on a $-(a+b) < 0$, $-a \geq 0$ et $-b \geq 0$, donc $g^{a+b} = h^{-(a+b)} = h^{(-a) + (-b)} = h^{-a} \cdot h^{-b} = g^a \cdot g^b$.
    \stopitemize
    Dans tous les cas, on a bien $g^{a+b} = g^a \cdot g^b$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(G,\cdot)$ un groupe abélien, $g$ un élément de $G$, et $a$ et $b$ deux entiers naturels. 
    Alors, $g^{a \times b} = (g^a)^b$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $h$ l'inverse de $g$. 
    \startitemize[nowhite]
        \item Si $a \geq 0$ et $b \geq 0$, on se ramène au cas déjà démontré.
        \item Si $a \geq 0$ et $b < 0$, on a $g^{a \times b} = g^{-(a \times (-b))} = h^{a \times (-b)} = (h^a)^{-b}$ et $(g^a)^b = (g^{-a})^{-b} = (h^a)^{-b}$.
        \item Si $a < 0$ et $b \geq 0$, on a $g^{a \times b} = g^{-((-a) \times b)} = h^{(-a) \times b} = (h^{-a})^b$ et $(g^a)^b = (h^{-a})^b$.
        \item Si $a < 0$ et $b < 0$, on a $g^{a \times b} = g^{(-a) \times (-b)} = (g^{-a})^{-b}$ et $(g^a)^b$ est égal à $(g^{-a})^{-b}$ puisque $g^{-a}$ est l'inverse de $g^a$.
    \stopitemize
    Dans tous les cas, on a bien $g^{a \times b} = (g^a)^b$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe abélien, $a$ et $b$ deux éléments de $G$, et $n$ un entier. 
    Alors, $(a \cdot b)^n = a^n \cdot b^n$.

\blank[medium]

\noindent\bold{Démonstration :} 
\startitemize[nowhite]
    \item Si $n \geq 0$, il s'agit d'un résultat déjà démontré.
    \item Supposons $n < 0$. 
        Soit $c$ l'inverse de $a$ et $d$ l'inverse de $b$.
        Alors, $(a \cdot b)^n = (d \cdot c)^{-n} = d^{-n} \cdot c^{-n} = b^n \cdot a^n = a^n \cdot b^n$.
\stopitemize
Dans les deux cas, on a bien $(a \cdot b)^n = a^n \cdot b^n$.

\done

\blank[medium]

\noindent\bold{Définition (\words{Cyclicité}) :} Un groupe abélien $(G, \cdot)$ est dit $\emph{cyclique}$ s'il existe un élément $g$ de $G$ tel que : 
    \startformula
        \forall x \in G, \, \exists n \in \mathbb{N}, \, g^n = x.
    \stopformula
    Un tel élément $g$ est dit \emph{générateur} du groupe.

\blank[medium]

\noindent\bold{Définition (\words{Sous-groupe}) :} Soit $(G, \cdot)$ un groupe et $H$ un sous-ensemble non-vide de $G$.
    Si $(H, \cdot)$ est un groupe, alors il est dit \emph{sous-groupe} de $(G, \cdot)$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe et $H$ un sous-ensemble non-vide de $G$ tel que $(H, \cdot)$ est un groupe.
    Soit $e$ lélément neutre de $(G, \cdot)$. 
    Alors $e \in H$.

\blank[medium]

\noindent\bold{Démonstration :} Puisque $(H, \cdot)$ est un groupe, donc unifere, $H$ est non vide. 
    Soit $h$ un élément de $H$. 
    Puisque $(H, \cdot)$ est un groupe, l'inverse $l$ de $h$ appartient aussi à $H$, et donc $l \cdot h$ également. 
    Puisque $l$ est l'inverse de $H$, $l \cdot h = e$, ce aui conclut la preuve.

    \done

\blank[medium]

\noindent\bold{Définition (\words{Action} d'un groupe) :} Soit $(G, \cdot)$ un groupe, $e$ son élément neutre, et $E$ un ensemble.
    Une opération $\star$ de $G \times E$ vers $E$ est dite \emph{action de $(G, \cdot)$ sur $E$} si les deux propriété suivantes sont satisfaites :
    \startitemize[nowhite]
        \item $\forall x \in E \, e \star x = x$,
        \item $\forall (g, g\p) \in G^2 \, \forall x \in E \, g\p \star (g \star x) = (g\p \cdot g) \star x$.
    \stopitemize

\blank[medium]

\noindent
Deux examples importants d'actions d'un groupe $(G, \cdot)$ sur son premie élément $G$ sont la \emph{\words{translation} à gauche} $\mathss{trans}: (g, x) \mapsto g \cdot x$ et la \emph{\words{conjugaison}} $\mathss{cong}: (g, x) \mapsto g \cdot (x \cdot g^{-1})$, où $g^{-1}$ désigne l'inverse de $g$. 
Montrons qu'il s'agit bien d'actions : 
\startitemize
    \item Soit $e$ l'élément neutre de $(G, \cdot)$. 
        Pour tout élément $x$ de $G$, on a : $\mathss{trans}(e, x) = e \cdot x = x$ et $\mathss{conj}(e, x) = e \cdot (x \cdot e^{-1}) = e \cdot (x \cdot e) = e \cdot x = x$.
    \item Soit $g$, $g\p$ et $x$ trois éléments de $G$. 
        Alors, $\mathss{trans}(g\p, \mathss{trans}(g,x)) = g\p \cdot (g \cdot x) = (g\p \cdot g) \cdot x = \mathss{trans}(g\p \cdot g, x)$ et $\mathss{conj}(g\p, \mathss{conj}(g,x)) = g\p \cdot ((g \cdot (x \cdot g^{-1})) \cdot g\p^{-1}) = (g\p \cdot g) \cdot (x \cdot (g^{-1} \cdot g\p^{-1})) = (g\p \cdot g) \cdot (x \cdot (g\p \cdot g)^{-1}) = \mathss{conj}(g\p \cdot g, x)$.
\stopitemize
Plus généralement, soit $(G, \cdot)$ et $(F, \ast)$ deux groupes et $m$ un morphisme de groupes de $(G, \cdot)$ vers $(F, \ast)$, alors les fonctions $t$ et $c$ de $G$ vers $F$ définies par $t: (g, f) \mapsto m(g) \ast f$ et $c: (g, f) \mapsto f(g) \ast (x \ast f(g^{-1}))$ sont des actions. 
En effet, 
\startitemize
    \item Soit $e$ l'élément neutre de $(G, \cdot)$ et $e'$ celui de $(F, \ast)$. 
        Pout tout élément $f$ de $F$, on a : $t(e, f) = m(e) \ast f = e' \ast f = f$ et $c(e, x) = m(e) \ast (f \ast m(e^{-1})) = e' \ast (f \ast e\p^{-1}) = e' \ast (g \ast e') = e' \ast f = f$.
    \item Soit $g$ et $g\p$ deux éléments de $G$ et $f$ un élément de $F$. 
        Alors, $t(g\p, t(g, f)) = m(g\p) \ast (m(g) \ast f) = (m(g\p) \ast m(g)) \ast f = m(g\p \cdot g) \ast f = t(g\p \cdot g, x)$ et $c(g\p, c(g, f)) = m(g\p) \ast ((m(g) \ast (f \ast m(g^{-1}))) \ast m(g\p^{-1})) = (m(g\p) \ast m(g)) \ast (x \ast (m(g^{-1}) \ast m(g\p^{-1}))) = m(g\p \cdot g) \ast (x \ast m(g^{-1} \cdot g\p^{-1})) = m(g\p \cdot g) \ast (x \ast m((g\p \cdot g)^{-1})) = c(g\p \cdot g, x)$.
\stopitemize

\stopsubsection

\startsubsection[title=Quelques résultats]

\noindent\bold{Lemme :} Parmis les ensembles construits précédemment, 
    \startitemize[nowhite]
        \item $(\mathbb{N},+)$, $(\mathbb{N},\times)$ et $(\mathbb{Z},\times)$ sont des monoïdes abéliens,
        \item $(\mathbb{Z},+)$ est un groupe abélien.
    \stopitemize

\blank[medium]

\noindent\bold{Démonstration :} Nous avons déjà démontré tous les éléments nécessaires. 
    En effet, 
    \startitemize[nowhite]
        \item Les opérations $+$ et $\times$ sont des lois de compositions internes sur $\mathbb{N}$ et $\mathbb{Z}$, donc $(\mathbb{N},+)$, $(\mathbb{N},\times)$, $(\mathbb{Z},+)$ et $(\mathbb{Z},\times)$ sont des magmas.
        \item Les opérations $+$ et $\times$ sont associatives et admettent chacune un élément neutre ($0$ pour la première et $1$ pour la seconde) dans $\mathbb{N}$ et $\mathbb{Z}$, donc $(\mathbb{N},+)$, $(\mathbb{N},\times)$, $(\mathbb{Z},+)$ et $(\mathbb{Z},\times)$ sont des monoïdes.
        \item Les opérations $+$ et $\times$ sont commutatives, donc $(\mathbb{N},+)$, $(\mathbb{N},\times)$, $(\mathbb{Z},+)$ et $(\mathbb{Z},\times)$ sont des monoïdes abéliens.
        \item Pour tout élément $z$ de $\mathbb{Z}$, on a $z + (-z) = (-z) + z = 0$, donc $-z$ est un inverse de $z$ pour $+$. 
            $(\mathbb{Z},+)$ est donc un groupe abélien.
    \stopitemize

\stopsubsection

\startsubsection[title=Groupe fini]

\noindent\bold{Définition :} Soit $(G, \cdot)$ un groupe. Si $G$ est fini, $(G, \cdot)$ est dit \emph{groupe \words{fini}}.
    Le cardinal de $G$ est parfois appelé cardinal du groupe $(G, \cdot)$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe commutatif fini, $1$ son élément neutre et $n$ le cardinal de $G$. 
    Soit $g$ un élément de $G$. 
    Alors, on peut choisir un entier $m$ tel que $m \neq 0$, $m \leq n$ et $g^m = 1$. 
    Soit $E$ l'ensemble des entiers naturels $m$ tels que $0 < m \leq n$ et $g^m = 1$.
    Il s'agit d'un sous-ensemble non vide de $\mathbb{N}$, donc il admet un plus petit élément. 
    Ce dernier (qui est inférieur ou égal à $n$ et strictement supérieur à $0$) est appelé \emph{ordre} de $g$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $f$ la fonction de $[\![0, n]\!]$ (égal à $n+1$) vers $G$ qui à tout élément $m$ de $[\![0, n]\!]$ associe $g^m$. 
    Puisque le cardinal de $G$ est strictement inférieur à $n+1$, $f$ ne peut être injective.
    Donc, on peut choisir deux éléments $a$ et $b$ de $[\![0,n]\!]$ tels que $a \neq b$ et $f(a) = f(b)$, et donc $g^a = g^b$. 
    \startitemize[nowhite]
        \item Si $a > b$, on a $g^{a-b} = e$.
            Puisque $a \leq n$ et $a > b$, on a $a-b > 0$ et $a-b \leq n$. 
        \item Sinon, $a < b$ et on a $g^{b-a} = e$.
            Puisque $b \leq n$ et $b > a$, on a $b-a > 0$ et $b-a \leq n$. 
    \stopitemize

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe commutatif fini et $g$ un élément de $G$. 
    Alors, $g$ est un générateur de $(G, \cdot)$ si et seulement si il est d'ordre $\abs{G}$. 

\blank[medium]

\noindent\bold{Démonstration :} 
Notons $n$ le cardinal de $G$.
Notons que, puisque $(G,\cdot)$ est un groupe, il admet au moins un élément neutre, donc $n > 0$.
Soit $f$ la fonction de $[\![0, n-1]\!]$ (égal à $n$) vers $G$ définie par : pour tout entier naturel $m$ strictement inférieur à $n$, $f(m) = g^m$.
\startitemize[nowhite]
    \item Supposons que $g$ est d'ordre $n$.
        Soit $a$ et $b$ deux éléments de $[\![0, n-1]\!]$ tels que $f(a) = f(b)$.
        Alors, $g^a = g^b$, donc $g^{a-b} = g^{b-a} = e$, donc $g^{\abs{a-b}} = e$.
        Puisque $a < n$ et $b < n$, $a-b < n$ et $b-a < n$, donc $\abs{a-b} < n$.
        Puisque $g$ est d'ordre $n$, on doit donc avoir $\abs{a-b} = 0$.
        Donc, $a = b$.
        Cela montre que $f$ est inective.
        Puisque $G$ et $[\![0, n-1]\!]$ ont le même cardinal $n$, $f$ est donc bijective.
        Donc, pour tout élément $h$ de $G$, on peut choisir un élément $m$ de $[\![0, n-1]\!]$ tel que $g^m = h$.
        Donc, $g$ est un générateur de $(G,\cdot)$.
    \item Supposons que $g$ n'est pas d'ordre $n$. 
        Soit $m$ l'ordre de $g$.
        Alors, $m \neq 0$ et $g^m = e = g^0$, donc $f(m) = f(0)$.
        Donc, $f$ n'est pas injective.
        Puisque $G$ et $[\![0, n-1]\!]$ ont le même cardinal $n$, $f$ n'est donc pas surjective (sans quoi elle serait bijective, et donc injective).
        Donc, on peut choisir un élément $h$ de $g$ tel que, pour tout élément $k$ de $[\![0, n-1]\!]$, $g^k \leq h$.
        Soit $k$ un entier naturel quelconque et $q$ et $l$ le quotient et le reste de la division euclidienne de $k$ par $m$. 
        ALors, $l < m$, donc $l < n$.
        On a : $g^k = g^{q m + l} = (g^m)^q \cdot g^l = g^l$.
        Donc, $g^k \neq h$.
        Ainsi, $g$ n'est pas un générateur de $(G,\cdot)$.
\stopitemize

\done

\stopsubsection

\startsubsection[title=Groupe quotient (groupe abélien)]

\index{Quotient}

\noindent\bold{Définition :} Soit $(G, \cdot)$ un groupe abélien et $(H, \cdot)$ un sous-groupe de $(G, \cdot)$.
    On définit l'ensemble $G \symbols{\divslash} H$ comme l'ensemble des classes d'équivalences de $G$ pour la relation $R$ définie par : $\forall g \in G \, \forall g\p \in G \, g R g\p \Leftrightarrow g^{-1} \cdot g\p \in H$, oú un exposant $-1$ indique l'inverse. 
    (On montre ci-dessous qu'il s'agit bien d'une relation d'équivalence.)
    Pour tout élément $g$ de $G$, on note $\bar{g}$ la classe d'équivalence de $g$ pour $R$.
    On définit la loi de composition interne $\cdot$ sur $G \divslash H$ de la manière suivante : soit $c_1$ et $c_2$ deux éléments de $G \divslash H$, on peut choisir un éléments $g_1$ de $c_1$ et un élément $g_2$ de $c_2$ ; on pose alors $c_1 \cdot c_2 = \overline{g_1 \cdot g_2}$. 
    Alors, 
    \startitemize[nowhite]
        \item La loi de composition interne $\cdot$ sur $G \divslash H$ est bien définie.
        \item $(G \divslash H, \cdot)$ est un groupe abélien, appelé \emph{groupe quotient} de $(G, \cdot)$ et $(H, \cdot)$.
    \stopitemize

\blank[medium]

\noindent\bold{Preuve qu'il s'agit d'une relation d'équivalence :} 
Soit $(G, \cdot)$ un groupe (abélien ou non) et $R$ la relation définie comme ci-dessus.
\startitemize[nowhite]
    \item \emph{Réflexivité :} Soit $g$ un élément de $G$.
        Alors, $g^{-1} \cdot g$ est l'élément neutre de $(G, +)$, donc appartient à $H$.
        Donc, $g R g$.
    \item \emph{Symétrie :} Soit $g$ et $g\p$ deux éléments de $G$ tels que $g R g\p$.
        Alors, $g^{-1} \cdot g\p \in H$.
        Puisque $(H, \cdot)$ est un groupe, $(g^{-1} \cdot g\p)^{-1} \in H$, donc $g\p^{-1} \cdot g \in H$, donc $g\p R g$.
    \item \emph{Transitivité :} Soit $g$, $g\p$ et $g\p'$ trois éléments de $G$ tels que $g R g\p$ et $g\p R g\p'$.
        Alors, on peut choisir deux éléments $h$ et $h'$ de $H$ tels que $g^{-1} \cdot g\p = h$ et $g\p^{-1} \cdot g\p' = h'$.
        Puisque $(H, \cdot)$ est un groupe, $h \cdot h' \in H$, donc $g^{-1} \cdot g\p' \in H$, donc $g R g\p'$.
\stopitemize

\done

\blank[medium]

\noindent
Notons que cette démonstration n'utilise pas le fait que $(G, \cdot)$ est abélien.
La relation $R$ définie comme ci-dessus est donc une relation d'équivalence pour tout groupe $(G, \cdot)$.

\blank[medium]

\noindent\bold{Preuve que la loi de composition interne $\cdot$ sur $G \divslash H$ est bien définie :} Il s'agit de montrer que le résultat ne dépend pas du choix de $g_1$ et $g_2$.
    Soit $c_1$ et $c_2$ deux éléments de $G \divslash H$. 
    Soit $g_1$ et $g_3$ deux éléments de $c_1$ et $g_2$ et $g_4$ deux éléments de $c_2$.
    On a alors $g_1 R g_3$ et $g_2 R g_4$.
    On peut donc choisir deux éléments $h_1$ et $h_2$ de $H$ tels que $g_1^{-1} \cdot g_3 = h_1$ et $g_2^{-1} \cdot g_4 = h_2$.
    On a alors : $(g_1 \cdot g_2)^{-1} \cdot (g_3 \cdot g_4) = (g_2^{-1} \cdot g_1^{-1}) \cdot (g_3 \cdot g_4) = (g_1^{-1} \cdot g_3) \cdot \cdot (g_2^{-1} \cdot g_4) = h_1 \cdot h_2$. 
    Puisque $(H, \cdot)$ est un groupe, on en déduit que $(g_1 \cdot g_2)^{-1} \cdot (g_3 \cdot g_4) \in H$, donc $(g_1 \cdot g_2) R (g_3 \cdot g_4)$, et donc $\overline{g_1 \cdot g_2} = \overline{g_3 \cdot g_4}$.

\done

\blank[medium]

\noindent\bold{Preuve que $(G \divslash H, \cdot)$ est un groupe abélien :}
\startitemize[nowhite]
    \item Par définition, $\cdot$ est une loi de composition interne sur $G \divslash H$.
    \item Commutativité : Soit $c_1$ et $c_2$ deux éléments de $G \divslash H$.
        Soit $g_1$ un élément de $c_1$ et $g_2$ un élément de $c_2$. 
        On a : $c_1 \cdot c_2 = \overline{g_1 \cdot g_2}$ et $c_2 \cdot c_1 = \overline{g_2 \cdot g_1}$.
        Puisque $(G, \cdot)$ est abélien, $g_1 \cdot g_2 = g_2 \cdot g_1$, donc $c_1 \cdot c_2 = c_2 \cdot c_1$.
    \item Soit $e$ l'élément neutre de $(G, \cdot)$. 
        Soit $c$ un élément de $G \divslash H$ et $g$ un élément de $c$. 
        On a : $c \cdot \bar{e} = \overline{g \cdot e} = \bar{g} = c$.
        Par commutativité, cela implique également $\bar{e} \times c = c$.
        Donc, $\bar{e}$ est un élément neutre de $G \divslash H$ pour $\cdot$.
    \item Associativité : Soit $c_1$, $c_2$ et $c_3$ trois éléments de $G \divslash H$.
        Soit $g_1$ un élément de $c_1$, $g_2$ un élément de $c_2$ et $g_3$ un élément de $c_3$.
        On a : $c_1 \cdot (c_2 \cdot c_3) = c_1 \cdot \overline{g_2 \cdot g_3} = \overline{g_1 \cdot (g_2 \cdot g_3)} = \overline{(g_1 \cdot g_2) \cdot g_3} = \overline{g_1 \cdot g_2} \cdot c_3 = (c_1 \cdot c_2) \cdot c_3$.
    \item Soit $c$ un élément de $G \divslash H$ et $g$ un élément de $c$. 
        Notons $g^{-1}$ l'inverse de $g$ et $e$ l'élément neutre de $(G,\cdot)$. 
        Alors, $c \cdot \bar{g^{-1}} = \overline{g \cdot g^{-1}} = \bar{e}$.
        Par commutativité, cela implique également $\bar{g^{-1}} \times c = \bar{e}$.
        Puisque $\bar{e}$ est un élément neutre pour $\cdot$, on conclut que $\bar{g^{-1}}$ est un inverse de $c$.
\stopitemize

\done

\stopsubsection

\startsubsection[title=Groupe quotient (cas général)]

\index{Quotient}

\noindent
Dans cette section, on note l'inverse par un exposant $-1$.

\blank[medium]

\noindent\bold{Définition :} Sous-groupe \words{normal} Soit $(G, \cdot)$ un groupe et $(H, \cdot)$ un sous-groupe de $(G, \cdot)$.
    On dit que $H$ est \emph{normal} si pour tout élément $h$ de $H$ et tout élément $g$ de $G$, $g \cdot h \cdot g^{-1} \in H$. 

\blank[medium]

\noindent\bold{Remarque :} Soit $(G, \cdot)$ un groupe et $(H, \cdot)$ un sous-groupe normal de $(G, \cdot)$.
    Pour tout élément $h$ de $H$ et tout élément $g$ de $G$, $g^{-1} \in G$, donc $g^{-1} \cdot h \cdot g \in H$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(G, \cdot)$ un groupe abélien et $(H, \cdot)$ un sous-groupe de $(G, \cdot)$.
    On définit les deux relations $R_G$ et $R_D$ sur $G$ de la manière suivante : pour tous éléments $g$ et $g\p$ de $G$, $g R_G g\p \Leftrightarrow g^{-1} \cdot g\p \in H$ et $g R_D g\p \Leftrightarrow g \cdot g\p^{-1} \in H$. 
    Alors, $R_G$ et $R_D$ sont des relations d'équivalence. 

\blank[medium]

\noindent\bold{Démnstration :} 
\startitemize[nowhite]
    \item \emph{Réflexivité :} Soit $g$ un élément de $G$.
        Alors, $g^{-1} \cdot g$ et $g \cdot g^{-1}$ sont égaux à l'élément neutre de $(G, +)$, donc appartiennent à $H$.
        Donc, $g R_G g$ et $g R_L g$.
    \item \emph{Symétrie ($R_G$) :} Soit $g$ et $g\p$ deux éléments de $G$ tels que $g R_G g\p$.
        Alors, $g^{-1} \cdot g\p \in H$.
        Puisque $(H, \cdot)$ est un groupe, $(g^{-1} \cdot g\p)^{-1} \in H$, donc $g\p^{-1} \cdot g \in H$, donc $g\p R_G g$.
    \item \emph{Symétrie ($R_D$) :} Soit $g$ et $g\p$ deux éléments de $G$ tels que $g R_D g\p$.
        Alors, $g \cdot g\p^{-1} \in H$.
        Puisque $(H, \cdot)$ est un groupe, $(g \cdot g\p^{-1})^{-1} \in H$, donc $g\p \cdot g^{-1} \in H$, donc $g\p R_D g$.
    \item \emph{Transitivité ($R_G$) :} Soit $g$, $g\p$ et $g\p'$ trois éléments de $G$ tels que $g R_G g\p$ et $g\p R_G g\p'$.
        Alors, on peut choisir deux éléments $h$ et $h'$ de $H$ tels que $g^{-1} \cdot g\p = h$ et $g\p^{-1} \cdot g\p' = h'$.
        Puisque $(H, \cdot)$ est un groupe, $h \cdot h' \in H$, donc $g^{-1} \cdot g\p' \in H$, donc $g R_G g\p'$.
    \item \emph{Transitivité ($R_D$) :} Soit $g$, $g\p$ et $g\p'$ trois éléments de $G$ tels que $g R_D g\p$ et $g\p R_D g\p'$.
        Alors, on peut choisir deux éléments $h$ et $h'$ de $H$ tels que $g \cdot g\p^{-1} = h$ et $g\p \cdot g\p\p^{-1} = h'$.
        Puisque $(H, \cdot)$ est un groupe, $h \cdot h' \in H$, donc $g \cdot g\p\p^{-1} \in H$, donc $g R_D g\p'$.
\stopitemize

\done

\blank[medium]

\noindent\bold{Lemme :} Avec les notations du lemme précédent, si $(H, \cdot)$ est normal, alors les relations $R_L$ et $R_D$ définies ci-dessus sont identiques, au sens suivant : $\forall g \in G \, \forall g\p \in G \, g R_L g\p \Leftrightarrow g R_D g\p$.
    On notera dans la suite simplement $R$ pour désigner une de ces deux relations.

\blank[medium]

\noindent\bold{Démnstration :} 
    Soit $g$ et $g\p$ deux éléments de $G$.
    
    Si $g R_G g\p$, alors on peut choisir un élément $h$ de $H$ $g^{-1} \cdot g\p = h$.
    Puisque $(H, \cdot)$ est un sous-groupe normal de $(G, \cdot)$, $g \cdot h \cdot g^{-1} \in H$.
    Donc, $g\p \cdot g^{-1} \in H$.
    Donc, $g R_D g\p$.
    
    Si $g R_D g\p$, alors on peut choisir un élément $h$ de $H$ $g\p \cdot g^{-1} = h$.
    Puisque $(H, \cdot)$ est un sous-groupe normal de $(G, \cdot)$, $g^{-1} \cdot h \cdot g \in H$.
    Donc, $g^{-1} \cdot g\p \in H$.
    Donc, $g R_L g\p$.

    \done

\blank[medium]

\noindent\bold{Notation :} Soit $(G, \cdot)$ un groupe et $(H, \cdot)$ un sous-groupe normal de $(G, \cdot)$.
    L'ensemble quotient $G \divslash R_L$, où $R_L$ est définie comme ci-dessus, est aussi noté $G \divslash H$.

\blank[medium]

\noindent\bold{Définition :} Soit $(G, \cdot)$ un groupe et $(H, \cdot)$ un sous-groupe normal de $(G, \cdot)$.
    On définit la loi de composition interne $\cdot$ sur $G \divslash H$ comme suit.
    Soit $E$ et $F$ deux éléments de $G \divslash H$.
    Soit $e$ un élément de $E$ et $f$ un élément de $F$.
    Alors, $E \times F$ est l'élément de $G \divslash H$ contenant $e \cdot f$.

\blank[medium]

\noindent\bold{Preuve que cette loi de composition interne est bien définie :} 
    Il s'agit de montrer qu'elle ne dépend pas des choix de représentants.
    Soit $e$ et $e'$ deux éléments de $E$ et $f$ et $f\p$ deux éléments de $f$.
    Alors, on peu choisir deux éléments $h$ et $h'$ de $H$ tels que $e^{-1} \cdot e' = h$ (puisque $e R_G e'$) et $f \cdot f\p^{-1} = h'$ (puisque $f R_D f\p$).
    Puisque $(H, \cdot)$ est un sous-groupe normal de $(G, \cdot)$, $f^{-1} \cdot h \cdot f \in H$.
    Donc, $f^{-1} \cdot e^{-1} \cdot e' \cdot f \in H$, donc $(e \cdot f)^{-1} \cdot (e' \cdot f) \in H$, donc $(e \cdot f) \mathrel{R} (e' \cdot f)$.
    De même, $e' \cdot h' \cdot e\p^{-1}$, donc $e' \cdot f \cdot f\p^{-1} \cdot e\p^{-1} \in H$, donc $(e' \cdot f) \cdot (e' \cdot f\p)^{-1} \in H$, donc $(e' \cdot f) \mathrel{R} (e' \cdot f\p)$.
    Puisque $R$ est transitive, on en conclut que $(e \cdot f) \mathrel{R} (e' \cdot f\p)$, et donc que les classes obtenues en choisissant les représentants $e$ et $f$ ou les représentants $e'$ et $f\p$ sont identiques.
    
    \done

\blank[medium]

\noindent\bold{Preuve que cela définit un groupe :}
    Pour tout élément $x$ de $G$, on note $\bar{x}$ l'élément de $G \divslash H$ contenant $e$.

    Soit $1$ l'élément neutre de $(G, \cdot)$. 
    Soit $F$ un élément de $G \divslash H$. 
    On peut choisir un élément $f$ de $F$.
    Alors, $\bar{1} \times F = \bar{1} \times \bar{f} = \overline{1 \cdot f} = \bar{f} = F$ et $F \times \bar{1} = \bar{f} \times \bar{1} = \overline{f \cdot 1} = \bar{f} = F$.
    Donc, $\bar{1}$ est un élément neutre pour $\times$.

    Soit $E$, $F$ et $H$ trois éléments de $G \divslash H$.
    On peut choisir trois éléments $e$, $f$ et $h$ de $G$ tels que $e \in E$, $f \in F$ et $h \in H$
    Alors, $(E \times F) \cdot H = (\bar{e} \times \bar{f}) \times \bar{h} = \overline{ \cdot f} \times h = \overline{(e \cdot f) \cdot h} = \overline{e \cdot (f \cdot h)} = \bar{e} \times \overline{f \cdot h} = \bar{e} \times (\bar{f} \times \bar{h}) = E \times (F \times H)$.
    Donc, la loi de composition interne $\times$ sur $G \divslash H$ est associative.

    Soit $E$ un élément de $G \divslash H$.
    On peut choisir un élément $e$ de $G$ tel que $e \in E$.
    Puisque $(G, \cdot)$ est un groupe, $e$ admet un inverse.
    Notons ce dernier $f$.
    Alors, $E \times \bar{f} = \bar{e} \times \bar{f} = \overline{e \times g} = \overline{1}$ et $\bar{f} \times E = \bar{f} \times \bar{e} = \overline{f \times e} = \bar{1}$.
    Cela montre que $\bar{f}$ est un inverse de $E$ pour $\times$.
    
    \done

\stopsubsection

\startsubsection[title=Anneaux et corps]

\noindent\bold{Définition (\words{Anneau}) :} Soit $A$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $A$. 
    Le triplet $(A, +, \times)$ est un \emph{anneau} si les propriétés suivantes sont satisfaites : 
    \startitemize[nowhite]
        \item $(A,+)$ est un groupe abélien,
        \item $(A, \times)$ est un magma associatif,
        \item $\times$ est distributive sur $+$ : pour tous éléments $a$, $b$ et $c$ de $A$, on a $a \times (b + c) = (a \times b) + (a \times c)$ et $(a + b) \times c = (a \times c) + (b \times c)$.
    \stopitemize
    L'élément neutre de $A$ pour l'opération $+$ est dit \emph{nul}.

\blank[medium]

\noindent\bold{Notation :} Soit $(A, +, \times)$ un anneau. 
    Quand il n'y a pas de confusion possible, on peut omettre le symbole $\times$ dans les formules.
    Ainsi, si $a$ et $b$ sont deux éléments de $A$, $a \times b$ pourra s'écrire $a b$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau. 
On note $0$ l'élément neutre de $(A, +)$.
Pour tout élément $a$ de $A$, $0 \times a = a \times 0 = 0$.

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $b$ l'élément $0 \times a$. 
    On a : $b + b = (0 \times a) + (0 \times a) = (0 + 0) \times a = 0 \times a = b$.
    Soit $\bar{b}$ l'inverse de $b$ pour l'opération $+$ (qui existe car $(A, +)$ est un groupe).
    Alors, $b = 0 + b = (\bar{b} + b) + b = \bar{b} + (b + b) = \bar{b} + b = 0$. 

    Notons $d$ l'élément $a \times 0$. 
    On a : $d + d = (a \times 0) + (a \times 0) = a \times (0 + 0) = a \times 0 = d$.
    Soit $\bar{d}$ l'inverse de $d$ pour l'opération $+$ (qui existe car $(A, +)$ est un groupe).
    Alors, $d = d + 0 = d + (d + \bar{d}) = (d + d) + \bar{d} = d + \bar{d} = 0$. 

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau. 
On note $0$ l'élément neutre de $(A, +)$.
On suppose que tout élément de $A$ distinct de $0$ admet un inverse pour $\times$.
Soit $a$ et $b$ deux éléments de $A$ tels que $a \times b = 0$.
Alors $a = 0$ ou $b = 0$.

\blank[medium]

\noindent\bold{Démonstration :} Si $a = 0$, le résultat est vrai.
Sinon, $a$ admet un inverse, pour $\times$, noté $\bar{a}$.
Alors, $b = (\bar{a} \times a) \times b = \bar{a} \times (a \times b) = \bar{a} \times 0 = 0$.

\done

\blank[medium]

\noindent\bold{Remarque :} Cela est faux en général si on enlève l'hypothèse sur $A$. 
Considérons par exemple l'anneau $(\mathbb{Z}_N, +, \times)$ (voir \in{section}{}[sec:def_Z_nZ]) où $N$ est un entier naturel strictement supérieur à $1$ non premier. 
Soit $d$ un diviseur de $N$ distinct de $1$ et $N$, et $k$ l'entier naturel tel que $d \times k = N$. 
Alors $d > 1$, $d < N$, $k > 1$ et $k < N$, donc $\bar{d} \ neq \bar{0}$ et $\bar{k} \ neq \bar{0}$ mais $\bar{d} \times \bar{k} = \bar{N} = 0$.

\blank[medium]

\noindent\bold{Définition (Anneau \words{unifère}) :} Soit $A$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $A$ tels que $(A, +, \times)$ est un anneau. 
    L'anneau $(A, +, \times)$ est dit \emph{unifère}, ou \emph{unitaire}, si la loi de composition interne $\times$ admet un élément neutre.

\blank[medium]

\noindent\bold{Définition :} Un anneau ne contenant qu'un seul élément est dit \emph{\words{nul}}. 

\blank[medium]

Soit $(A, +, \times)$ un anneau unifère, $0$ l'élément neutre de $+$, et $1$ l'élément neutre de $\times$. 
Si $0 = 1$, alors $A$ est nul. 
En effet, pour tout élément $a$ de $A$, on a $a = 1 \times a$, donc $a = 0 \times a$, donc $a = 0$. 

\blank[medium]

\noindent\bold{Définition :} Un anneau $(A, +, \times)$ est dit \emph{\words{intègre}} si 
    \startitemize[nowhite]
        \item $(A, +, \times)$ est non nul, 
        \item soit $0$ l'élément neutre de $A$ pour $+$, $\forall (a, b) \in A^2 \, a \times b = 0 \Rightarrow (a = 0 \vee b = 0)$.
    \stopitemize

\blank[medium]

\noindent\bold{Définition (\words{Morphisme} d'anneaux) :} Soit $(A, +, \times)$ et $(B, \oplus, \otimes)$ deux anneaux unifères.
    Notons $1_A$ l'élément neutre de $A$ pour $\times$ et $1_B$ celui de $B$ pour $\otimes$.
    Soit $f$ une fonction de $A$ vers $B$. 
    Alors, $f$ est un \emph{morphisme d'anneaux} de $(A, +, \times)$ vers $(B, \oplus, \otimes)$ si (et seulement si)
    \startitemize[nowhite]
        \item pour tous éléments $a$ et $b$ de $A$, $f(a + b) = f(a) \oplus f(b)$,
        \item pour tous éléments $a$ et $b$ de $A$, $f(a \times b) = f(a) \otimes f(b)$,
        \item $f(1_A) = 1_B$.
    \stopitemize

\blank[medium]

\noindent\bold{Remarque :} Soit $(A, +, \times)$ et $(B, \oplus, \otimes)$ deux anneaux unifères.
    Notons $0_A$ l'élément neutre de $A$ pour $+$ et $0_B$ celui de $B$ pour $\oplus$.
    Soit $f$ un morphisme d'anneaux de $(A, +, \times)$ vers $(B, \oplus, \otimes)$.
    Alors, $f$ est un morphisme de groupes de $(A, +)$ vers $(B, \oplus)$.
    Donc, 
    \startitemize[nowhite]
        \item $f(0_A) = 0_B$, 
        \item soit $a$ et $b$ deux éléments de $a$ tels que $b$ est l'inverse de $a$ pour $+$, alors $f(b)$ es l'inverse de $f(a)$ pour $\oplus$.
    \stopitemize

\blank[medium]

\noindent\bold{Définition (Anneau \words{commutatif}) :} Soit $A$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $A$ tels que $(A, +, \times)$ est un anneau. 
    L'anneau $(A, +, \times)$ est dit \emph{commutatif} si la loi de composition interne $\times$ est commutative.

\blank[medium]

\noindent\bold{Définition (\words{Corps}) :} Soit $K$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $K$. 
    Le triplet $(K, +, \times)$ est un \emph{corps} si les propriétés suivantes sont satisfaites : 
    \startitemize[nowhite]
        \item $(A,+,\times)$ est un anneau commutatif, unifère, et non nul,
        \item en notant $0$ l'élément neutre de $+$ et $1$ celui de $\times$, tout élément de $K$ distinct de $0$ admet un inverse pour $\times$, c'est-à-dire : $\forall k \in K \, (k \neq 0) \Rightarrow (\exists l \in K \, k \times l = l \times k = 1)$.
    \stopitemize

\blank[medium]

\noindent\bold{Remarque :} Soit $K$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $K$ tels que $(K, +, \times)$ est un corps. 
    Soit $0$ l'élément neutre de $+$.
    Notons $K^*$ l'ensemble $K \setminus \lbrace 0 \rbrace$.
    Si $a$ et $b$ sont deux éléments de $K^*$, alors $a \times b$ est encore un élément de $K^*$. 
    (En effet, il ne peut être égal à $0$ : puisque $a$ est non nul, il adet un inverse $c$ pout $\times$, et on a $c \times (a \times b) = b$, donc $c \times (a \times b) \neq 0$, donc $a \times b \neq 0$.)
    Donc, $(K^*, \times)$ est un magma.
    En outre, 
    \startitemize[nowhite]
        \item La loi de composition interne $\times$ est associative.
        \item La loi de composition interne $\times$ est commutative.
        \item La loi de composition interne $\times$ admet un élément neutre $1$ (qui est distinct de $0$ puisque l'anneau $(K, +, \times)$ est non nul, et donc apppartient à $K^*$).
        \item Soit $a$ un élément de $K^*$. 
            Puisque $a \in K$ et $a \neq 0$, $a$ admet un inverse $b$ pour $\times$ dans $K$.
            Puisque $a \times b = 1$ et $1 \neq 0$, $b \neq 0$, donc $b \in K^*$.
            Donc, $a$ admet un inverse pour $\times$ dans $K^*$.
    \stopitemize
    Ainsi, $(K^*, \times_*)$, où $\times_*$ désigne la restriction de la loi de composition interne $\times$ à $K^*$, est un groupe abélien.

\blank[medium]

\noindent\bold{Lemme :} Soit $K$ un ensemble et $+$ et $\times$ deux lois de composition interne sur $K$ tels que $(K, +, \times)$ est un corps. 
    Soit $0$ l'élément neutre de $+$.
    Soit $a$ et $b$ deux éléments de $K$ tels que $a \neq 0$ et $b \neq 0$.
    Alors, $a \times b \neq 0$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $c$ l'inverse de $a$ pour $\times$.
    On a : $c \times (a \times b) = (c \times a) \times b = b$.
    Donc, : $c \times (a \times b) \neq 0$.
    Donc, : $a \times b \neq 0$.

    \done

\blank[medium]

\noindent\bold{Notation :} Soit $(A, +, \times)$ un anneau, $0_A$ son élément neutre pour $+$ et, si l'anneau est unifère $1_A$ son élément neutre pour $\times$.
    Si $n$ et $m$ sont deux entiers tels que $n \geq 0$ et $a_n$, $a_{n+1}$, ..., $a_m$ des éléments de $A$ (aucun élément si $m < n$), alors 
    \startitemize[nowhite]
        \item On pourra noter $\sum_{i=n}^m a_i$ (où $i$ est une variable) l'élément $a_0 + a_1 + \cdots + a_m$ (ou $0_A$ si $m < n$).
        \item Si $(A, +, \times)$ est unifère, on pourra noter $\prod_{i=n}^m a_i$ (où $i$ est une variable) l'élément $a_0 \times a_1 \times \cdots \times a_m$ (ou $1_A$ si $m < n$).
    \stopitemize
    On peut définir ces notations plus rigoureusement de manière suivante. 
    Soit $a$ une suite d'éléments de $A$.
    Soit $n$ un entier naturel.
    Pour tout entier strictement négatif $m$, on définit $\sum_{i=n}^{n+m} a_i$ comme égal à $0_A$ et $\prod_{i=n}^{n+m} a_i$ comme égal à $1_A$.
    On définit $\sum_{i=n}^n a_i$ et $\prod_{i=n}^n a_i$ comme égaux à $a_n$.
    Enfin, pour tout entier naturel $m$, on définit $\sum_{i=n}^{n+(m+1)} a_i$ comme égal à $\left( \sum_{i=n}^{n+m} a_i \right) + a_{n+(m+1)}$ et $\prod_{i=n}^{n+(m+1)} a_i$ comme égal à $\left( \prod_{i=n}^{n+m} a_i \right) \times a_{n+(m+1)}$.

\blank[medium]

\noindent\bold{Remarque :} On peut étendre les définitions ci-dessus au cas d'un $n$-uplet $(b_0, b_1, \dots b_{n-1})$ d'éléments de $A$, où $n$ est un entier naturel strictement supérieur à $0$, en définissant la suite $a$ d'éléments de $A$ par : 
\startitemize[nowhite]
    \item pour tout élément $m$ de $[\![0, n-1]\!]$, $a_m = b_m$, 
    \item pour $\sum$, pour tout entier $m$ supérieur ou égal à $n$, $a_m = 0_A$, 
    \item si $(A, +, \times)$ est unifère, pour $\prod$, pour tout entier $m$ supérieur ou égal à $n$, $a_m = 1_A$.
\stopitemize
On pose alors, pour tous entiers naturels $k$ et $l$ strictement inférieurs à $n$ : 
\startformula
    \sum_{i=k}^l b_i = \sum_{i=k}^l a_i
\stopformula
et, si $(A, +, \times)$ est unifère
\startformula
    \prod_{i=k}^l b_i = \prod_{i=k}^l a_i.
\stopformula

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $n$ et $m$ deux entiers naturels, et $a$ une suite d'éléments de $A$ et $b$ un élément de $A$. 
    Alors,
    \startformula
        \sum_{i=n}^m (a_i \times b) = \left( \sum_{i=n}^m a_i \right) \times b 
    \stopformula
    et
    \startformula
        \sum_{i=n}^m (b \times a_i) = b \times \left( \sum_{i=n}^m a_i \right) .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $0_A$ l'élément neutre de $(A, +)$.
    Si $m < n$, on a $\sum_{i=n}^m (a_i \times b) = 0_A$, $\left( \sum_{i=n}^m a_i \right) \times b = 0_A \times b = 0_A$, $\sum_{i=n}^m (b \times a_i) = 0_A$ et $b \times \left( \sum_{i=n}^m a_i \right) = b \times 0_A = 0_A$, donc les deux formules sont vraies.
    Montrons par récurrence qur $m$ que cela reste le cas pour $m \geq n$.

    Pour $m = n$, $\sum_{i=n}^m (a_i \times b)$ et $\left( \sum_{i=n}^m a_i \right) \times b$ sont tous deux égaux à $a_n \times b$, donc égaux, et $\sum_{i=n}^m (b \times a_i)$ et $b \times \left( \sum_{i=n}^m a_i \right)$ sont tous deux égaux à $b \times a_n$, donc égaux.

    Soit $m$ un entier tel que les deux formules du lemme sont vraies.
    Alors, 
    \startformula
        \sum_{i=n}^{m+1} (a_i \times b) 
        = \left( \sum_{i=n}^m (a_i \times b) \right) + a_{m+1} \times b
        = \left( \sum_{i=n}^m a_i \right) \times b + a_{m+1} \times b
        = \left( \left( \sum_{i=n}^m a_i \right) + a_{m+1} \right) \times b 
        = \left( \sum_{i=n}^{m+1} a_i \right) \times b 
    \stopformula
    et
    \startformula
        \sum_{i=n}^{m+1} (b \times a_i) 
        = \left( \sum_{i=n}^m (b \times a_i) \right) + b \times a_{m+1}
        = b \times \left( \sum_{i=n}^m a_i \right) + b \times a_{m+1}
        = b \times \left( \left( \sum_{i=n}^m a_i \right) + a_{m+1} \right)
        = b \times \left( \sum_{i=n}^{m+1} a_i \right) .
    \stopformula
    Ce sont les deux formulles du lemme avec $m$ remplacé par $m+1$.
    Par récurrence, ce résultat est vrai pour tout entier $m$ supérieur ou égal à $n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $n$, $m$ et $k$ trois entiers naturels et $a$ une suite d'éléments de $A$. 
    On suppose que $n \leq k \leq m$
    Alors,
    \startformula
        \left( \sum_{i=n}^k a_i \right) + \left( \sum_{i=k+1}^m a_i \right) = \sum_{i=n}^m a_i
    \stopformula
    et, si $(A, +, \times)$ est unifère,
    \startformula
        \left( \prod_{i=n}^k a_i \right) \times \left( \prod_{i=k+1}^m a_i \right) = \prod_{i=n}^m a_i.
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :}  
    On procède par récurrence sur $m$.
    Notons $0_A$ l'élément neutres de $(A, +)$ et, si l'anneau est unifère, $1_A$ celui de $(A, \times)$.
    Pour $m = n$, on a nécessairement $k = n$, donc $k+1 > m$ et 
    \startformula
        \left( \sum_{i=n}^k a_i \right) + \left( \sum_{i=k+1}^m a_i \right) 
        = \left( \sum_{i=n}^n a_i \right) + 0_A
        = \sum_{i=n}^n a_i 
        = \sum_{i=n}^m a_i .
    \stopformula
    De même, si $(A, +, \times)$ est unifère, 
    \startformula
        \left( \prod_{i=n}^k a_i \right) \times \left( \prod_{i=k+1}^m a_i \right) 
        = \left( \prod_{i=n}^n a_i \right) \times 1_A
        = \prod_{i=n}^n a_i 
        = \prod_{i=n}^m a_i .
    \stopformula

    Soit $m$ un entier naturel supérieur ou égal à $n$ tel que le résultat soit vrai.
    Soit $k$ un entier tel que $n \leq k \leq m+1$. 
    Alors, $k \leq m$ ou $ = m+1$.
    Si $k = m+1$, alors $k+1 > m+1$, donc 
    \startformula
        \left( \sum_{i=n}^k a_i \right) + \left( \sum_{i=k+1}^{m+1} a_i \right)
        = \left( \sum_{i=n}^{m+1} a_i \right) + 0_A
        = \sum_{i=n}^{m+1} a_i 
    \stopformula
    et, si $(A, +, \times)$ est unifère, 
    \startformula
        \left( \prod_{i=n}^k a_i \right) \times \left( \prod_{i=k+1}^{m+1} a_i \right)
        = \left( \prod_{i=n}^{m+1} a_i \right) \times 1_A
        = \prod_{i=n}^{m+1} a_i .
    \stopformula
    Sinon, on a
    \startformula
        \left( \sum_{i=n}^k a_i \right) + \left( \sum_{i=k+1}^{m+1} a_i \right)
        = \left( \sum_{i=n}^k a_i \right) + \left( \sum_{i=k+1}^m a_i \right) + a_{m+1}
        = \left( \sum_{i=n}^k a_m \right) + a_{m+1}
        = \sum_{i=n}^{m+1} a_i
    \stopformula
    et, si $(A, +, \times)$ est unifère, 
    \startformula
        \left( \prod_{i=n}^k a_i \right) \times \left( \prod_{i=k+1}^{m+1} a_i \right)
        = \left( \prod_{i=n}^k a_i \right) \times \left( \prod_{i=k+1}^m a_i \right) + a_{m+1}
        = \left( \prod_{i=n}^k a_m \right) \times a_{m+1}
        = \prod_{i=n}^{m+1} a_i .
    \stopformula
    Le résultat attendu est donc vrai au rang $m+1$.
    Par récurrence, il l'est pour tout entier $m$ supérieur ou égal à $n$.

    \done

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $n$ et $m$ deux entiers naturels, et $a$ et $b$ deux suites d'éléments de $A$. 
    Alors,
    \startformula
        \sum_{i=n}^m (a_i + b_i) = \left( \sum_{i=n}^m a_i \right) + \left( \sum_{i=n}^m b_i \right) .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $0_A$ l'élément neutre de $(A, +)$.
    Si $m < n$, alors $\sum_{i=n}^m (a_i + b_i) = \left( \sum_{i=n}^m a_i \right) = \left( \sum_{i=n}^m b_i \right) = 0_A$, donc le résultat est évident.
    Montrons par récurrence sur $m$ que le lemme reste vrai pour $m \geq n$.

    Si $m = n$, alors 
    \startformula
        \left( \sum_{i=n}^m a_i \right) + \left( \sum_{i=n}^m b_i \right)
        = a_n + b_n
        = \sum_{i=n}^m (a_i + b_i).
    \stopformula

    Soit $n$ et $m$ deux entiers naturels tels que l'énoncé du lemme est vrai et $m \geq n$.
    Alors, 
    \startformula
        \sum_{i=n}^{m+1} (a_i + b_i) 
        = \left( \sum_{i=n}^m (a_i + b_i) \right) + (a_{m+1} + b_{m+1})
        = \left( \left( \sum_{i=n}^m a_i \right) + \left( \sum_{i=n}^m b_i \right) \right) + (a_{m+1} + b_{m+1})
    \stopformula
    \startformula
        \sum_{i=n}^{m+1} (a_i + b_i) 
        = \left( \left( \sum_{i=n}^m a_i \right) + a_{m+1} \right) + \left( \left( \sum_{i=n}^m b_i \right) + b_{m+1} \right)
        = \left( \sum_{i=n}^m a_i \right) + \left( \sum_{i=n}^m b_i \right) .
    \stopformula
    L'énoncé du lemme reste vrai en remplaçant $m$ par $m+1$.
    Par récurrence, il est donc vrai pour tout entier $m$ supérieur ou égal à $n$. 

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau commutatif, $n$ et $m$ deux entiers naturels, et $a$ et $b$ deux suites d'éléments de $A$. 
    Alors,
    \startformula
        \prod_{i=n}^m (a_i \times b_i) = \left( \prod_{i=n}^m a_i \right) \times \left( \prod_{i=n}^m b_i \right) .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} Identique à la précédente en remplaçant $\sum$ par $\prod$ et $+$ par $\times$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $n$, $m$, $k$ et $l$ quatre entiers naturels, et $a$ et $b$ deux suites d'éléments de $A$. 
    Alors,
    \startformula
        \left( \sum_{i=n}^m a_i \right) \times \left( \sum_{j=k}^l b_j \right) = \sum_{i=n}^m \sum_{j=k}^l a_i \times b_j .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} 
    Notons $0_A$ l'élément neutre de $(A, +)$.
    Si $m < n$, on a $\sum_{i=n}^m a_i = 0_A$ et $\sum_{i=n}^m \sum_{j=k}^l a_i \times b_j = 0_A$, donc les deux membres de l'égalité sont égaux à $0_A$, et donc égaux.
    Montrons par récurrence que le résultat reste vrai pour $m \geq n$. 

    Si $m = n$, on a
    \startformula
        \left( \sum_{i=n}^m a_i \right) \times \left( \sum_{j=k}^l b_j \right) = a_n \times \sum_{j=k}^l b_j
    \stopformula
    et
    \startformula
        \sum_{i=n}^m \sum_{j=k}^l a_i \times b_j = \sum_{j=k}^l a_n \times b_j = a_n \times \sum_{j=k}^l b_j,
    \stopformula
    donc le résultat est vrai.

    Supposons maontenant que, pour un certain netier $m$,
    \startformula
        \left( \sum_{i=n}^m a_i \right) \times \left( \sum_{j=k}^l b_j \right) = \sum_{i=n}^m \sum_{j=k}^l a_i \times b_j .
    \stopformula
    Alors, 
    \startformula
        \left( \sum_{i=n}^{m+1} a_i \right) \times \left( \sum_{j=k}^l b_j \right)
        = \left( \left( \sum_{i=n}^m a_i \right) + a_{m+1} \right) \times \left( \sum_{j=k}^l b_j \right)
        = \left( \sum_{i=n}^m a_i \right) \times \left( \sum_{j=k}^l b_j \right) + a_{m+1} \times \left( \sum_{j=k}^l b_j \right)
    \stopformula
    \startformula
        \left( \sum_{i=n}^{m+1} a_i \right) \times \left( \sum_{j=k}^l b_j \right)
        = \left( \sum_{i=n}^m \sum_{j=k}^l a_i \times b_j \right) + \left( \sum_{j=k}^l a_{m+1} \times b_j \right)
        = \left( \sum_{i=n}^{m+1} \sum_{j=k}^l a_i \times b_j \right) .
    \stopformula
    Le résultat reste vrai au rang $m+1$.
    Par récurrrence, il est vrai pour tout entier $m$ tel que $m \geq n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $n$, $m$, $k$ et $l$ quatre entiers naturels tels que $m \geq n$ et $k \geq l$, $a$ une suite d'éléments de $A$ et $f$ une bijection de $[\![n, m]\!]$ vers $[\![k, l]\!]$. 
    Alors,
    \startformula
        \sum_{i=n}^m a_{f(i)} = \sum_{i=k}^l a_i.
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} 
    On procède par récurrence sur $m$. 
    Pour $m = n$, on a $[\![n, m]\!] = \lbrace n \rbrace$, donc on doit avoir $k = l$ (puisque le cardinal de $[\![k, l]\!]$ doit être $1$) la seule bijection (en fait, la seule fonction) de cet ensemble vers $\lbrace k \rbrace$ est la fonction $\lbrace (n, k) \rbrace$. 
    En notant $f$ cette fonction, on a 
    \startformula
        \sum_{i=n}^m a_{f(i)} = a_k = \sum_{i=k}^l a_i.
    \stopformula
    
    Soit $m$ un entier naturel supérieur ou égal à $n$ pour lequel le résultat est vrai.
    Soit $k$ et $l$ deux entiers naturels et $f$ une bijection de $[\![n, m+1]\!]$ vers $[\![k, l]\!]$.
    Soit $p$ l'unique antécédent de $l$ par $f$.
    Supposons d'abord $p \leq m$.
    On a alors : 
    \startformula
        \sum_{i=n}^{m+1} a_{f(i)}
        = \sum_{i=n}^p a_{f(i)} + \sum_{i=p+1}^{m+1} a_{f(i)}
        = \sum_{i=n}^{p-1} a_{f(i)} + a_l + \sum_{i=p+1}^m a_{f(i)} + a_{f(m+1)}.
    \stopformula
    Puisque le groupe $(A, +)$ est abélien, cela donne : 
    \startformula
        \sum_{i=n}^{m+1} a_{f(i)}
        = \sum_{i=n}^{p-1} a_{f(i)} + a_{f(m+1)} + \sum_{i=k+1}^m a_{f(i)} + a_l .
    \stopformula
    Définissons la fonction $g$ de $[\![n, m]\!]$ vers $[\![k, l-1]\!]$ de la manière suivante : pour tout élément $x$ de $[\![n, m]\!]$, $g(x) = f(x)$ si $x \neq p$ et $g(p) = f(m+1)$. 
    On a alors : 
    \startformula
        \sum_{i=n}^{m+1} a_{f(i)}
        = \sum_{i=n}^{p-1} a_{g(i)} + a_{g(p)} + \sum_{i=k+1}^m a_{g(i)} + a_l
        = \sum_{i=n}^p a_{g(i)} + \sum_{i=p+1}^m a_{g(i)} + a_l 
        = \sum_{i=n}^m a_{g(i)} + a_l .
    \stopformula
    En outre, $g$ est bijective. 
    En effet, soit $y$ un élément de $[\![k, l-1]\!]$, 
    \startitemize[nowhite]
        \item Si $y \neq f(m+1)$, son unique antécédent par $g$ est $f^{-1}(y)$ (qui est bien dans $[\![n, m]\!]$ puisqu'il ne peut être égal à $m+1$).
        \item Si $y = f(m+1)$, $p$ est un antécédent de $y$, et tout autre élément $x$ de $[\![n, m]\!]$ ne peut être un antécédent de $y$ (sans quoi on aurait $f(x) = f(m+1)$), donc $y$ admet à nouveau un unique antécédent.
    \stopitemize
    Donc, 
    \startformula
        \sum_{i=n}^{m+1} a_{f(i)}
        = \sum_{i=k}^{l-1} a_i + a_l 
        = \sum_{i=k}^l a_i .
    \stopformula

    Sinon, $p = m+1$. 
    Définissons la fonction $g$ de $[\![n, m]\!]$ vers $[\![k, l-1]\!]$ par $g(x) = f(x)$ pour tout élément $x$ de cet ensemble. 
    Cette fonction est bien définie puisque, pour tout tel élément $x$, $x \neq m+1$ donc $f(x) \neq l$, et est bijective puisque, pour tout élément $y$ de $[\![n, m]\!]$, il existe un unique antécédent $x$ de $y$ par $g$—il s'agit de son unique antécédent par $f$ (qui est bien dans cet ensemble puisqu'il ne peut être égal à $m+1$, puisque $f(m+1) = l$). 
    On a alors : 
    \startformula
        \sum_{i=n}^{m+1} a_{f(i)}
        = \sum_{i=n}^m a_{f(i)} + a_{f(m+1)}
        = \sum_{i=n}^m a_{f(i)} + a_l
        = \sum_{i=k}^{l-1} a_i + a_l
        = \sum_{i=k}^l a_i.
    \stopformula

    Dans les deux cas, le résultat atttendu est vrai au rang $m+1$.
    Par récurrence, il est vrai pour tout entier $m$ supérieur ou égal à $n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau unifère et commutatif, $n$ et $m$ deux entiers naturels tels que $m \geq n$, $a$ une suite d'éléments de $A$ et $f$ une bijection de $[\![n, m]\!]$ vers lui-même. 
    Alors,
    \startformula
        \prod_{i=n}^m a_{f(i)} = \prod_{i=n}^m a_i.
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} Même que ci-dessus en remplaçant $\sum$ par $\prod$ et $+$ par $\times$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $a$ une suite d'éléments de $A$, $n$, $m$, $k$ et $l$ quatre entiers naturels tels que $m \geq n$ et $k \leq l$ et $f$ une fonction de $[\![n, m]\!] \times [\![k, l]\!]$ vers $\mathbb{N}$.
    Alors
    \startformula
        \sum_{i=n}^m \sum_{j=k}^l a_{f(i,j)}
             = \sum_{j=k}^l \sum_{i=n}^m a_{f(i,j)} .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur $m$. 
    Pour $m = n$, on a $\sum_{i=n}^m \sum_{j=k}^l a_{f(i,j)} = \sum_{j=k}^l a_{f(n,j)}$ et $\sum_{j=k}^l \sum_{i=j}^m a_{f(i,j)} = \sum_{j=k}^l a_{f(n,j)}$.
    Le résultat attendu est donc vrai. 

    Soit $m$ un entier naturel supérieur ou égal à $n$ tel que le résultat est vrai.
    On a alors : 
    \startformula
        \startmathalignment
            \NC \sum_{i=n}^{m+1} \sum_{j=k}^l a_{f(i,j)}
            \NC = \left[ \sum_{i=n}^m \sum_{j=k}^l a_{f(i,j)} \right] + \sum_{j=k}^l a_{f(m+1,j)}
                = \left[ \sum_{j=k}^l \sum_{i=n}^m a_{f(i,j)} \right] + \sum_{j=k}^l a_{f(m+1,j)} \NR
            \NC\NC = \sum_{j=k}^l \left[ \sum_{i=n}^{m} a_{f(i,j)} + a_{f(m+1,j)} \right]
                   = \sum_{j=k}^l \sum_{i=n}^{m+1} a_{f(i,j)}. \NR
        \stopmathalignment
    \stopformula
    Le résultat attendu est vrai au rang $m+1$. 
    Par récurrence, il l'est pour tout entier naturel $m$ supérieur ou égal à $n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau unifère et commutatif commutatif, $a$ une suite d'éléments de $A$, $n$, $m$, $k$ et $l$ quatre entiers naturels tels que $m \geq n$ et $k \leq l$ et $f$ une fonction de $[\![n, m]\!] \times [\![k, l]\!]$ vers $\mathbb{N}$.
    Alors
    \startformula
        \prod_{i=n}^m \sum_{j=k}^l a_{f(i,j)}
             = \prod_{j=k}^l \sum_{i=n}^m a_{f(i,j)} .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} Même que la précédente en remplaçant $\sum$ par $\prod$ et $+$ par $\times$.

\blank[medium]

\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau, $a$ une suite d'éléments de $A$, $n$ et $m$ deux entiers naturels tels que $m \geq n$ et $f$ une fonction de $[\![n, m]\!]^2$ vers $\mathbb{N}$.
    Alors
    \startformula
        \sum_{i=n}^m \sum_{j=n}^i a_{f(i,j)}
             = \sum_{j=n}^m \sum_{i=j}^m a_{f(i,j)} .
    \stopformula

\blank[medium]

\noindent\bold{Démonstration :} On procède par récurrence sur $m$. 
    Pour $m = n$, on a $\sum_{i=n}^m \sum_{j=n}^i a_{f(i,j)} = \sum_{j=n}^n a_{f(n,j)} = a_{f(n,n)}$ et $\sum_{j=n}^m \sum_{i=j}^m a_{f(i,j)} = \sum_{i=n}^n a_{f(i,n)} = a_{f(n,n)}$.
    Le résultat attendu est donc vrai. 

    Soit $k$ un entier naturel supérieur ou égal à $n$ tel que le résultat est vrai pour $m = k$.
    On a alors : 
    \startformula
        \startmathalignment
            \NC \sum_{i=n}^{k+1} \sum_{j=n}^i a_{f(i,j)}
            \NC = \left( \sum_{i=n}^k \sum_{j=n}^i a_{f(i,j)} \right) + \sum_{j=n}^{k+1} a_{f(k+1,j)}
            = \left( \sum_{j=n}^k \sum_{i=j}^k a_{f(i,j)} \right) + \sum_{j=n}^k a_{f(k+1,j)} + a_{f(k+1,k+1)} \NR
            \NC\NC = \left( \sum_{j=n}^k \sum_{i=j}^{k+1} a_{f(i,j)} \right) + a_{f(k+1,k+1)}
            = \left( \sum_{j=n}^k \sum_{i=j}^{k+1} a_{f(i,j)} \right) + \sum_{i=k+1}^{k+1} a_{f(k+1,j)}, \NR
        \stopmathalignment
    \stopformula
    d'où
    \startformula
        \sum_{i=n}^{k+1} \sum_{j=n}^i a_{f(i,j)}
        = \left( \sum_{j=n}^{k+1} \sum_{i=j}^{k+1} a_{f(i,j)} \right) .
    \stopformula
    Le résultat attendu est donc vrai pour $m = k+1$.

    Par récurrence, il l'est pour tout entier naturel $m$ supérieur ou égal à $n$.

    \done

\blank[medium]

\noindent\bold{Remarque :} Prenons les mâmes notations et supposons $n = 0$. 
    Puisque la fonction de $[\![0, m]\!]$ vers lui-même qui à tout élément $x$ associe $m-x$ est une bijection (chaque élément $y$ a pour unique antécédent $m-y$), on a : 
    \startformula
        \sum_{i=0}^m \sum_{j=0}^{m-i} a_{f(i,j)}
             = \sum_{i=0}^m \sum_{j=n}^i a_{f(m-i,j)}
             = \sum_{j=0}^m \sum_{i=j}^m a_{f(m-i,j)}
             = \sum_{j=0}^m \sum_{i=0}^{m-j} a_{f(i,j)},
    \stopformula
    où, pour obtenir la dernière égalité, on a utilisé le fait que $x \mapsto m - x$ est une bijection de $[\![j, m]\!]$ vers $[\![0, m-j]\!]$.\footnote{
    En effet, 
    \startitemize[nowhite]
        \item L'image de chaque élément de $[\![j,m]\!]$ est supérieure ou égale à $m - m$, égal à $0$, et inférieure ou égale à $m-j$.
        \item Chaque élément $x$ de $[\![0, m-j]\!]$ a un unique antécédent $m - x$.
    \stopitemize
}

\stopsubsection

\startsubsection[title=Espace vectoriel]

\noindent\bold{Définition (\words{Espace vectoriel}) :} Soit $(K, +, ×)$ un corps et $(E, +)$ un groupe abélien.
    On note $1$ l'élément neutre de $K$ pour $+$.
    Soit $\cdot$ une opération de $K \times E$ vers $E$.
    On dit que $(E, +, \cdot)$ est un \emph{espace vectoriel} sur $(K, +, \times)$ si les trois propriétés suivantes sont satisfaites : 
    \startitemize[nowhite]
        \item \emph{$\cdot$ est distributive à gauche :} Pour tous éléments $k_1$ et $k_2$ de $K$ et tout élément $e$ de $E$, $(k_1 + k_2) \cdot e = (k_1 \cdot e) + (k_2 \cdot e)$.
        \item \emph{$\cdot$ est distributive à droite :} Pour tout élément $k$ de $K$ et tous éléments $e_1$ et $e_2$ de $E$, $k \cdot (e_1 = e_2) = (k \cdot e_1) + (k \cdot e_2)$.
        \item \emph{$\cdot$ est associative avec $\times$ :} Pour tous éléments $k_1$ et $k_2$ de $K$ et $e$ de $E$, $(k_1 \times k_2) \cdot e = k_1 \cdot (k_2 \cdot e)$.
        \item \emph{$1$ est neutre à gauche pour $\cdot$ :} Pour tout élément $e$ de $E$, $1 \cdot e = e$.
    \stopitemize
    Les éléments de $E$ sont parfois appelés \emph{vecteurs}.

\blank[medium]

\noindent\bold{Définition (\words{Combinaison linéaire}) :} ***

\blank[medium]

\noindent\bold{Définition (ecteurs linéairements indépendants) :} ***

\blank[medium]

\noindent\bold{Définition (\words{Base}) :} ***

\blank[medium]

***

\stopsubsection

\stopsection
