\startsection[title=Construction de $\mathbb{Z}$]

\startsubsection[title=Définition]
\index{Nombre entier} \index{Entier} \index{Entier relatif}

On définit l'ensemble $\symbols{\mathbb{Z}}$ par : 
\startformula
...
On note $\symbols{\mathbb{Z}^*}$ l'ensemble $\mathbb{Z} \setminus \lbrace (0,0) \rbrace$.
On qualifie les éléments de $\mathbb{Z}$ d'\emph{entiers} ou \emph{entiers relatifs}, et ceux de $\mathbb{N}$ d'\emph{entiers naturels}. 
...
Le premier est appelé \emph{signe} de l'entier $a$ et le second, aussi noté $\abs{a}$, sa \emph{valeur absolue}.
\index{Valeur absolue} \symbolsRegister{$\abs{\cdot}$}

Notons que, si $a$ et $b$ sont deux entiers tels que $\abs{a} = \abs{b}$ et $\mathrm{sgn}(a) = \mathrm{sgn}(b)$, alors $a = b$. 

Un entier est dit \emph{nul} s'il est égal à $(0,0)$.
...
Notons que, pour tous entiers $a$ et $b$, $a > b$ est équivalent à $\lnot (a \leq b)$ et $a < b$ est équivalent à $a \geq b$.

\blank[medium]

\noindent\bold{Lemme :} La relation $\leq$ est une relation d'ordre sur $\mathbb{Z}$.

\blank[medium]

\noindent\bold{Démonstration :} Vérifions qu'elle satisfait les trois propriétés définissant une relation d'ordre : 
\startitemize[nowhite]
    \item \emph{Réflexivité :} Soit $x$ un entier relatif. 
    ...
    \item \emph{Antisymétrie :} Soit $x$ et $y$ deux éléments de $\mathbb{Z}$ tels que $x \leq y$ et $y \leq x$.
    ...
    \item \emph{Transitivité :} Soit $x$, $y$ et $z$ trois éléments de $\mathbb{Z}$ tels que $x \leq y$ et $y \leq z$. 
        Alors, 
        \startitemize[nowhite]
...
\noindent\bold{Corolaire :} La relation $\geq$ est une relation d'ordre et les relations $<$ et $>$ sont des relations d'ordre strict sur $\mathbb{Z}$.

\blank[medium]

\noindent\bold{Lemme :} La relation $\leq$ est une relation d'ordre total sur $\mathbb{Z}$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $a$ et $b$ deux éléments de $\mathbb{Z}$. 
    ...
    \startitemize[nowhite]
        ...
        \item Si $\ep = 0$, $\eta = 0$ et $\lnot (n \leq m)$, on a $n > m$, donc $m \leq n$, et donc $b \leq a$.
        \item Si $\ep = 1$, $\eta = 1$ et $n \leq m$, on a $b \leq a$.
        \item Si $\ep = 1$, $\eta = 1$ et $\lnot (n \leq m)$, on a $n > m$, donc $m \leq n$, et donc $a \leq b$.
    \stopitemize
...
\noindent\bold{Corolaire :} La relation $\geq$ est une relation d'ordre total sur $\mathbb{Z}$.

\blank[medium]

\noindent\bold{Définitions :} Un entier $x$ est dit : 
\startitemize[nowhite]
    \item \emph{positif}\index{Positif} si $x \geq 0$,
    \item \emph{négatif}\index{Négatif} si $x \leq 0$,
    \item \emph{strictement positif} si $x > 0$,
    \item \emph{strictement négatif} si $x < 0$.
\stopitemize

\stopsubsection

\startsubsection[title=Addition]

\noindent\bold{Définition :} 
    On définit l'opération $\symbols{+}$ sur $\mathbb{Z}$ (vue comme une fonction de $\mathbb{Z} \times \mathbb{Z}$ vers $\mathbb{Z}$) de la manière suivante.
...
\noindent\bold{Lemme :} Soit $z$ un élément de $\mathbb{Z}$. 
    Alors $z + 0 = z$ et $0 + z = z$.

\blank[medium]

\noindent\bold{Démonstration :} Éxaminons tour à tour les deux cas possibles, notant que $0 = (0,0)$ : 
    \startitemize[nowhite]
...
\noindent\bold{Lemme :} L'addition est commutative : pour tous éléments $a$ et $b$ de $\mathbb{Z}$, $a + b = b + a$.

\blank[medium]

\noindent\bold{Démonstration :} Éxaminons les différents cas possibles : 
    \startitemize[nowhite]
...
\noindent\bold{Lemme :} L'addition est associative : pour tous éléments $a$, $b$ et $c$ de $\mathbb{Z}$, $a + (b + c) = (a + b) + c$.
...
\noindent\bold{Lemme :} Soit $n$ et $m$ deux éléments de $\mathbb{Z}$.
    Alors, 
    \startitemize[nowhite]
        \item si $n = (0,0)$, $n + m = m$,
        \item si $n > (0,0)$, $n + m > m$,
...
\noindent\bold{Lemme :} Soit $a$, $b$ et $c$ trois éléments de $\mathbb{Z}$.
...
\noindent\bold{Démonstration :} Si $a$ et $b$ sont de même signe, alors, par définition de l'addition $\abs{a + b} = \abs{a} + \abs{b}$, donc $\abs{a + b} \leq \abs{a} + \abs{b}$.
...
\startsubsection[title=Opposé]

\noindent\bold{Définition :} On définit l'opération $-$ sur $\mathbb{Z}$, vue comme une fonction de $\mathbb{Z}$ vers $\mathbb{Z}$, de la manière suivante : 
\startitemize[nowhite]
    \item $-(0,0) = (0,0)$ ;
    \item soit $n$ un entier naturel non nul, $-(0,n) = (1,n)$.
    \item soit $n$ un entier naturel non nul, $-(1,n) = (0,n)$.
\stopitemize
Pour tout entier $n$, l'entier $-n$ est apelé son \emph{\words{opposé}}.
...
\noindent\bold{Démonstration :} Éxaminons tout à tour les trois cas possibles : 
\startitemize[nowhite]
...
\noindent\bold{Corolaire :} Soit $n$ et $m$ deux éléments de $\mathbb{Z}$. 
...
\noindent\bold{Lemme :} Soit $a$ et $b$ deux entiers. 
    Alors, $-(a+b) = (-a) + (-b)$. 
...
\noindent\bold{Lemme :} Pour tout élément $z$ de $\mathbb{Z}$, on a :
    \startitemize[nowhite]
        \item $z - z = 0$,
        \item $z - 0 = z$,
        \item $0 - z = -z$.
...
\noindent\bold{Démonstration :} Soit $z$ un élément de $\mathbb{Z}$. On a :
    \startitemize[nowhite]
        \item $z - z = z + (-z) = 0$,
...
\noindent\bold{Démonstration :} On a : $(n-m) + m = (n + (-m)) + m = n + ((-m) + m) = n + 0 = n$.

    \done

\blank[medium]

\noindent\bold{Lemme :} Soit $a$ et $b$ deux élément de $\mathbb{Z}$. 
    Alors 
    \startitemize[nowhite]
        \item Si $b > -a$, $a + b > 0$.
        \item Si $b < -a$, $a + b < 0$.
...
\noindent\bold{Démonstration :} Notons d'abords que le premier résultat implique le second. 
...
\noindent\bold{Corolaire :} Soit $a$ et $b$ deux élément de $\mathbb{Z}$. 
    Alors 
    \startitemize[nowhite]
...
\noindent\bold{Lemme :} Soit $a$, $b$ et $c$ trois élément de $\mathbb{Z}$ tels que $b > c$.
    Alors, $a + b > a + c$
...
\startsubsection[title=Multiplication]

\noindent\bold{Définition :} 
    On définit l'opération $\symbols{\times}$ sur $\mathbb{Z}$ (vue comme une fonction de $\mathbb{Z} \times \mathbb{Z}$ vers $\mathbb{Z}$) de la manière suivante.
...
\noindent\bold{Lemme :} Soit $a$ et $b$ deux entiers relatifs.
...
\noindent\bold{Lemme :} La multiplication est commutative : pour tous éléments $a$ et $b$ de $\mathbb{Z}$, $a \times b = b \times a$.

\blank[medium]

\noindent\bold{Démonstration :} Soit $a$ et $b$ deux entiers relatifs. 
...
\noindent\bold{Lemme :} Soit $n$, $m$ et $k$ trois entiers relatifs. 
    Alors, 
    \startitemize[nowhite]
...
\noindent\bold{Lemme :} La multiplication est associative : pour tous éléments $a$, $b$ et $c$ de $\mathbb{Z}$, $a \times (b \times c) = (a \times b) \times c$.

\blank[medium]

\noindent\bold{Démonstration :} On distingue différents cas : 
    \startitemize[nowhite]
...
\noindent\bold{Lemme :} La multiplication est distributive sur l'addition : pour tous éléments $a$, $b$ et $c$ de $\mathbb{Z}$, $a \times (b + c) = (a \times b) + (a \times c)$.
...
\startitemize[nowhite]
    ...
    \item Supposons $\mathrm{sgn}(a) = \mathrm{sgn}(c) = 0$, $\mathrm{sgn}(b) = 1$ et $a \neq 0$. 
        ...
        Cette quantité est égale à $(1, \abs{a} \times \abs{b} - \abs{a} \times \abs{c})$ si $\abs{a} \times \abs{b} > \abs{a} \times \abs{c}$, \emph{i.e.}, si $\abs{b} > \abs{c}$, et à $(0, \abs{a} \times \abs{c} - \abs{a} \times \abs{b})$ sinon.
...
\startsubsection[title=Puissance]

\noindent\bold{\words{Puissance} d'entiers relatifs :} Soit $E$ l'ensemble des fonctions de $\mathbb{Z}$ dans $\mathbb{Z}$. 
...
\noindent\bold{Lemme :} Soit $n$ et $m$ deux entiers naturels. 
    Alors, $(0,m)^n = (0,m^n)$.
...
\noindent\bold{Lemme :} Soit $n$ un entier naturel et $m$ un entier naturel non nul. 
...
\noindent\bold{Démonstration :} On procède par récurrence sur $n$.
    ...
    Soit $n$ un entier naturel tel que $(1,m)^{2n} = (0,m^{2 n})$ et $(1,m)^{2n+1} = (1,m^{2 n + 1})$. 
...
\startsubsection[title=Factoriel]

\noindent\bold{Définition :} Soit $n$ un entier relatif. 
...
\startsubsection[title=Fonctions min et max]
