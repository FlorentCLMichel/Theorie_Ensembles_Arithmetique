\startsection[title=Polynômes]

\startsubsection[title=Définition] 

\noindent\bold{Définition (\words{Polynôme}) :} Soit $(A, +, \times)$ un anneau commutatif. 
    ...
    \startitemize[nowhite]
        \item Définition de $\mathbf{A}$ : Pour tout $x$, $x \in \mathbf{A}$ si et seulement si on peut choisir un entier naturel $n$, appelé \emph{\words{degré}} de $x$ et noté $\mathrm{deg}(x)$, et $n+1$ éléments $a_0, a_1, \dots, a_n$ de $A$ tels que $x = (a_0, a_1, \cdots, a_n)$ et $a_n \neq 0_A \vee n = 0$.
            ...
            \startformula
                ...
                \right) .
            \stopformula
            Le polynôme $(0_A)$ est dit \emph{nul}.
        ...
        \startitemize[nowhite]
            ...
            \item Sinon, on a $n \leq \mathrm{deg}(\mathbf{a}) \vee n \leq \mathrm{deg}(\mathbf{b})$. 
                On a alors trois possibilités.\footnote{
                    On utilise ici le fait que, si $f$ et $g$ sont deux formules, alors $f \vee g$ est équivalente à $(f \wedge g) \vee (f \wedge (\lnot g)) \vee ((\lnot f) \wedge g)$.
...
\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau commutatif et unifère. 
    Alors, $(A[X], +, \times)$ est unifère.

\blank[medium]

\noindent\bold{Preuve qu'il s'agit bien d'un anneau commutatif (et unifère si $(A, +, \times)$ l'est) :} Montrons qu'il s'agit bien d'un anneau, avec pour éléments neutres $(0_A)$ et, si $(A, +, \times)$ est unifère, $(1_A)$, où $1_A$ est l'élément neutre de $A$ pour $\times$. 
...
\startitemize[nowhite]
    \item \emph{$(A[X], +)$ est un groupe abélien :}
        \startitemize[nowhite]
            \item $(A[X], +)$ est un magma puisque $+$ est une loi de composition interne sur $A[X]$.
            \item \emph{L'opération $+$ est commutative :} 
                ...
                Donc, les suites des coefficients de $\mathbf{a} + \mathbf{b}$ et $\mathbf{b} + \mathbf{a}$ sont identiques.
                Donc, $\mathbf{a} + \mathbf{b} = \mathbf{b} + \mathbf{a}$.
            \item \emph{Le polynôme $(0_A)$ est neutre pour $+$ :} 
                Soit $n$ un entier naturel.
                On a : $(\mathbf{a} + \mathbf{0})_n = \mathbf{a}_n + \mathbf{0}_n$.
                En outre $\mathbf{0}_n = 0_A$.
            ...
            \item \emph{L'opération $+$ est associative :} 
                ...
                Cela montre que $\mathbf{d} = \mathbf{e}$.
            \item \emph{Tout élément de $A[X]$ admet un inverse pour l'opération $+$ :}
        ...
        \stopitemize
    \item L'opération $\times$ est une fonction de $A[X] \times A[X]$ vers $A[X]$, donc $(A[X], \times)$ est un magma.
    \item \emph{L'opération $\times$ est commutative :}
        Soit $n$ un entier naturel.
    ...
    \item \emph{L'opération $\times$ est associative :}
        ...
        \startformula
            \mathbf{d}_n = \sum_{i = 0}^n \mathbf{a}_i \times \left( \mathbf{b} \times \mathbf{c} \right)_{n-i}
            = \sum_{i = 0}^n \mathbf{a}_i \times \left( \sum_{j=0}^{n-i} \mathbf{b}_j \times \mathbf{c}_{n-i-j} \right)
            = \sum_{i = 0}^n \mathbf{a}_i \times \left( \sum_{j=0}^{n-i} \mathbf{b}_{n-i-j} \times \mathbf{c}_j \right)
            = \sum_{i = 0}^n \sum_{j=0}^{n-i} \mathbf{a}_i \times \left( \mathbf{b}_{n-i-j} \times \mathbf{c}_j \right),
        \stopformula
        où on a utilisé la bijection de $[\![0, n-i]\!]$ vers lui-même $x \mapsto n-i-x$ pour obteir la seconde égalité.
        Puisque $(A, \times)$ est associatif, cela donne : 
        \startformula
            \mathbf{d}_n = \sum_{i = 0}^n \sum_{j=0}^{n-i} \left( \mathbf{a}_i \times \mathbf{b}_{n-i-j} \right) \times \mathbf{c}_j
                         = \sum_{j = 0}^n \sum_{i=0}^{n-j} \left( \mathbf{a}_i \times \mathbf{b}_{n-i-j} \right) \times \mathbf{c}_j
                         = \sum_{j = 0}^n \left( \mathbf{a} \times \mathbf{b} \right)_{n-j} \times \mathbf{c}_j
                         = \mathbf{e}_n.
        \stopformula
    ...
    \item \emph{L'opération $\times$ est distributive sur $+$ :} 
        ...
        Donc, $\mathbf{d} = \mathbf{e}$.
    \item \emph{Si l'anneau $(A, +, \times)$ est unifère, le polynôme $(1_A)$ est neutre pour $\times$ :}
...
%\noindent\bold{Preuve qu'il s'agit bien d'un anneau commutatif :} Montrons qu'il s'agit bien d'un anneau, avec pour éléments neutres $(0_A)$ et $(1_A)$, où $1_A$ est l'élément neutre de $A$ pour $\times$. 
...
%        Puisque l'opération $\times$ est commutative, cela montrera également $(\mathbf{b} + \mathbf{c}) \times \mathbf{a} = (\mathbf{b} \times \mathbf{a}) + (\mathbf{c} \times \mathbf{a})$.
%        ***
%    \item \emph{Le polynôme $(1_A)$ est neutre pour $\times$ :}
%        ***
%\stopitemize

\blank[medium]

\noindent\bold{Évaluation d'un polynôme :} Soit $(A, +, \times)$ un anneau commutatif et $\mathbf{a}$ un polynôme sur $A$.
    On peut choisir un entier naturel $n$ et $n+1$ éléments $a_0$, $a_2$, ..., $a_n$ de $A$ tels que $\mathbf{a} = (a_0, a_1, \dots, a_n)$.
    Pour tout élément $e$ de $A$, on note $\mathbf{a}(e)$ l'élément $\sum_{i=0}^n a_i e^i$. 

\blank[medium]

\noindent\bold{Remarque :} Soit $\mathbf{a}$ un élément de $A[X]$, $a$ la suite de ses coefficients et $d$ son degré.
    On note $0_A$ l'élément neutre de $A$ pour $+$.
...
\noindent\bold{Lemme :} Soit $(A, +, \times)$ un anneau unifère commutatif et $x$ un élément de $A$.
...
\noindent\bold{Démonstration :} 
    ...
    \startitemize[nowhite]
        ...
        \item Soit $\mathbf{a}$ et $\mathbf{b}$ deux éléments de $A[X]$.
            Notons $\mathbf{c}$ l'élément de $A[X]$ défini par $\mathbf{c} = \mathbf{a} + \mathbf{b}$ et $a$, $b$, $c$ les suites des coefficients de $\mathbf{a}$, $\mathbf{b}$ et $\mathbf{c}$.
            ...
            \stopformula
            Donc, $\mathbf{c}(x) = \mathbf{a}(x) + \mathbf{b}(x)$.
        \item Soit $\mathbf{a}$ et $\mathbf{b}$ deux éléments de $A[X]$.
            Notons $\mathbf{c}$ l'élément de $A[X]$ défini par $\mathbf{c} = \mathbf{a} \times \mathbf{b}$ et $a$, $b$, $c$ les suites des coefficients de $\mathbf{a}$, $\mathbf{b}$ et $\mathbf{c}$.
            Soit $d_a$ le degré de $\mathbf{a}$ et $d_b$ celui de $\mathbf{b}$. 
            Soit $d$ l'entier naturel défini par $d = d_a + d_b$.
            Alors, $d$ est supérieur ou égal aux degrés de $\mathbf{a}$, $\mathbf{b}$ et $\mathbf{c}$ (ce dernier étant égal à $0$ si $\mathbf{a}$ ou $\mathbf{b}$ est le polynôme nul et à $d$ sinon).
            On a donc $\mathbf{a}(x) = \sum_{i=0}^d a_i x^i$, $\mathbf{b}(x) = \sum_{i=0}^d b_i x^i$ et $\mathbf{c}(x) = \sum_{i=0}^d c_i x^i$. 
            Par ailleurs
            \startformula
                \sum_{i=0}^d c_i x^i 
            ...
            \stopformula
            (Où nous avons utilisé le fait que, pour tout élément $j$ de $[\![0, d]\!]$, la fonction définie par $i \mapsto i-j$ est une bijection de $[\![j, d]\!]$ vers $[\![0, d-j]\!]$.)
...
\startsubsection[title=Degré]

\noindent\bold{Définition (\words{Degré}) (rappel) :} Soit $(A, +, \times)$ un anneau commutatif et $\mathbf{a}$ un élément de $A[X]$.
    Soit $n$ un entier naturel et $a_0$, $a_1$, ..., $a_n$ des éléments de $A$ tels que $\mathbf{a} = (a_0, a_1, \dots, a_n)$.
    L'entier naturel $n$ est appelé \emph{degré} de $\mathbf{a}$, et peut être noté $\mathrm{deg}(\mathbf{a})$. 
    Si $a$ est la suite des éléments de $a$ et $0_A$ l'élément neutre de $A$ pour l'opération $+$, alors $d$ est le plus grand entier naturel $i$ tel que $a_i \neq 0_A$, ou $0$ si $a_i = 0$ pour tout entier naturel $i$.

\blank[medium]

\noindent\bold{Lemme :} Soit $\mathcal{A}$ un anneau commmutatif et intègre.
...
\noindent\bold{Démonstration :} Évident d'après la définition de la multiplication :
    \startitemize[nowhite]
        \item avec ces notations, si ni $\mathbf{a}$ ni $\mathbf{b}$ n'est le polynôme nul, alors $a_n \neq 0_A$ et $b_m \neq 0_A$, donc $d_{n+m} \neq 0_A$ (et $d_i = 0_A$ pour tout entier naturel $i$ strictement supérieur à $n+m$ par définition de la multiplication de polynômes) ; 
        \item si $\mathbf{a}$ ou $\mathbf{b}$ est le polyôme nul, alors leur produit est nul, donc de degré $0$.
    \stopitemize

\stopsubsection

\startsubsection[title=Racines]

\noindent\bold{Définition (\words{racine}) :} Soit $(A, +, \times)$ un anneau commutatif et $\mathbf{a}$ un élément de $A[X]$.
    On note $0_A$ l'élément neutre de $A$ pour $+$.
    Un élément $r$ de $A$ est dit \emph{racine} de $\mathbf{a}$ si $\mathbf{a}(r) = 0_A$.

\blank[medium]

\noindent\bold{Remarque :} Notons qu'un polynôme peut, en général, avoir plus de racines distinctes que son degré. 
    Considérons par exemple l'anneau $(\mathbb{Z}_6, +, \times)$ (voir définition \in{section}{}[sec:def_Z_nZ]), d'élément neutre pour $+$ $\bar{0}$, et le polynôme $\mathbf{p} = X^2 - \bar{5} X$. 
